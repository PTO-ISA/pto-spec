...
- Irregular memory shape: Every MGATHER and MSCATTER form exposes mandatory Col, ValidRow, and optional ValidCol=Col; Col drives LB2 and ValidCol drives LB0, while physical Tile Row remains outside the macro interface.
...
- A source range keeps the canonical `[base=GPR, offset=uimm11]` spelling. Assemble variants additionally identify INIT, INIT_LAST, MIDDLE, or LAST because the phase changes allocation and parent-selection semantics.
- An assemble continuation folds only the exact owner-derived phase contract: Local uses a final source-form B.IOT SizeCode=0 `AssembleParent` with physical role `assemble-parent-ref`; Shared retains its owning destination B.IOS SharedTileID with SizeCode=0 and physical role `shared-assemble-destination-reuse`. Each carrier is absent from ordinary semantic sources and handler source arguments; Local capacity is seven ordinary sources plus its carrier, Shared capacity is three plus its reused destination.
- Canonical assemble suffixes are `->T<ParentCapacity>[assemble=INIT,RegSrc+uimm11:WriterSize]`, `->T<ParentCapacity>[assemble=INIT_LAST,RegSrc+uimm11:WriterSize]`, `->T#n[assemble=MIDDLE,RegSrc+uimm11:WriterSize]`, `->T#n[assemble=LAST,RegSrc+uimm11:WriterSize]`, and the analogous `->Sx` forms; ambiguous matches retain physical assembly.
- Representative concrete assemble spellings are `->T<8KB>[assemble=INIT,a0+0:2KB]`, `->T#3[assemble=MIDDLE,a0+2048:2KB]`, `->T#3[assemble=LAST,a0+4096:2KB]`, `->S7<64KB>[assemble=INIT_LAST,zero+0:64KB]`, and `->S7[assemble=LAST,a1+32768:32KB]`.
...
| TileOp | Engine | Canonical macro format |
| --- | --- | --- |
| `MGATHER` | `TLSU` | `MGATHER <Col, ValidRow=1, ValidCol=Col, DataType, PadValue?, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, ->DstTile<Size>` |
| `MGATHER_ADD` | `TLSU` | `MGATHER_ADD <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_AND` | `TLSU` | `MGATHER_AND <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_CAS` | `TLSU` | `MGATHER_CAS <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, SrcTile2, ->DstTile<Size>` |
| `MGATHER_DEC` | `TLSU` | `MGATHER_DEC <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_EXCH` | `TLSU` | `MGATHER_EXCH <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_INC` | `TLSU` | `MGATHER_INC <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_MASK` | `TLSU` | `MGATHER_MASK <Col, ValidRow=1, ValidCol=Col, DataType, PadValue?, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, PredicateTile1, ->DstTile<Size>` |
| `MGATHER_MAX` | `TLSU` | `MGATHER_MAX <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_MIN` | `TLSU` | `MGATHER_MIN <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_OR` | `TLSU` | `MGATHER_OR <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MGATHER_XOR` | `TLSU` | `MGATHER_XOR <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, ->DstTile<Size>` |
| `MSCATTER` | `TLSU` | `MSCATTER <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_ADD` | `TLSU` | `MSCATTER_ADD <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_AND` | `TLSU` | `MSCATTER_AND <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_DEC` | `TLSU` | `MSCATTER_DEC <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_INC` | `TLSU` | `MSCATTER_INC <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_MASK` | `TLSU` | `MSCATTER_MASK <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1, PredicateTile2` |
| `MSCATTER_MAX` | `TLSU` | `MSCATTER_MAX <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_MIN` | `TLSU` | `MSCATTER_MIN <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_OR` | `TLSU` | `MSCATTER_OR <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
| `MSCATTER_POPC` | `TLSU` | `MSCATTER_POPC <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0` |
| `MSCATTER_XOR` | `TLSU` | `MSCATTER_XOR <Col, ValidRow=1, ValidCol=Col, DataType, Layout?, PEMask=AllPE>, [BaseGPR], SrcTile0, SrcTile1` |
