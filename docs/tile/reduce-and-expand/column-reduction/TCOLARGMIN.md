...
After all descriptor and operand checks succeed, the owning ASL handler scans each column in increasing row order and returns the lowest row index holding the minimum. Source payloads are snapshotted before destination writes whenever the contract permits aliasing.
...
For a small `TCOLARGMIN` example, columns `[[1, 4], [3, 2]]` produce lowest winning row indices `[0, 1]`.
<!-- SUPPLEMENTARY-END -->
