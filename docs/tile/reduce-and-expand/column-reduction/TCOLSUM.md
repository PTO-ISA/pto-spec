...
After all descriptor and operand checks succeed, the owning ASL handler folds each column in increasing row order with typed addition. Source payloads are snapshotted before destination writes whenever the contract permits aliasing.
...
For a small `TCOLSUM` example, columns `[[1, 4], [3, 2]]` reduce to `[4, 6]`.
<!-- SUPPLEMENTARY-END -->
