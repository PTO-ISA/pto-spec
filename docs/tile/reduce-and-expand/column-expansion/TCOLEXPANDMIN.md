...
Take the typed minimum of a full-shape source and a broadcast one-row vector.
...
After all descriptor and operand checks succeed, the owning ASL handler selects the typed minimum of each full-shape element and its same-column broadcast value. Source payloads are snapshotted before destination writes whenever the contract permits aliasing.
...
For a small `TCOLEXPANDMIN` example, full-shape row `[1, 5]` with broadcast row `[3, 4]` produces `[1, 4]`.
<!-- SUPPLEMENTARY-END -->
