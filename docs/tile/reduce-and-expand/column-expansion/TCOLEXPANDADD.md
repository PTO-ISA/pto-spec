...
Add a first-row column-broadcast source to a full-shape source with exact typed semantics.
...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: tile-tcolexpandadd-purpose role=purpose -->
## What TCOLEXPANDADD does
...
<!-- PTO-READER-BLOCK: tile-tcolexpandadd-mechanism role=mechanism -->
## Element and Tile mechanism

After all descriptor and operand checks succeed, the owning ASL handler adds each full-shape element to the broadcast value from the same column. Source payloads are snapshotted before destination writes whenever the contract permits aliasing.
...
<!-- PTO-READER-BLOCK: tile-tcolexpandadd-inputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: tile-tcolexpandadd-effects role=effects -->
...
<!-- PTO-READER-BLOCK: tile-tcolexpandadd-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: tile-tcolexpandadd-example role=example -->
## Non-normative worked example

This example illustrates the current ASL owner and does not replace the normative operation.

For a small `TCOLEXPANDADD` example, full-shape row `[1, 2]` plus broadcast row `[10, 20]` produces `[11, 22]`.
<!-- SUPPLEMENTARY-END -->
...
- TCOLEXPANDADD is selected by the TEPL raw encoding carrier Mode 2 Function 21; canonical execution-engine assembly is BSTART.SFU and there is no standalone opcode.
...
- BSTART.SFU TCOLEXPANDADD, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, BroadcastTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
