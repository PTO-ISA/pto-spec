...
Reduce each valid row to its sum with exact typed column-order semantics.
...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: tile-c-trowsum-purpose role=purpose -->
## What TROWSUM does

`TROWSUM` reduces each row to its ordered sum.

<!-- PTO-READER-BLOCK: tile-c-trowsum-mechanism role=mechanism -->
## Operation mechanism
...
<!-- PTO-READER-BLOCK: tile-c-trowsum-inputs-outputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: tile-c-trowsum-effects role=effects -->
...
<!-- PTO-READER-BLOCK: tile-c-trowsum-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: tile-c-trowsum-example role=example -->
## Non-normative example

This example illustrates the current ASL-bound contract and is not a second instruction definition.

`TROWSUM <bundle operands>` snapshots the source and scans each valid row in increasing column order before atomically publishing the row result.
<!-- SUPPLEMENTARY-END -->
...
- TROWSUM computes a typed increasing-column left fold from zero using TADD; the scan order is architectural and tree reassociation is not permitted.

## Legality

- TROWSUM is selected by the TEPL raw encoding carrier Mode 2 Function 0; canonical execution-engine assembly is BSTART.SFU and there is no standalone opcode.
...
- For each valid row, compute a typed increasing-column left fold from zero using TADD.
...
- BSTART.SFU TROWSUM, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
