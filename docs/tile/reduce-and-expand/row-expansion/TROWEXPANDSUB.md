<!-- GENERATED FROM: asl/tile/reduce-and-expand/row-expansion/TROWEXPANDSUB.asl -->
# TROWEXPANDSUB

**Normative ASL source:** `asl/tile/reduce-and-expand/row-expansion/TROWEXPANDSUB.asl`

Subtract a first-column row-broadcast source from a full-shape source with exact typed semantics.
...
`TROWEXPANDSUB <bundle operands>` performs complete preflight and source snapshotting before atomically publishing the mnemonic-defined result and padding state.
<!-- SUPPLEMENTARY-END -->
...
<!-- GENERATED-ASL-BEGIN: decode source=asl/tile/reduce-and-expand/row-expansion/TROWEXPANDSUB.asl -->
```asl
...
<!-- GENERATED-ASL-BEGIN: operation source=asl/tile/reduce-and-expand/row-expansion/TROWEXPANDSUB.asl -->
```asl
...
- TROWEXPANDSUB is selected by the TEPL raw encoding carrier Mode 2 Function 6; canonical execution-engine assembly is BSTART.SFU and there is no standalone opcode.
...
- BSTART.SFU TROWEXPANDSUB, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, BroadcastTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
