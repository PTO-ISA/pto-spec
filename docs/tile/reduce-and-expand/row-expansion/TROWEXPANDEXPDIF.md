...
- For every valid destination element, first compute typed source0 - BroadcastTile[r,0], then compute the same-type natural exponential.
...
- TROWEXPANDEXPDIF is selected by the TEPL raw encoding carrier Mode 2 Function 11; canonical execution-engine assembly is BSTART.SFU and there is no standalone opcode.
...
- BSTART.SFU TROWEXPANDEXPDIF, DataType; B.DATR PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, BroadcastTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
