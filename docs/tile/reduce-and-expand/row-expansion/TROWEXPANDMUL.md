<!-- GENERATED FROM: asl/tile/reduce-and-expand/row-expansion/TROWEXPANDMUL.asl -->
# TROWEXPANDMUL

**Normative ASL source:** `asl/tile/reduce-and-expand/row-expansion/TROWEXPANDMUL.asl`

Multiply a full-shape source by a first-column row-broadcast source with exact typed semantics.
...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: tile-c-trowexpandmul-purpose role=purpose -->
## What TROWEXPANDMUL does

`TROWEXPANDMUL` multiplies by a one-column broadcast across each valid row.

<!-- PTO-READER-BLOCK: tile-c-trowexpandmul-mechanism role=mechanism -->
## Operation mechanism
...
<!-- PTO-READER-BLOCK: tile-c-trowexpandmul-inputs-outputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: tile-c-trowexpandmul-effects role=effects -->
...
<!-- PTO-READER-BLOCK: tile-c-trowexpandmul-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: tile-c-trowexpandmul-example role=example -->
## Non-normative example

This example illustrates the current ASL-bound contract and is not a second instruction definition.

`TROWEXPANDMUL <bundle operands>` performs complete preflight and source snapshotting before atomically publishing the mnemonic-defined result and padding state.
<!-- SUPPLEMENTARY-END -->
...
<!-- GENERATED-ASL-BEGIN: decode source=asl/tile/reduce-and-expand/row-expansion/TROWEXPANDMUL.asl -->
```asl
...
<!-- GENERATED-ASL-BEGIN: operation source=asl/tile/reduce-and-expand/row-expansion/TROWEXPANDMUL.asl -->
```asl
...
- Integer width, floating rounding, exceptional values, signed-zero behavior, and numeric status are exactly the corresponding TMUL typed operation.

## Legality

- TROWEXPANDMUL is selected by the TEPL raw encoding carrier Mode 2 Function 7; canonical execution-engine assembly is BSTART.SFU and there is no standalone opcode.
...
- BSTART.SFU TROWEXPANDMUL, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, BroadcastTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
