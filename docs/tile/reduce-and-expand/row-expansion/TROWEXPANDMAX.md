...
Take the typed maximum of a full-shape source and a first-column row-broadcast source.
...
`TROWEXPANDMAX <bundle operands>` performs complete preflight and source snapshotting before atomically publishing the mnemonic-defined result and padding state.
<!-- SUPPLEMENTARY-END -->
...
- For every valid destination element, compute typed max(source0[r,c], operation-view BroadcastTile[r,0]) at the selected element width.
- Integer width, floating rounding, exceptional values, signed-zero behavior, and numeric status are exactly the corresponding TMAX typed operation.
...
- BSTART.SFU TROWEXPANDMAX, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, BroadcastTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
