...
The ASL DOC contract selects `TileHandler_TMRGSORT` through the instruction's selector-encoded block carrier.
...
`destination0` is the new Local merged destination; `source0` is the persistent sorted Local left source; `source1` is the persistent sorted Local right source; `flag0` is the ascending or descending selection.
...
Read `BSTART.SFU TMRGSORT, FP16; B.IOR a0; B.IOT T0, T1, mask=1111, <last>, ->T0<TSize>; BSTOP` as a non-normative binding walkthrough, then use the generated contract below for exact dimensions, attributes, and fault behavior.
<!-- SUPPLEMENTARY-END -->
