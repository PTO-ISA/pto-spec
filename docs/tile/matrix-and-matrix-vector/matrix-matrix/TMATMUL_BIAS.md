...
Compute A[M x K] times B[K x N], then add one Local resolved-M-layout 1xN private-result Bias into a new Local destination.
...
- AType and BType must be supported ordinary Matrix types from one numeric class. Bias uses the resolved M layout ML (CUBE_M16 or CUBE_M32), matching D and Local A when present; it is one Local logical 1xN accumulator-type source (FP32 for MX). Published Shared operands may replace the right group or both matrix groups; supplementary operands and destinations remain Local.
