...
The ASL DOC contract selects `TileHandler_TGEMV_MX` through the instruction's selector-encoded block carrier.
...
`destination0` is the destination; `source0` is the left-vector; `source1` is the row-scale; `source2` is the right-matrix; `source3` is the column-scale.
