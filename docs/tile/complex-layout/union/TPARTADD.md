<!-- GENERATED FROM: asl/tile/complex-layout/union/TPARTADD.asl -->
# TPARTADD

**Normative ASL source:** `asl/tile/complex-layout/union/TPARTADD.asl`

Combine corresponding source partitions by addition.

## Normative identity {#PTO-INST-TILE-TPARTADD}

<!-- ndf: kind=executable level=L3 layer=tile status=accepted -->

The current instruction contract is owned by the ASL source linked above.

## Assembly

```asm
TPARTADD <bundle operands>
```

## Encoding

| Operation | Family | Selector | Function | Mode | Handler |
| --- | --- | --- | ---: | ---: | --- |
| TPARTADD | TEPL | 0x071 | 17 | 3 | ExecuteTilePartial |

## Operands and results

| Field | Architectural role |
| --- | --- |
| destination0 | destination |
| source0 | source-left |
| source1 | source-right |

## Decode

<!-- GENERATED-ASL-BEGIN: decode source=asl/tile/complex-layout/union/TPARTADD.asl -->
```asl
readonly func InstructionContractOperation_TPARTADD() => TileOperation
begin
    return TileOperation_TPARTADD;
end;
```
<!-- GENERATED-ASL-END: decode -->

## Block composition

```asm
BSTART.TEPL TPARTADD, DataType
B.DATR (optional)
B.DIM LB0
B.DIM (LB1/LB2 for 2D)
B.IOT
BSTOP
```

## Operation

<!-- GENERATED-ASL-BEGIN: operation source=asl/tile/complex-layout/union/TPARTADD.asl -->
```asl
readonly func InstructionContractHandler_TPARTADD() => TileSemanticHandler
begin
    return TileHandler_ExecuteTilePartial;
end;
```
<!-- GENERATED-ASL-END: operation -->

## Legality and exceptions

- **Legality handler:** `TileOperandsLegal_ExecuteTilePartial`
- **Fault contract:** `ExecuteTileInstruction`
- **Datr contract:** `{"allowed_nonzero_fields": [], "pad_union": "must-zero"}`

## Operational information

- **Semantic handler:** `ExecuteTilePartial`
- **Effect contract:** `ExecuteTilePartial`
- **Restart contract:** `CompleteBundleAtWithAcceptedApplicabilityRules`
- **State effects:** `["operand:destination0:destination", "operand:source0:source-left", "operand:source1:source-right"]`

<!-- SUPPLEMENTARY-BEGIN -->

<!-- SUPPLEMENTARY-END -->
