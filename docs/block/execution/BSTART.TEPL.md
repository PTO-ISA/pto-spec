...
`BSTART.TEPL` is a 32-bit block-start command for the TEPL form. It establishes the pending block identity and selectors; the completed block, not the start command alone, owns body execution and result commitment.
...
- `Mode` supplies the named selector or attribute field; its exact assigned domain remains in the generated contract below.
- `Function` supplies the named selector or attribute field; its exact assigned domain remains in the generated contract below.
...
Assume predecessor retirement and target checks succeed. `BSTART.TEPL Mode, Function, DataType` opens the pending `BSTART.TEPL` form; subsequent header/body commands remain provisional until `BSTOP` or the next `BSTART` validates the complete composition.
<!-- SUPPLEMENTARY-END -->
