...
`ESAVE` identifies an extension-owned raw carrier family that PTO inventories but never accepts for execution.

<!-- PTO-READER-BLOCK: block-esave-mechanism role=mechanism -->
## Placement and execution mechanism

Every raw carrier matching the `ESAVE` family is reserved in PTO; it is not a standalone command or a Block-body member.
