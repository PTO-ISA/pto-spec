...
`BSTOP` is a Block completion boundary that validates and commits the active descriptor before selecting the next architectural PC.

<!-- PTO-READER-BLOCK: block-bstop-mechanism role=mechanism -->
## Placement and execution mechanism

`BSTOP` is not a body attribute: it consumes the already active Block and is illegal when no compatible Block is active.
