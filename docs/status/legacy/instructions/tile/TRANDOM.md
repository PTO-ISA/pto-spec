...
{
  "schema_version": 1,
  "id": "intrinsic.trandom",
  "kind": "intrinsic",
  "title": "TRANDOM Intrinsic",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TRANDOM.md",
    "pto": "tile/ops/irregular-and-complex/trandom.md"
  },
  "opcode": "TRANDOM",
  "family": "complex-special",
  "bundle": "BSTART.TEPL TRANDOM, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT\nB.IOR",
  "operands": {
    "output": "dst random tile",
    "input0": "TRandomKey via B.IOR",
    "input1": "TRandomCounter via B.IOR",
    "input2": null
  },
  "dtypes": [
    "S32",
...
- PTO 来源页：[`../pto/TRANDOM.md`](../pto/TRANDOM.md)
- PTO 指令名：`TRANDOM`
- C++ intrinsic / RecordEvent 签名摘录：

```cpp
template <typename DstTile, typename... WaitEvents>
PTO_INST RecordEvent TRANDOM(DstTile &dst, TRandomKey &key, TRandomCounter &counter, WaitEvents &... events);
```

PTO/DavinciOO 语义摘要：使用 key/counter state 生成 counter-based pseudo-random values 到 destination tile。
...
```asm
TRANDOM <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType>, key, counter, ->DstTile<Size>
```
...
```asm
BSTART.TEPL TRANDOM, U32
B.DATR      PadValue.Null       # 可省略；省略时使用默认 data attribute
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TRANDOM 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TRANDOM 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TRANDOM 需要
B.IOT       mask=1111, last, ->T<1KB>
B.IOR       rKeyLo, rKeyHi, rCounter, ->-
```
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `TRANDOM` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
- `TRandomKey` 与 `TRandomCounter` 通过 `B.IOR`/profile operand 绑定；它们不是普通 Tile source。

## Header 展开说明

| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TRANDOM` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
- 二维 `TRANDOM` 的 `ValidCol/ValidRow/Col` 均为 16-bit dimension 值，并应满足 `ValidCol <= Col`。
- A5 要求 `DstTile::DType` 为 `int32_t` 或 `uint32_t`，且 destination row-major。
- `Rounds` 必须为 7 或 10；默认 10。
- `key` 与 `counter` 必须有效且非空。
- 本指令不隐含 memory ordering、cross-PE visibility 或 group-level barrier。

## Lowering 摘要

1. Frontend 从 PTO `TRANDOM` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride、profile/scalar 参数和 destination allocation size。
...
5. Execute 阶段按 PTO `TRANDOM` 语义执行；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
