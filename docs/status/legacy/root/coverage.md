...
hardware or that the named 0.58.0 hardware contract has implementation
conformance. It also does not mean release review is complete. The staged exit criteria
and owned residual gaps are in `docs/status/legacy/root/maturity-bringup-plan.md` and
...
- The named 0.58.0 hardware numeric contract is normative, but no implementation
...
clone-verifiable release gates, proves the pinned least-privilege manual
exact-head workflow and exact 53-shard/113-call/107-subprogram topology, and enumerates ten
