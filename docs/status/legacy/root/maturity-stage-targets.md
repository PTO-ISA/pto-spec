...
| `S6-T2-B` — execution topology | Closed | Prove the manual exact-head release workflow and shard suite are bounded and exact. | Full action pins, contents-read permission, required `Release / validate` aggregator, no cancellation, 360-minute timeout, and 53 shards covering 113 calls/107 subprograms. |
...
| `S6-T2-D` — candidate reproduction | Waiting on C | Execute every gate without moving the candidate. | Ten local results and passing hosted `Release / validate` all name the candidate commit. |
...
- `docs/status/legacy/root/maturity-bringup-plan.md` explains the rationale and sequencing.
