...
`preview-change` 写入 `/tmp`，不覆盖受 Git 跟踪的 Complete；`build-html` 只生成候选站点，验证后再同步到 `docs/status/legacy/html/`。
...
- Complete HTML 入口：`docs/status/legacy/DavinciOO_PTO_Intrinsic_Complete.html`
