...
For the reviewer-oriented stage gates and target register, see
[Maturity evaluation and staged targets](maturity-stage-targets.md).
...
conformance remain open in Stage 5. Passing `make pr-check` is necessary for
integration but does not prove semantic or maturity closure; promotion requires
the exact-head `make release-verify` and manual release workflow evidence.
...
| Stage 1 | Closed | `S1-T1` and `S1-T2` close exact bundle-start descriptors, transactional commit, direct tile launch, timing, and rollback. `S1-T3` closes stable form-ID decoded before/after evidence for all 474 scalar forms across AGU, ALU, AMO, BRU, FSU, and SYS. `S1-T4` closes the shared scalar/command/tile execution-attempt status, one-tick, stale-fault isolation, trap-preservation, and legality no-effect contract. `S1-T5` closes selector-to-handler and operand binding for all 109 direct-tile operations. | No Stage 1 target remains open; deeper family totality remains in Stages 4–5. |
| Stage 2 | Closed | The 72-register behavior and all-bank reset contract (`S2-T1`), complete nonzero-seed architectural reset (`S2-T2`), all 13 trap dispositions and extended recovery envelopes (`S2-T3`), separate 64-bit MPAR/MSEQ execution-mask and eight-register 32-bit warp-predicate contract plus current-public v0.6 typed-mask reconciliation (`S2-T4`), precise tile capacity/storage (`S2-T5`), element-level definedness, and explicit tile handoff (`S2-T6`) execute. | No Stage 2 target remains open. The public v0.6 `!pto.mask<G>` and vector surface is pinned and classified outside the accepted PTO ISA 0.58.0 release line; no physical predicate-register mapping, `V.CMP.* ->p` producer, or dimension/tail group initialization is claimed. |
...
| Stage 4 | Closed | All scalar, bundle, TEPL, TLSU, and CUBE targets have checked selector/form inventories, legality, state/effect, alias, boundary, and pre-effect rejection evidence under `pto-v0`. | No Stage 4 target remains open; target numerical conformance remains in Stage 5. |
...
| Stage 6 | Open | The generated S6-T1 ledger closes exact traceability inventory/link coverage across 925 units. The generated S6-T2 ledger closes the ten-gate contract, manual exact-head workflow policy, and exact 53-shard/113-call/107-subprogram topology. | S5-T2 leaves 11 requirements and 28 hooks open; no immutable candidate, complete local/hosted results, protected-branch snapshot, or architecture/formal approvals exist. |
...
| --- | --- | --- | --- | --- |
| `S0-T1` | 0 | Closed | Freeze and regenerate the 474 scalar, 99 bundle/command, and 109 direct-tile accepted-form inventories. | Deterministic generation diff, catalog checks, positive and reserved-code witnesses |
...
| `S1-T1` | 1 | Closed | Bind all 74 bundle-start forms to an operation-bearing descriptor or an explicit pre-state unsupported result; no decoded selector or modifier is discarded. | ADR 0022; generated exact-form descriptor witnesses; DataType, selector, Mode, and BrType sensitivity; reserved selector/type/family no-install tests |
...
| `S2-T4` | 2 | Closed | Define architectural use or reserved behavior for the eight 32-bit warp predicates and the separate 64-bit MPAR/MSEQ execution mask; keep the current public v0.6 typed-mask source surface outside PTO ISA 0.58.0; no visible predicate-domain state remains accidental. | ADRs 0046 and 0051 plus P0 hardwiring, P1/P7 width boundaries, all-nine bundle-kind domain selection, branch-source selection, trap preservation, reset, no-encoding-consumer tests, and exact-pin/source-class predicate-version evidence |
...
| `S3-T1` | 3 | Closed | Emit normative PTO-TSO events from every production scalar and tile load, store, atomic, and fence path. | ADR 0020 plus event-kind/address/size/agent/order/program-order assertions for scalar singles/pairs, LR/SC, RMW/CAS, DMA, fences, and every TLSU access class |
...
| `S4-T7` | 4 | Closed | Replace every bundle-command placeholder, truncated operand, fixed-zero surrogate, and unused binding with a defined effect or explicit rejection. | ADR 0032; generated 99-form consumed-field/effect matrix; exact 89 executed/10 rejected dispositions; checked bundle-to-tile representability; decoded retirement and rollback tests |
| `S4-T8` | 4 | Closed | Close TEPL legality, valid-region effects, layouts, aliases, exceptional values, and management behavior. | ADR 0035; exact 87/41 accepted/reserved selector partition; deterministic decoded state matrix; 25 raw-carrier types; alias, preserved-region, invalid-index, sort/merge, histogram, and management evidence; numeric conformance remains S5-T2 |
| `S4-T9` | 4 | Closed | Close TLSU precise accesses, masks, restart, atomicity, ordering, fault precedence, and sub-byte transfer disposition. | ADR 0033; all-ten decoded-selector matrix; packed nibble, duplicate-index, masked, CAS, event, and first/middle/last fault-preflight evidence |
| `S4-T10` | 4 | Closed | Close CUBE type/shape combinations, accumulation, rounding, saturation, aliases, and composite preflight. | ADR 0034; all-12 decoded selector matrix; all-25 raw-carrier types; mixed layout/location, alias, and composite no-partial-effect matrices; numeric conformance remains S5-T2 |
| `S5-T1` | 5 | Closed | Inventory every operation whose result depends on numeric behavior beyond the raw-carrier reference profile. | Checked `spec/evidence/numeric-contracts.json`: 19 scalar forms, 85 direct-tile operations, 28 hooks, an owner per row, and explicit `S5-T2` conformance obligations |
...
| `S5-T3` | 5 | Closed | Cross-check every shared scalar mnemonic and architectural pattern against a pinned independent executable ISA model, then resolve each difference as a PTO rule, profile difference, defect, or intentional non-equivalence. | The 682-row publication-safe disposition matrix is complete with 554 exact encoding matches, 90 explicit divergences, and 38 non-comparable rows; the clean content-addressed snapshot passes all eight repository gates and the pinned Sail parser/C-backend gate. Independent executable parity is tracked separately and remains 0/38. |
| `S6-T1` | 6 | Open | Prove requirements-to-model-to-test traceability with no unsupported completeness claim. Exact inventory and link sub-stages are closed over 925 units; cumulative closure and review remain open. | Generated release-traceability readiness ledger; closed S5-T2-dependent requirement statuses; immutable-commit evidence-hygiene review |
| `S6-T2` | 6 | Open | Pass clean regeneration, ASLRef, repository, publication, protected-branch, and independent architecture/formal review gates. Gate-contract and topology sub-stages are closed; candidate execution and approval remain open. | Generated release-gate readiness ledger; clean `make release-verify` and `make release-prepare`; `git diff --check`; hosted `Release / validate`; GitHub control snapshot; recorded approvals at one signed commit |
...
trap-preserved state without a PTO ISA 0.58.0 instruction consumer.
...
- Reconcile `docs/status/legacy/root/coverage.md` and `specification.toml` with the distinction
...
- Preserve the current catalog baseline: 474 scalar forms, 99 bundle/command
  forms, 109 direct tile operations, 72 system registers, and 13 trap numbers.
...
- Cover every semantically distinct equivalence class across all 474 scalar
  forms, 99 command forms, and 109 tile selectors.
...
| `S4-T7` | Bundle commands | Dimensions, attributes, IO, hints, launch, save/recover, frames, queues, copies, and transfers | Closed: ADR 0032 and `bundle-command-totality.json` give all 99 forms an exact consumed-field and executed-or-rejected disposition, with transactional rollback evidence. |
...
| `S4-T9` | TLSU | Load/store/move/prefetch/gather/scatter/masked/CAS/GMOV | Closed: ADR 0033 and `tlsu-totality.json` cover all ten selectors, packed sub-byte transfer, duplicate lanes, masks, events, atomicity, restart, and first/middle/last preflight faults. |
| `S4-T10` | CUBE | Explicit-destination base, bias, accumulate, and MX matrix/vector forms | Closed: ADRs 0034 and 0052 plus `cube-totality.json` cover all 12 accepted operations, 20 reserved selectors, 25 raw carriers, layouts, aliases including D == C read-old/write-new behavior, and composite preflight; target numeric results remain `S5-T2`. |
...
0033 close the AMO, SYS, and TLSU value, fault, restart, and event contracts.
...
- `docs/status/legacy/root/coverage.md` can justify `complete under PTO v0` per group without
...
The promotion sequence is deliberately split so partial evidence cannot be
mistaken for M5 closure:

| Checkpoint | Current target | Closed when |
| --- | --- | --- |
| `S5-T2-A` — decide PTO numeric rules | In progress; A1, A4, A5, and A6 are closed | All 12 decisions, 20 domain rules, profile support matrices, selected routes, and bounded results are accepted. |
| `S5-T2-B` — qualify oracles | Waiting on A | Six independent oracle identities, versions, domains, limitations, and invocation records are immutable. |
| `S5-T2-C` — freeze vectors | Waiting on A–B | Every applicable operation has deterministic normal, boundary, exceptional, rounding, saturation, and accumulation vectors. |
| `S5-T2-D` — execute differentials | Waiting on B–C | All 18 domains, 28 hooks, and 104 operations have reproducible raw PTO and oracle results. |
| `S5-T2-E` — adjudicate | Waiting on D | No result is missing, duplicated, or unclassified; every mismatch has an accepted PTO disposition and regression. |
| `S5-T2-F` — promote | Waiting on A–E | Clean reproduction, immutable evidence, all repository gates, and independent architecture/formal approvals close together. |

Within A, ADR 0044 closes only the conditional integer-result subset: 48
unequal-width public integer `TCVT` tuples and three result rules. It does not
close target support, same-width conversions, floating conversions,
overflow/saturation, rounding/flags, vectors, PD-07, any complete domain rule,
or `S5-T2-A` itself.

ADR 0048 closes only the shared value-classification prerequisite. It provides
bit-exact classification and canonical-NaN helpers for all 25 formats without
wiring those helpers into `pto-v0` arithmetic. It therefore does not close
operation-specific NaN propagation, flags, legality, PD-02, PD-05, or
`S5-T2-A`.

ADR 0049 closes PD-04 only for the named hardware profile. It fixes input
preservation, gradual underflow, after-rounding tininess, and fail-closed
configuration selection over 95 operations and eleven formats. It does not
create operation/type support, add arithmetic conformance vectors, close a
complete domain, promote the hardware profile into the generic PD-12 selector
catalog, or change `pto-v0`.

ADR 0050 adds a bounded PD-05-SC2 special-value checkpoint for the named
hardware profile. It fixes produced canonical NaNs, tile comparison
NaN/signed-zero results, and scalar/tile MIN/MAX NaN/signed-zero results across
eight operations and 154 conditional operation/type rows. It does not create
operation/type support, change `pto-v0`, close PD-05, close a complete domain,
or select another generic variation route. Infinity arithmetic, broader NaN
creation, conversions, reductions, quantization, matrix results, and full
flag/status behavior remain under `S5-T2-A`.
...
promotion input. It covers globally unique traceability units: 47
requirements, 474 scalar forms, 99 bundle/command forms, 109 direct tile
operations, 72 system registers, 13 traps, 36 profile hooks, and 75 top-level
ASL state roots. Composite roots expand to 235 leaf fields so nested bundle,
...
| `S6-T1-C` | Blocked by S5-T2 | Cumulative Stage 0–5 closure | The 11 S5-T2-dependent requirement rows and all 28 numeric hooks close with accepted conformance evidence. |
...
manual exact-head workflow uses full action pins and least-privilege contents access, and
proves 53 shards partition 113 canonical calls reaching 107 test subprograms.
...
| `S6-T2-B` | Closed | Manual exact-head and shard execution contract | Full action pins, least-privilege workflow, required `Release / validate` aggregator, 360-minute bound, and exact 53/113/107 partition pass fail-closed checks. |
...
| `S6-T2-D` | Waiting on C | Reproduce the candidate | Every local gate and hosted `Release / validate` passes at the same candidate commit. |
...
- `docs/status/legacy/root/coverage.md`, `specification.toml`, profile metadata, generated
...
- The pinned ASLRef gate, deterministic regeneration, script checks,
  publication-hygiene checks, and exact-head `Release / validate` all pass.
