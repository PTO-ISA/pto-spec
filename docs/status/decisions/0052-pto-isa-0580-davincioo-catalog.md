...
`docs/tile/`.  The catalogs and ASL remain normative; each page is
the single human-facing description for that operation.  Generated Complete
HTML from that release is retained only as non-normative history under
`docs/status/legacy/`. The active reference is generated through MkDocs from
the exact ASL mirror.
