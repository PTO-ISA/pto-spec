...
The eight conditional branch forms reserved by Decision 161 in ADR-SCALAR-0006 are excluded from the
...
three assigned cache-hint models from Decision 148 in ADR-SCALAR-0006. These repairs change decoded
...
The mnemonic audit left a broad set of scalar, system, frame, memory-template, prefetch, condition-setting, and queue operations without one decision-scoped owner. Each needed explicit operands, defaults, legality, fault ordering, and publication behavior rather than relying on mnemonic names or implementation convention.

助记符审计留下了一组缺少单一决策 owner 的标量、系统、帧、内存模板、预取、条件设置和队列操作。每项都需要明确操作数、默认值、合法性、故障顺序和发布行为，不能依赖助记符名称或实现惯例。

### Detailed decision / 详细决策

The numbered decisions close the affected catalog family by family: cache hints and prefetch remain non-faulting hints; frame and `MCOPY` templates receive restartable boundaries; GQM operations receive common selector, initialization, missing-queue, acquire/release, and atomicity rules; condition setters bind to BARG; system-register placement and local reads are explicit; pair-result remainder forms publish in the recorded order.

编号决策按族闭合受影响目录：缓存提示与预取保持非故障提示；帧及 `MCOPY` 模板获得可重启边界；GQM 操作获得通用 selector、初始化、队列缺失、acquire/release 和原子性规则；条件设置绑定到 BARG；系统寄存器位置与本地读取显式化；双结果余数形式按记录顺序发布。

### What changed / 改动内容

#### English

- Converted the reviewed mnemonic set into explicit operation-level contracts and reservations.
- Closed operand, default, restart, atomicity, fault-order, and result-publication gaps across the listed units.

#### 中文

- 将已审计助记符集合转化为明确的操作级契约与编码保留。
- 闭合所列单元中的操作数、默认值、重启、原子性、故障顺序和结果发布缺口。

### Scope and boundaries / 范围与边界

This ADR is a decision-scoped consolidation of the listed operations, not permission to infer semantics for unlisted mnemonics. It preserves the encodings, widths, signedness, and retirement rules explicitly stated as unchanged in each decision.

本 ADR 是对所列操作的决策范围整合，不授权推断未列助记符的语义。它保留各决策明确声明不变的编码、宽度、有符号性和退役规则。
