---
{
  ...
  "title_zh": "标量 SYS 全域语义与 PTO v0 Profile 边界",
  "status": "accepted",
  "authors": [
...
Decoded reachability did not prove all system-register transfer shapes, access failures, maintenance operands, scheduler requests, ACRE modes, profile-gated registers, or the boundary between ACRC execution and bundle placement.

可解码覆盖不能证明全部系统寄存器传输形状、访问失败、maintenance 操作数、调度请求、ACRE 模式、Profile 门控寄存器，或 ACRC 执行与 Bundle 放置之间的边界。

### Detailed decision / 详细决策

System-register transfers resolve full addresses and preflight access before any read side effect or destination update; SSRSWAP is one alias-safe transaction. Maintenance and fence classes have explicit completion and privilege rules without claiming physical cache, TLB, scheduler, MMU, or debug machinery. ACRE modes share the recorded behavior, and ACRC placement remains separately owned.

系统寄存器传输先解析完整地址并预检访问，再允许读取副作用或目的更新；SSRSWAP 是一次别名安全事务。maintenance 和 fence 类具有显式完成与权限规则，但不声称存在物理 cache、TLB、scheduler、MMU 或 debug 机制。ACRE 模式共享已记录行为，ACRC 放置规则仍由其他记录所有。

### What changed / 改动内容

#### English

- Closed the system-register transfer matrix, access classes, aliases, and rejection rollback.
- Defined maintenance, fence, scheduling, and recovery dispositions within the executable boundary.
- Kept unmodeled physical structures and bundle-formation legality outside instruction-local semantics.

#### 中文

- 闭合系统寄存器传输矩阵、访问类别、别名和拒绝回滚。
- 在可执行边界内定义 maintenance、fence、调度和恢复处置。
- 将未建模的物理结构与 Bundle formation 合法性留在指令局部语义之外。

### Scope and boundaries / 范围与边界

This record does not model physical cache, TLB, scheduler, MMU, or debug hardware. It also does not reopen ACRC service semantics or decide its bundle placement.

本记录不对物理 cache、TLB、scheduler、MMU 或 debug 硬件建模，也不重新定义 ACRC 服务语义或决定其 Bundle 放置。
