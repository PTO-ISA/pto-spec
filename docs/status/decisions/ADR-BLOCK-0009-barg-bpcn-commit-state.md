---
{
  ...
  "title_zh": "Bundle 提交状态使用 BARG/BPCN",
  "status": "accepted",
  "authors": [
...
---
# ADR-BLOCK-0009: Bundle commit state uses BARG/BPCN
...
Bundle start, condition setting, and stop must share one architecturally visible continuation decision. A single BARG/BPCN state path prevents hidden branch state and makes snapshot, retry, and commit behavior testable.

Bundle 启动、条件设置和停止必须共享一个架构可见的 continuation 决策。统一的 BARG/BPCN 状态路径可避免隐藏分支状态，并使快照、重试和提交行为可测试。

### Detailed decision / 详细决策

`BSTART` initializes BARG, `SETC.*` or `C.SETC.TGT` records the selected condition or target in the BARG/BPCN view, and the stop form consumes that committed decision to choose continuation. The state participates in the existing preflight, selected-path installation, trap, and retry boundaries described by the affected units.

`BSTART` 初始化 BARG，`SETC.*` 或 `C.SETC.TGT` 在 BARG/BPCN 视图中记录所选条件或目标，停止指令消费该已提交决策以选择 continuation。该状态遵循受影响单元定义的既有预检、选中路径安装、陷阱和重试边界。

### What changed / 改动内容

#### English

- Unified Bundle continuation state under BARG/BPCN.
- Bound start, condition setters, and all stop forms to one commit interface.

#### 中文

- 将 Bundle continuation 状态统一到 BARG/BPCN。
- 将启动、条件设置指令和所有停止形式绑定到一个提交接口。

### Scope and boundaries / 范围与边界

This ADR defines continuation-state ownership and transfer. It does not add a separate branch family, and later selected-boundary corrections remain part of this same interface.

本 ADR 定义 continuation 状态的归属与传递；不新增独立分支族，后续 selected-boundary 修正仍属于同一接口。
