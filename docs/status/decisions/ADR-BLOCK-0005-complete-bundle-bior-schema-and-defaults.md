---
{
  ...
  "title_zh": "完整 Bundle 的 B.IOR 模式与默认值",
  "status": "accepted",
  "authors": [
...
Complete Bundle execution needs one deterministic mapping from encoded `B.IOR` fields to each operation's scalar and address operands. Implicit defaults, unused fields, repeated selectors, and omission otherwise create decoder and toolchain disagreement.

完整 Bundle 执行需要将编码的 `B.IOR` 字段确定地映射到各操作的标量与地址操作数。若省略值、未使用字段、重复选择器和缺省规则不明确，解码器与工具链会产生分歧。

### Detailed decision / 详细决策

The operation schema determines whether `B.IOR` is present, which fields are used, and what omission means. Encoded zero names the architectural `zero` register; unused nonzero fields are rejected. A valid binding is preserved until the complete operation passes preflight and commits, and a second `B.IOR` does not silently overwrite the first.

操作模式决定 `B.IOR` 是否存在、使用哪些字段以及省略时的含义。编码零指向架构 `zero` 寄存器；未使用字段若非零则拒绝。有效绑定在完整操作通过预检并提交前保持不变，第二条 `B.IOR` 不会静默覆盖第一条。

### What changed / 改动内容

#### English

- Closed presence, arity, default, encoded-zero, and unused-field rules for `B.IOR`.
- Added fail-closed preservation and preflight behavior across affected Bundle schemas.

#### 中文

- 闭合 `B.IOR` 的存在性、元数、默认值、编码零和未使用字段规则。
- 为相关 Bundle 模式增加失败关闭的保留与预检行为。
