...
Encoding ownership had three defects: removed spellings looked reserved, extension reservations were incomplete, and Shared TLSU reused one PE's scalar values for all quarters. These defects could reallocate protected encodings or calculate the wrong GM addresses.

编码所有权存在三项缺陷：已删除拼写看起来仍占用保留空间，扩展保留不完整，Shared TLSU 还把一个 PE 的标量值复用于所有 quarter。这些缺陷可能重新分配受保护编码或计算错误的 GM 地址。

### Detailed decision / 详细决策

PTO owns accepted encodings and reserves the complete two-level vector starts and 64-bit vector opcode root without assigning PTO semantics. Deleted spellings are not reservations. For TLOAD/TSTORE, B.IOR selects a GM base and row stride through absolute GPR selectors resolved separately in each selected PE's private register file, with explicit mask, preflight, packed-four-bit, and conflict rules.

PTO 拥有已接受编码，并完整保留两级 Vector start 和 64 位 Vector opcode root，但不分配 PTO 语义。已删除拼写不是保留项。对 TLOAD/TSTORE，B.IOR 通过绝对 GPR 选择器指定 GM 基址和行步长，并在每个选中 PE 的私有寄存器文件中分别解析，同时遵循显式 mask、预检、四位打包和冲突规则。

### What changed / 改动内容

#### English

- Moved extension reservation ownership into ASL and completed the protected roots.
- Distinguished permanently deleted spellings from reserved encodings.
- Changed Shared TLSU base/stride resolution to each selected PE's private GPRs.

#### 中文

- 将扩展保留所有权移入 ASL，并补全受保护根空间。
- 区分永久删除拼写与保留编码。
- 将 Shared TLSU 基址/步长解析改为使用各选中 PE 的私有 GPR。

### Scope and boundaries / 范围与边界

The record does not assign execution semantics to vector reservations or order selected PEs. Software must avoid conflicting GM regions, and current instruction meaning remains in owning ASL/NDF clauses.

本记录不为 Vector 保留空间分配执行语义，也不规定选中 PE 之间的顺序。软件必须避免冲突的 GM 区域；当前指令含义仍归 owning ASL/NDF 条款所有。
