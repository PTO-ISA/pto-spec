...
---
# ADR-BLOCK-0014: Block start and extension reservations
...
`BSTART.FP ICALL` are deleted; calls use Decision 034 in ADR-BLOCK-0014 and Decision 035 in ADR-BLOCK-0014 respectively,
...
deleted; Decision 034 in ADR-BLOCK-0014 and Decision 035 in ADR-BLOCK-0014 are the only call forms.
...
Block entry forms determine control transfer, BARG initialization, body model, and reserved encoding ownership. Mixing accepted PTO starts with machine-parallel, machine-sequential, or Fixup payloads would leave incompatible execution models active under one catalog.

Block 进入形式决定控制转移、BARG 初始化、Body 模型和保留编码归属。若把已接受 PTO 启动与机器并行、机器顺序或 Fixup 载荷混在同一目录，会同时激活不兼容的执行模型。

### Detailed decision / 详细决策

The record closes the public `BSTART`, fused call/indirect-call, FP, STD, SYS, compressed, and long-displacement forms and their continuation behavior. It reserves `B.TEXT`, Fixup payloads, and all machine-parallel and machine-sequential starts, leaving them without PTO body-entry, vector-size, or execution-mask semantics.

本记录闭合公开 `BSTART`、融合直接/间接调用、FP、STD、SYS、压缩和长位移形式及其 continuation 行为。它保留 `B.TEXT`、Fixup 载荷以及全部机器并行和机器顺序启动，使其不具有 PTO Body 进入、向量大小或执行掩码语义。

### What changed / 改动内容

#### English

- Closed accepted start-form target selection, BARG initialization, predecessor retirement, and commit behavior.
- Reserved incompatible extension and machine-body encoding families as complete collision-protected spaces.
- Distinguished active PTO start forms from reserved encodings before any field interpretation or state effect.

#### 中文

- 闭合已接受启动形式的控制与提交行为。
- 将不兼容的扩展和机器 Body 编码族作为完整空间保留。
