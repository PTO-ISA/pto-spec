...
---
# ADR-NUM-0006: Public numeric type identity and target availability
...
**English.** Familiar public names and backend carriers could otherwise be
mistaken for architectural types or target support. Checked bindings separate
published identity from catalog-only types and operation legality.

**中文。** 熟悉的公开名称和 backend carrier 可能被误认为架构类型或目标支持。受检
绑定将公开标识、仅目录类型和操作合法性分开。

### Detailed decision / 详细决策

**English.** Sixteen public identities bind to exact PTO catalog identities;
A2/A3 and A5 receive stated type-availability baselines. Nine other catalog
types retain architectural identities without gaining public aliases. Type
availability alone does not accept any operation/type tuple or numeric result.

**中文。** 十六个公开标识绑定到精确 PTO 目录标识，并给出 A2/A3 与 A5 的类型可用
基线。另九个目录类型保留架构标识但不获得公开 alias。类型可用性本身不接受任何
操作/类型组合或数值结果。
...
- Bound all public type names to catalog identities.
- Recorded target type-availability baselines.
- Kept catalog-only types and result semantics separate.
...
**English.** No operation legality, exceptional-value, rounding, flag, or
conversion behavior follows from these bindings. Separate accepted rules must
still establish every supported tuple, result, rejection, and conformance
claim.
