...
The scalar ALU catalog contains single-result, pair-result, materialization, selection, multiply/divide, and bit-manipulation forms. Totality requires every accepted encoding to define operands, result widths, exceptional corners, and writes when selectors alias.

标量 ALU 目录包含单结果、双结果、物化、选择、乘除和位操作形式。要实现完备性，每个已接受编码都必须定义操作数、结果宽度、异常边界以及 selector 别名时的写入行为。

### Detailed decision / 详细决策

The affected forms execute through one decoded ALU semantics path with explicit width extension, truncation, division corner, pair-result ordering, and destination-selector rules. Sources are resolved before writes; pair destinations publish in the documented order, including absolute, discard, T, and U selector equivalence classes. Illegal bounds reject before architectural effects.

受影响形式通过统一的解码 ALU 语义路径执行，并明确宽度扩展、截断、除法边界、双结果顺序和目的 selector 规则。写入前先解析源；双目的按照记录顺序发布，并覆盖 absolute、discard、T、U selector 等价类。非法边界在架构副作用前拒绝。

### What changed / 改动内容

#### English

- Closed result and corner behavior for the accepted scalar ALU catalog.
- Defined source snapshot and ordered destination publication under aliasing.

#### 中文

- 闭合已接受标量 ALU 目录的结果与边界行为。
- 定义别名情况下的源快照与目的有序发布。

### Scope and boundaries / 范围与边界

This ADR covers scalar ALU execution totality. Branch targets, memory faults, floating-point policy, and system/queue semantics remain with their separate decision families.

本 ADR 覆盖标量 ALU 执行完备性；分支目标、内存故障、浮点策略和系统/队列语义仍由各自决策族管理。
