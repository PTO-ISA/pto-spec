...
---
# ADR 0081: Tile scalar and immediate operations
...
Decision 082 in ADR-0081. `PadValueOrByteId` is the only applicable `B.DATR` field: omission is
...
Decision 082 in ADR-0081. No additional scalar conversion, saturation, or encoded rounding
control is defined.
...
The scalar binding and raw low-width DataType interpretation follow Decision 082 in ADR-0081.
...
and atomic publication as Decision 082 in ADR-0081. Omitting `B.IOR` supplies zero: therefore an
...
masks, persistence, aliases, and transaction rules follow Decision 082 in ADR-0081. Integer
...
preflight, and atomic publication follow Decision 082 in ADR-0081. `PadValueOrByteId` is the
...
complete preflight, and atomic publication follow Decision 082 in ADR-0081.
...
Dimensions/defaults, row-major matching geometry, source definedness,
Local-only bindings, equal and zero mask rules, persistence, aliasing,
`PadValueOrByteId`, complete preflight, and atomic publication follow Decision 082 in ADR-0081.
...
publication follow Decision 082 in ADR-0081 and Decision 067 in ADR-0080.
...
Local-only bindings, equal and zero mask rules, persistence, aliasing,
`PadValueOrByteId`, complete preflight, numeric-status transaction, and atomic
publication follow Decision 082 in ADR-0081 and Decision 068 in ADR-0080.
...
equality, follow Decision 069 in ADR-0080. The scalar is the raw low source-element-width
...
Source dimensions/defaults and row-major layout follow Decision 082 in ADR-0081. `CMode` and
...
definedness, and atomic destination publication follow Decision 079 in ADR-0080 and Decision 082 in ADR-0081.
