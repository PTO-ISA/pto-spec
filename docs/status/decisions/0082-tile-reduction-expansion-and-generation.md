...
---
# ADR 0082: Tile reduction, expansion, and generation
...
identical to Decision 067 in ADR-0080, including positive zero for a mixed-sign maximum tie.
...
Decision 097 in ADR-0082. `PadValueOrByteId` is the only applicable `B.DATR` field and follows
Decision 097 in ADR-0082 for every physical destination element outside the valid one-column
...
unsigned integers use their respective numeric order. Floating one-NaN,
two-NaN, signaling-NaN status, canonical-NaN, and signed-zero tie behavior is
identical to Decision 068 in ADR-0080, including negative zero for a mixed-sign minimum tie.
Invalid source encodings reject before effects.
...
field and follows Decision 097 in ADR-0082 over destination elements outside its valid rectangle.
...
Dimension defaults and bounds follow Decision 082 in ADR-0081. Both source valid regions MUST be
...
Decision 102 in ADR-0082. Integer overflow and floating rounding, exceptional values, signed
...
applicability, prohibited `B.IOR`/`B.IOS`, equal and zero mask rules, source
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 102 in ADR-0082. Integer overflow and floating rounding, exceptional values, and
...
applicability, prohibited `B.IOR`/`B.IOS`, equal and zero mask rules, source
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 102 in ADR-0082. Signed, unsigned, and floating division semantics follow Decision 060 in ADR-0080.
...
applicability, prohibited `B.IOR`/`B.IOS`, equal and zero mask rules, source
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 102 in ADR-0082. Signed and unsigned ordering and floating one-NaN, two-NaN,
signaling-NaN, canonical-NaN, and signed-zero tie behavior follow Decision 067 in ADR-0080;
...
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 102 in ADR-0082. Signed and unsigned ordering and floating one-NaN, two-NaN,
signaling-NaN, canonical-NaN, and signed-zero tie behavior follow Decision 068 in ADR-0080;
...
behavior, complete preflight, rollback, and atomic publication follow Decision 102 in ADR-0082.
...
to U32 destination padding follow Decision 097 in ADR-0082. `B.IOR` and `B.IOS` are illegal.
...
numeric-status transaction, rollback, and atomic publication follow Decision 111 in ADR-0082.
...
derive from capacity. Dimension defaults and bounds follow Decision 082 in ADR-0081. Both
...
applicability, prohibited `B.IOR`/`B.IOS`, equal and zero mask rules, source
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 116 in ADR-0082. Integer overflow and floating rounding, exceptional values, signed
zeros, and numeric status are exactly the typed subtraction rules of `TSUB`
for each element.
...
applicability, prohibited `B.IOR`/`B.IOS`, equal and zero mask rules, source
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 116 in ADR-0082. Signed, unsigned, and floating division and numeric status follow
Decision 060 in ADR-0080. A zero integer broadcast element causes an Illegal Block Exception
...
applicability, prohibited `B.IOR`/`B.IOS`, equal and zero mask rules, source
persistence, legal alias snapshot behavior, and atomic publication follow
Decision 116 in ADR-0082. Signed and unsigned ordering and floating NaN, signaling-NaN,
...
atomic publication follow Decision 116 in ADR-0082.
...
`PadValueOrByteId` application to U32 destination padding follow Decision 111 in ADR-0082.
