...
Representative AMO tests did not close every width, ordering modifier, address-class hint, reservation transition, alias combination, fault path, or overlapping DMA range. These corners affect both returned values and whether memory changes atomically.

代表性 AMO 测试没有闭合所有宽度、顺序修饰、地址类别提示、保留状态转换、别名组合、故障路径或重叠 DMA 范围。这些边界既影响返回值，也影响内存是否以原子方式改变。

### Detailed decision / 详细决策

The record defines width normalization, signed and unsigned min/max, wrapping arithmetic, CAS success and mismatch, ordering bits, the `far` hint, a 64-byte LR/SC reservation contract, and precise preflight/commit behavior. Sources are snapshotted before effects, rejected operations preserve destinations and memory as specified, and DMA overlap follows the recorded transactional rules.

本记录定义宽度规范化、有符号和无符号 min/max、回绕算术、CAS 成功与不匹配、顺序位、`far` 提示、64 字节 LR/SC 保留契约，以及精确的预检/提交行为。源在产生效果前快照；被拒操作按规定保留目的和内存；DMA 重叠遵循所记录的事务规则。

### What changed / 改动内容

#### English

- Completed value and ordering semantics for all accepted AMO forms.
- Made reservation match, invalidation, clearing, and failed-SC precedence explicit.
- Added alias-safe snapshots and rollback/restart requirements for atomic and DMA effects.

#### 中文

- 补全所有已接受 AMO 形式的值与顺序语义。
- 显式规定保留匹配、失效、清除和失败 SC 优先级。
- 为原子和 DMA 效果增加别名安全快照及回滚/重启要求。

### Scope and boundaries / 范围与边界

The 64-byte line rule and failed-SC no-probe rule are architecture choices. Physical cache topology and refinements to the address-class hint remain outside this record.

64 字节行规则和失败 SC 不探测规则属于架构选择。物理 cache 拓扑以及地址类别提示的进一步细化不在本记录范围内。
