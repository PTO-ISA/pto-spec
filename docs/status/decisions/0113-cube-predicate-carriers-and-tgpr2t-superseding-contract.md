...
- PredicateCell records the producing comparison basis DataType separately
  from its U8 storage DataType. Consumers require exact basis, valid shape,
  and layout agreement; same-shape cross-basis reuse is illegal.
...
- TGPR2T requires explicit dimensions. `LB1/LB0=32/4` selects CUBE_M32 and
  `16/8` selects CUBE_M16; LB2 is absent. These exact pairs are the layout
  selector and no implicit dimension or TSize inference is permitted.
...
- CUBE GPR masks accept only representable 32-bit and 16-bit basis types plus
  U8. U8 TSEL/TSELS consumes two complete mask GPRs; TSELS consumes its
  independent scalar-false source only after those two mask words.
- PredicateCell valid payload bytes are exactly `0x00` or `0x01`. PredicateCell
  ...
  Predicate padding uses Zero/Min=`0x00`, Max=`0x01`, and Null=undefined.
  GPR predicate padding uses the corresponding zero/one bit contract.
- A GPR has no per-bit definedness state, so Null padding bits outside the valid
  predicate region are selected by the registered
  `TileProfilePredicateNullGPRPadding` hook. The PTO v0 reference profile
  selects zero; an override may change only those invalid-region bits and MUST
  preserve valid predicate bits, Zero/Min and Max padding, destination
  selection, and numeric status.
