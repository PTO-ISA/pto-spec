...
`PadValueOrByteId` is the only applicable `B.DATR` field and follows Decision 056 in ADR-TILE-0008.
...
The mnemonic audit grouped many elementwise and irregular Tile selectors under broad carrier descriptions. Each selector still needed a closed contract for engine, operand order, types, shape, padding, aliases, exceptional inputs, preflight, and result publication.

助记符审计曾把大量逐元素与非规则 Tile selector 归在宽泛载体描述下。每个 selector 仍需闭合执行引擎、操作数顺序、类型、形状、padding、别名、特殊输入、预检和结果发布契约。

### Detailed decision / 详细决策

The numbered decisions close binary arithmetic, division/remainder, bitwise and shift operations, typed min/max and comparison, unary floating/integer operations, selection, conversion, fused multiply-add, extraction/insertion, and image-to-column behavior. They specify exact binding roles, supported types and layouts, valid-region rules, mask-zero no-op behavior, snapshot aliases, and atomic descriptor/payload/status publication.

编号决策闭合二元算术、除法/余数、位运算与移位、typed min/max 与比较、一元浮点/整数操作、选择、转换、融合乘加、提取/插入和 image-to-column 行为；并明确绑定角色、受支持类型与布局、有效区域规则、掩码零无操作、别名快照以及描述符/payload/状态原子发布。

### What changed / 改动内容

#### English

- Converted the audited selector set into explicit per-operation contracts.
- Closed type, shape, padding, alias, preflight, exceptional-input, and atomic-publication gaps.

#### 中文

- 将审计后的 selector 集合转化为明确的逐操作契约。
- 闭合类型、形状、padding、别名、预检、特殊输入和原子发布缺口。
