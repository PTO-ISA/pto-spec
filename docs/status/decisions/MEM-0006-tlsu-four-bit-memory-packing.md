...
ADR-TILE-0001 defines packed storage for tile allocation capacity, but the tile
...
The model stores each Tile payload in a wide carrier, but four-bit architectural data must not consume one byte per logical element. Without a packing rule, footprints, odd tails, sibling-nibble preservation, events, and fault rollback would all be wrong.

模型用宽载体保存每个 Tile payload，但架构上的四位数据不能让每个逻辑元素占用一个字节。若没有打包规则，访问范围、奇数尾部、同字节另一半 nibble 的保留、事件和故障回滚都会错误。

### Detailed decision / 详细决策

Two logical four-bit elements share one byte: even indices use the low nibble and odd indices the high nibble. Loads zero-extend the selected nibble; stores preserve its sibling. Indexed, masked, CAS, prefetch, and contiguous operations use containing-byte footprints, emit byte-sized events, preflight the complete instruction footprint, and fully reissue after faults.

两个逻辑四位元素共享一个字节：偶数索引使用低 nibble，奇数索引使用高 nibble。加载对选中 nibble 零扩展，存储保留另一半 nibble。索引、掩码、CAS、prefetch 和连续操作均使用包含字节的访问范围、产生字节大小事件、在执行前预检完整指令范围，并在故障后完整重发。

### What changed / 改动内容

#### English

- Separated architectural four-bit layout from the model's payload-carrier width.
- Specified nibble selection, sibling preservation, odd tails, and byte-sized events.
- Applied instruction-wide preflight and rollback across all TLSU memory classes.

#### 中文

- 将架构四位布局与模型 payload 载体宽度分离。
- 规定 nibble 选择、另一半保留、奇数尾部和字节大小事件。
- 对所有 TLSU 内存类别应用整条指令预检与回滚。

### Scope and boundaries / 范围与边界

This record defines the listed four-bit TLSU layout and totality obligations. It does not turn carrier width into architecture or silently authorize an alternative packed layout.

本记录定义所列四位 TLSU 布局与全域语义义务。它不把载体宽度变成架构，也不暗中授权其他打包布局。
