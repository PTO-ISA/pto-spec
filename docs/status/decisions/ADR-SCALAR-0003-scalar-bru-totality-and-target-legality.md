...
Control-flow instructions affect TPC, return state, Bundle commit state, and fault ordering. A helper-level description was insufficient because target legality and alias behavior must be proven from raw decoded forms before any control state changes.

控制流指令会影响 TPC、返回状态、Bundle 提交状态与故障顺序。仅描述辅助函数不足，因为必须从原始解码形式证明目标合法性与别名行为，并确保验证发生在任何控制状态改变之前。

### Detailed decision / 详细决策

All accepted BRU forms are assigned to explicit effect classes covering jumps, indirect targets, return-address production, condition setters, comparisons, and page-relative target construction. Target alignment/range, modifier legality, source snapshots, destination aliases, predicate/Bundle preservation, and fault-before-effect ordering are closed at the decoded instruction boundary.

所有已接受 BRU 形式都被分配到明确效果类别，覆盖跳转、间接目标、返回地址生成、条件设置、比较与页相对目标构造。目标对齐/范围、修饰符合法性、源快照、目的别名、谓词/Bundle 保留及故障先于副作用的顺序均在解码指令边界闭合。

### What changed / 改动内容

#### English

- Closed all accepted BRU forms under six explicit effect classes.
- Required decoded witnesses for target, alias, preservation, and fault obligations.

#### 中文

- 以六类明确效果闭合所有已接受 BRU 形式。
- 要求对目标、别名、保留和故障义务提供解码见证。

### Scope and boundaries / 范围与边界

The record governs scalar BRU totality and target legality. Bundle start/stop continuation ownership remains with the Block ADRs, and memory-address legality remains with the memory decisions.

本记录管理标量 BRU 完备性与目标合法性。Bundle 启动/停止的 continuation 归属仍由 Block ADR 管理，内存地址合法性仍属于内存决策。
