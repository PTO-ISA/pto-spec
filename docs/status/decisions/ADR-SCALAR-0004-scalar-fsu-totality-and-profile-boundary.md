---
{
  ...
  "title_zh": "标量 FSU 完备性与数值 profile 边界",
  "status": "accepted",
  "authors": [
...
---
# ADR-SCALAR-0004: Scalar FSU totality and numeric-profile boundary
...
- Requirement: PTO-REQ-SCALAR-FP-001, PTO-REQ-SCALAR-OPERAND-001,
  PTO-REQ-SCALAR-EXECUTION-001, PTO-REQ-RESET-001
...
Scalar floating operations need a complete decoded execution contract while numerical variation must stay inside explicitly owned numeric hooks. Otherwise instruction legality, conversion direction, status updates, and exceptional values could be confused with target-dependent arithmetic policy.

标量浮点操作需要完整的解码执行契约，同时数值变化必须限制在明确归属的数值钩子内。否则指令合法性、转换方向、状态更新和特殊值会与目标相关算术策略混淆。

### Detailed decision / 详细决策

The accepted FSU forms receive explicit source/destination types, conversion directions, rounding-selector use, comparison/min/max behavior, numeric-status publication, alias handling, and preflight. The instruction units own encoded legality and state effects; the referenced numeric boundary owns only the finite and exceptional arithmetic choices identified in the record.

已接受 FSU 形式获得明确的源/目的类型、转换方向、舍入 selector 使用、比较/min/max 行为、数值状态发布、别名处理和预检规则。指令单元管理编码合法性与状态效果；所引用数值边界仅管理记录中标识的有限值及特殊值算术选择。

### What changed / 改动内容

#### English

- Closed decoded execution and state publication for the scalar FSU catalog.
- Separated instruction-owned legality from explicitly delegated numeric behavior.

#### 中文

- 闭合标量 FSU 目录的解码执行与状态发布。
- 分离指令归属的合法性和显式委托的数值行为。

### Scope and boundaries / 范围与边界

This ADR does not invent numeric outcomes beyond the existing hooks, nor does it allocate new encodings. Corrections within an already assigned raw form remain ASL fixes under the affected owners.

本 ADR 不在既有钩子之外创造数值结果，也不分配新编码。已分配原始形式内部的修正仍作为受影响 owner 下的 ASL 修复处理。
