...
---
# ADR-MEM-0004: Scalar AGU totality, aliases, and restart
...
Reaching every decoded AGU form did not prove exact offset formation, reserved modifier handling, pair atomicity, writeback order, alias behavior, fault precedence, or restart. These details determine whether a fault leaks partial memory or register effects.

覆盖每个已解码 AGU 形式并不能证明精确的偏移形成、保留修饰处理、成对原子性、回写顺序、别名行为、故障优先级或重启语义。这些细节决定故障时是否泄漏部分内存或寄存器效果。

### Detailed decision / 详细决策

AGU arithmetic is modulo 2^64 with form-defined extension and scaling. Reserved `SrcRType` rejects before reads. Update and pair forms preflight required accesses before effects, snapshot aliased sources, publish destinations and writeback only after success, report the first failing original address, and use full reissue after faults. Alignment, translation, permission, and bounded-memory checks have defined precedence.

AGU 算术按 2^64 取模，并遵循各形式规定的扩展与缩放。保留的 `SrcRType` 在读取前拒绝。更新和成对形式在产生效果前预检全部访问，快照可能别名的源，只在成功后发布目的和回写，报告第一个失败的原始地址，并在故障后完整重发。对齐、转换、权限和有界内存检查具有确定优先级。

### What changed / 改动内容

#### English

- Closed all encoded address modifiers and reserved values.
- Made pair accesses and update forms fail-before-effect transactions.
- Defined source snapshots, destination/writeback order, fault address, and restart behavior for aliases.

#### 中文

- 闭合所有编码地址修饰和保留值。
- 将成对访问与更新形式定义为效果前失败的事务。
- 为别名情况规定源快照、目的/回写顺序、故障地址和重启行为。

### Scope and boundaries / 范围与边界

The record covers scalar AGU forms and their listed ASL/NDF owners. It does not infer a physical MMU or cache, and unreachable helper branches are not ISA behavior.

本记录覆盖标量 AGU 形式及其所列 ASL/NDF owner。它不推断物理 MMU 或 cache；不可达 helper 分支也不是 ISA 行为。
