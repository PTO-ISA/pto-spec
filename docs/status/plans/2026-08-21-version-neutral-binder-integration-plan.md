...
Create a test that reads `docs/status/decisions/ADR-BLOCK-0015-b-iot-b-ios-sizecode-pemode.md` and rejects release-owned language:
...
- Modify: `docs/status/decisions/ADR-BLOCK-0015-b-iot-b-ios-sizecode-pemode.md`
- Test: `tests/scripts/test_architecture_release_boundary.py`
- Test: `tests/scripts/test_release_closure.py`
...
! rg -n '\b0\.[0-9]+(?:\.[0-9]+)?\b|post-0\.|mode-function-v[0-9]+|must not merge until' \
  docs/status/decisions/ADR-BLOCK-0015-b-iot-b-ios-sizecode-pemode.md
```
