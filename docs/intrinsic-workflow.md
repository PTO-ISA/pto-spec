...
6. 109 条正式指令位于 [instructions/tile/](instructions/tile.md)，Block/Header 文档位于 [instructions/bundle/](instructions/bundle/overview/block-header-model.md)。

## 架构边界

- 本仓的 109 条 Tile 指令是当前 PTO ISA 的唯一正式目录，Markdown、catalog、ASL、Excel 与 HTML 必须保持同一集合。执行引擎严格分为 VEC、SFU、TLSU 与 CUBE；TEPL 只保留为二进制 carrier 名称。
- `GMOV`、`TFMA`、`THISTOGRAM` 与 `TSORT` 属于 canonical catalog；`TRANDOM` 已删除且旧 selector 保留，`TSORT32` 是旧拼写，`SYNCALL` 不属于 Tile 集合。
