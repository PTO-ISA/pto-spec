...
`MCOPY` 是独立的可重启内存命令；操作数快照与进度状态共同规定精确完成和恢复。

<!-- PTO-READER-BLOCK: block-mcopy-mechanism role=mechanism -->
## 放置与执行机制

`MCOPY` 作为独立的 `32` 位命令执行，不要求放在 `BSTART`/`BSTOP` Block 体内。
...
- 编码操作数：`RegSrc0` — 保存目的字节地址的绝对 GPR; `RegSrc1` — 保存源字节地址的绝对 GPR; `RegSrc2` — 保存完整无符号 XLEN 字节数的绝对 GPR。
