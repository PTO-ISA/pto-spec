...
E1M2X2 是 PTO 已分配的数值格式。本页帮助读者把载体布局、有限值分解和值分类联系起来；确切契约仍由 ASL 所有者中的 `E1M2X2NumericFormatDescriptor`、`E1M2X2FiniteDecomposition` 和 `ClassifyE1M2X2` 定义。
...
`E1M2X2FiniteDecomposition` 先报告有限值分解是否可用；可用时返回符号、整数有效数和二进制指数。`ClassifyE1M2X2` 则独立地把原始 lane 归入该格式所支持的零、次正规数、正规数、无穷大或 NaN 类别。
