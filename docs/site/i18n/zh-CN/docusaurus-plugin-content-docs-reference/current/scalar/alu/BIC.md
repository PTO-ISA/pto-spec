...
`BIC` 是一条 32 位标量 ALU 指令。它清除独立选择且可回绕目标位域中的每一位；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-bic-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后清除独立选择且可回绕目标位域中的每一位，最后才产生目标效果。
...
以一个小型 `BIC` 示例说明：清除基值 `0xf` 的所选位 `1..2` 后得到 `0x9`。
<!-- SUPPLEMENTARY-END -->
