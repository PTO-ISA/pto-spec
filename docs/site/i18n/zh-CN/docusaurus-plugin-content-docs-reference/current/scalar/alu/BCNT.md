...
`BCNT` 是一条 32 位标量 ALU 指令。它统计独立选择且可回绕位域中的置位位数；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-bcnt-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后统计独立选择且可回绕位域中的置位位数，最后才产生目标效果。
...
以一个小型 `BCNT` 示例说明：四位所选位域 `1011` 含有三个置位位，因此结果为 `3`。
<!-- SUPPLEMENTARY-END -->
