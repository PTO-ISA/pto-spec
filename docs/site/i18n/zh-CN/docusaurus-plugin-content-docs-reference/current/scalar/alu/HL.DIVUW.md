...
`HL.DIVUW` 是一条 48 位标量 ALU 指令。它根据源快照同时计算无符号商和余数；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-hl-divuw-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后根据源快照同时计算无符号商和余数，最后才产生目标效果。
...
以一个小型 `HL.DIVUW` 示例说明：被除数 `13` 与除数 `5` 产生商 `2`，并按目标顺序产生余数 `3`。
<!-- SUPPLEMENTARY-END -->
