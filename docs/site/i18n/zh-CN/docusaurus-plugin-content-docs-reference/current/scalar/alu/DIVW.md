...
`DIVW` 是一条 32 位标量 ALU 指令。它对低 32 位字，再符号扩展到 XLEN计算有符号商；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-divw-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后对低 32 位字，再符号扩展到 XLEN计算有符号商，最后才产生目标效果。
...
以一个小型 `DIVW` 示例说明：被除数 `13` 与除数 `5` 产生商 `2`。
<!-- SUPPLEMENTARY-END -->
