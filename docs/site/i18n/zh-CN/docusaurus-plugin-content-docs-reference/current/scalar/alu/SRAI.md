...
`SRAI` 是一条 32 位标量 ALU 指令。它按照完整 XLEN 值移位规则对源值执行算术右移；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-srai-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后按照完整 XLEN 值移位规则对源值执行算术右移，最后才产生目标效果。
...
以一个小型 `SRAI` 示例说明：源值 `-8` 算术右移 `2` 位后得到 `-2`。
<!-- SUPPLEMENTARY-END -->
