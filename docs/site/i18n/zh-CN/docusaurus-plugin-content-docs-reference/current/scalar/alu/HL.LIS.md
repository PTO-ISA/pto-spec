...
`HL.LIS` 是一条 48 位标量 ALU 指令。它组合分段编码的 32 位立即数并将其符号扩展到 XLEN；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-hl-lis-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后组合分段编码的 32 位立即数并将其符号扩展到 XLEN，最后才产生目标效果。
...
- `simm32` 是 32 位有符号字段，携带有符号分段 32 位立即数。
...
以一个小型 `HL.LIS` 示例说明：分段立即数值 `1` 物化为 XLEN 值 `1`。
<!-- SUPPLEMENTARY-END -->
