...
`BXU` 是一条 32 位标量 ALU 指令。它提取独立选择且可回绕的位域，并将其零扩展到 XLEN；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-bxu-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后提取独立选择且可回绕的位域，并将其零扩展到 XLEN，最后才产生目标效果。
...
以一个小型 `BXU` 示例说明：提取四位位域 `1110` 后零扩展为 XLEN 值 `14`。
<!-- SUPPLEMENTARY-END -->
