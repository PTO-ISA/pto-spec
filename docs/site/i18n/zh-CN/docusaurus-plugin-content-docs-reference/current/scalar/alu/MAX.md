...
`MAX` 是一条 32 位标量 ALU 指令。它把完整操作数按有符号值比较，并选择最大值的位模式；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-max-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后把完整操作数按有符号值比较，并选择最大值的位模式，最后才产生目标效果。
...
以一个小型 `MAX` 示例说明：操作数 `7` 与 `3` 选择结果 `7`。
<!-- SUPPLEMENTARY-END -->
