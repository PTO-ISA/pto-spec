...
`C.ZEXT.W` 是一条 16 位标量 ALU 指令。它把源值低 32 位零扩展到 XLEN；当前指令契约定义结果发布路径以及任何额外状态效果。

<!-- PTO-READER-BLOCK: scalar-c-zext-w-mechanism role=mechanism -->
## 结果形成方式

执行时先对编码输入做快照，然后把源值低 32 位零扩展到 XLEN，最后才产生目标效果。
...
以一个小型 `C.ZEXT.W` 示例说明：源值低 32 位 `0x80000000` 零扩展后得到正 XLEN 值 `2147483648`。
<!-- SUPPLEMENTARY-END -->
