...
`HL.CASD` 把 `SrcL` 指向的双字与 `SrcR` 进行原子比较；相等时写入 `SrcD`，而两条路径都会发布先前的 64 位值。
...
`SrcL` 承载 Reg5 原子地址源；`SrcR` 承载 Reg5 期望双字源；`SrcD` 承载 Reg5 目标双字源；`RegDst` 承载 Reg5 旧值目的地；`aq` 承载获取排序位；`rl` 承载释放排序位；`far` 承载平坦地址路由提示。
...
初次阅读可从 `hl.casd [SrcL], SrcR, SrcD, ->Rd` 开始，再只改变上文说明的排序或路由修饰位。
<!-- SUPPLEMENTARY-END -->
