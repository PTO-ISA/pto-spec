...
`SWAPB` 用 `SrcR` 原子替换一个字节，并发布先前的 8 位值。

<!-- PTO-READER-BLOCK: scalar-swapb-mechanism role=mechanism -->
## 原子机制

ASL DOC 契约选择 `ScalarHandler_AtomicReadModifyWrite`，访问宽度为 `1` 字节。
...
`SrcL` 承载 Reg5 原子地址源；`SrcR` 承载 Reg5 字节替换值源；`RegDst` 承载 Reg5 旧值目的地；`aq` 承载获取排序位；`rl` 承载释放排序位；`far` 承载平坦地址路由提示。
...
初次阅读可从 `swapb [SrcL], SrcR, ->Rd` 开始，再只改变上文说明的排序或路由修饰位。
<!-- SUPPLEMENTARY-END -->
