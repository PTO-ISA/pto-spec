...
`SD.SMIN` 对一个双字原子执行有符号最小值并存储结果，但不发布旧值。
...
初次阅读可从 `sd.smin [SrcL], SrcR` 开始，再只改变上文说明的排序或路由修饰位。
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| sd_smin_32_188d93125032 | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| sd_smin_32_188d93125032 | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| sd_smin_32_188d93125032 | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| sd_smin_32_188d93125032 | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero selects relaxed ordering. |
...
- SD.SMIN compares both values as two's-complement signed integers and selects the smaller value at 64-bit width and stores that value; it does not publish the old value.
...
- Atomically read one aligned 8-byte little-endian value, compute the signed minimum, and write one 8-byte result to the same location.
