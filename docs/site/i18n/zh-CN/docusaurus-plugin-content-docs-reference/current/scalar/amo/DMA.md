...
`DMA` 把 `SrcL` 中的地址指向的恰好 `64` 字节复制到 `SrcR` 中的地址，并把整个复制视为一条可重新执行的标量操作。

<!-- PTO-READER-BLOCK: scalar-dma-mechanism role=mechanism -->
## 原子机制

ASL DOC 契约选择 `ExecuteScalarDMACopy64`：它先探测完整源范围，再探测完整目的范围。

第一次写入目的地之前会先捕获全部源字节，因此完全重叠、向前重叠和向后重叠都遵循类似 memmove 的快照行为。

<!-- PTO-READER-BLOCK: scalar-dma-inputs-outputs role=inputs-outputs -->
## 输入与结果

`SrcL` 承载 Reg5 源字节地址源；`SrcR` 承载 Reg5 目的字节地址源。
...
复制成功时，先发出八个宽松的 8 字节载入事件，再发出八个宽松的 8 字节存储事件，随后让 `TPC` 前进 `4` 字节。

只有成功写入的目的范围与本地保留重叠时才会使保留失效；不相关的保留状态保持不变。

<!-- PTO-READER-BLOCK: scalar-dma-constraints role=constraints -->
## 合法性与精确故障

读取或写入任何字节之前，两个完整的 64 字节范围都必须通过预检；源或目的范围中第一个失败的探测决定故障。

发生故障时不会暴露事件前缀或部分目的更新，内存保持不变，并保留原始 `TPC` 以便完整重新执行。
...
初次阅读可从 `dma [SrcL], SrcR` 开始，依次跟踪源预检、目的预检、64 字节快照和最终复制提交；该指令没有排序或路由修饰位。
<!-- SUPPLEMENTARY-END -->
