...
`SD.OR` 对一个双字原子执行按位或并存储结果，但不发布旧值。
...
初次阅读可从 `sd.or [SrcL], SrcR` 开始，再只改变上文说明的排序或路由修饰位。
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| sd_or_32_2a65eedfae0f | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| sd_or_32_2a65eedfae0f | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| sd_or_32_2a65eedfae0f | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| sd_or_32_2a65eedfae0f | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero selects relaxed ordering. |
...
- SD.OR computes the bitwise OR of the old value and operand at 64-bit width and stores that value; it does not publish the old value.
...
- Atomically read one aligned 8-byte little-endian value, compute the bitwise OR, and write one 8-byte result to the same location.
