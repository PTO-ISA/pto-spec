...
`LW.UMIN` 对一个字原子执行无符号最小值、存储结果，并发布先前的内存值。
...
初次阅读可从 `lw.umin [SrcL], SrcR, ->Rd` 开始，再只改变上文说明的排序或路由修饰位。
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| lw_umin_32_fd9b7bb538a3 | RegDst | 5 | 0–31 | none | none | Reg5 old-value destination | Encoded zero discards the published old value. |
| lw_umin_32_fd9b7bb538a3 | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| lw_umin_32_fd9b7bb538a3 | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| lw_umin_32_fd9b7bb538a3 | aq | 1 | 0–1 | none | none | acquire ordering bit | Encoded zero disables acquire ordering. |
| lw_umin_32_fd9b7bb538a3 | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| lw_umin_32_fd9b7bb538a3 | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero disables release ordering. |
...
- LW.UMIN computes the unsigned minimum at 32-bit width and publishes the prior memory value only after a successful atomic commit.
...
- Atomically read one aligned 4-byte little-endian value, compute the width-sized unsigned minimum, and write one 4-byte result to the same location.
