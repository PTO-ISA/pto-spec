...
`HL.SETC.GEI` 判断有符号大于等于，并把结果发布为当前条件指令束的提交判定。
...
指令对源取快照，判断有符号大于等于，再规范化为 XLEN 一或零。
...
成功时，`HL.SETC.GEI` 让 `TPC` 前进 `6` 字节；它没有标量目的位置，也不产生内存或保留状态效果。
...
`hl.setc.gei SrcL, simm` 按上述规则计算条件，把规范化判定写入提交状态，并且只在更新完成后推进 `TPC`。
<!-- SUPPLEMENTARY-END -->
