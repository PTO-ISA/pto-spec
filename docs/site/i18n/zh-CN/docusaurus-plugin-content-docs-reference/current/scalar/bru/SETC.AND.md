...
`SETC.AND` 计算按位与条件，并把它发布为当前条件指令束的提交判定。
...
成功时，`SETC.AND` 让 `TPC` 前进 `4` 字节；它没有标量目的位置，也不产生内存或保留状态效果。
...
`setc.and SrcL, SrcR<.sw, .uw, .not>` 按上述规则计算条件，把规范化判定写入提交状态，并且只在更新完成后推进 `TPC`。
<!-- SUPPLEMENTARY-END -->
