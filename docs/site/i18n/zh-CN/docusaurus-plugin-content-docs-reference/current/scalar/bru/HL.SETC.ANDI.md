...
`HL.SETC.ANDI` 计算按位与条件，并把它发布为当前条件指令束的提交判定。

<!-- PTO-READER-BLOCK: scalar-hl-setc-andi-mechanism role=mechanism -->
## 执行机制

放置和单次设置检查通过后，指令对操作数取快照并执行按位与。
...
成功时，`HL.SETC.ANDI` 让 `TPC` 前进 `6` 字节；它没有标量目的位置，也不产生内存或保留状态效果。
...
`hl.setc.andi SrcL, simm` 按上述规则计算条件，把规范化判定写入提交状态，并且只在更新完成后推进 `TPC`。
<!-- SUPPLEMENTARY-END -->
