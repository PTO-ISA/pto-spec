...
在 `pto-v0` 参考配置档中，零除数返回全一载体位并产生 `DZ`；否则执行无符号载体除法。该确定性参考规则不是 IEEE-754 或目标硬件声明。
...
`fdiv.fd a0, a1, ->a2` 选择载体，对源取快照，调用当前配置档，累积返回标志，发布结果，最后推进 `TPC`。
<!-- SUPPLEMENTARY-END -->
