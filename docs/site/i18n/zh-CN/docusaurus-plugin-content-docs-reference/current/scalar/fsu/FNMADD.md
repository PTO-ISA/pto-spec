...
`FNMADD` 通过当前数值配置档对融合的左乘右再加加数结果取负。
...
在 `pto-v0` 参考配置档中，载体乘积与加数相加后按选定宽度取模取负。该确定性参考规则不是 IEEE-754 或目标硬件声明。
...
`fnmadd.fd a0, a1, a2, ->a3` 选择载体，对源取快照，调用当前配置档，累积返回标志，发布结果，最后推进 `TPC`。
<!-- SUPPLEMENTARY-END -->
