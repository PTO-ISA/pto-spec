...
`FCVTA` 通过当前数值配置档使用固定最近值舍入（平局时远离零），把 FP64 或 FP32 输入转换到原始 DstType 编码 `0..7`（UD/UW/UH/UB 或 SD/SW/SH/SB）。
...
`fcvta.fd2sd a0, ->a1` 选择载体，对源取快照，调用当前配置档，累积返回标志，发布结果，最后推进 `TPC`。
<!-- SUPPLEMENTARY-END -->
