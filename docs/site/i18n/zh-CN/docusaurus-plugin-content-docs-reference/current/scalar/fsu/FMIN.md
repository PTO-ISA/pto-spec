...
`FMIN` 对选定载体应用架构定义的有序最小值、NaN 和有符号零规则。
...
只有一个 NaN 时选择数值操作数；两个 NaN 时选择对应宽度的规范静默 NaN。信号 NaN 会记录粘滞 `NV`。

有符号零选择遵循架构定义的 `FMIN` 规则。
...
`fmin.fd a0, a1, ->a2` 应用架构定义的特殊值规则，在推进 `TPC` 前发布规范化输出。
<!-- SUPPLEMENTARY-END -->
