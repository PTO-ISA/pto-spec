...
`FCVT` 通过当前数值配置档使用当前舍入模式，把 FP64、FP32、FP16 或 E4M3 输入转换到raw `DstType=0..3` 所选的 FP64、FP32、FP16 或 E4M3 目的类型。
...
在 `pto-v0` 参考配置档中，有限值和特殊值 FP64、FP32、FP16 及 E4M3 源按当前舍入模式通过公共标量/TCVT 配置档转换。
