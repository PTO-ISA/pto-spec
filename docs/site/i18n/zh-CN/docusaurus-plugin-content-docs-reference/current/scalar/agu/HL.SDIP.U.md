...
`HL.SDIP.U` 是一条独立的 `48` 位 AGU 指令；它形成有符号立即数地址，并存储两个相邻且对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sdip-u-mechanism role=mechanism -->
## 地址与内存机制

`HL.SDIP.U` 将完整 `-65536..65535` 取值域中的 `simm17` 符号扩展，随后不再缩放，并将位移按 `2^PTO_XLEN` 取模后加到快照中的 `SrcR` 基址。
...
若基址为 `0x100`、有符号立即数为 `8`，位移就是 `8`，基址加位移为 `0x108`。内存访问使用 `0x108`。两个地址全部通过预检后，第二个 `8` 字节存储使用 `0x110`。
<!-- SUPPLEMENTARY-END -->
