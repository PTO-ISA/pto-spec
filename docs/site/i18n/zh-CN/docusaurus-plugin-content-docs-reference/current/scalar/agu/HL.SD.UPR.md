...
`HL.SD.UPR` 是一条独立的 `48` 位 AGU 指令；它形成寄存器偏移地址，并存储一个对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sd-upr-mechanism role=mechanism -->
## 地址与内存机制

`HL.SD.UPR` 按 `SrcRType` 转换 `SrcR`，不再缩放，并将结果按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。
...
若基址为 `0x100`、偏移源为不经转换的 `8`，并采用固定左移 `0` 位，对齐位移就是 `8`，基址加位移为 `0x108`。内存访问使用 `0x108`。若权限允许，指令在该对齐地址存储 `8` 字节。
<!-- SUPPLEMENTARY-END -->
