...
`HL.SB.PCR` 是一条独立的 `48` 位标量 AGU 指令，使用 `PCRelative` 寻址，按小端序存储一个 `1` 字节值。
...
所有结果或回写发布完成后，`HL.SB.PCR` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.sb.pcr SrcL, [<symbol>]` 开始，找出已编码的地址字段。
