...
`SD` 是一条独立的 `32` 位 AGU 指令；它形成寄存器偏移地址，并存储一个对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-sd-mechanism role=mechanism -->
## 地址与内存机制

`SD` 先按 `SrcRType` 转换 `SrcR`，再将结果乘以 `8`，并按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。
...
| sd_32_9dbc40328653 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
