...
`HL.LWUI.U` 是一条独立的 `48` 位标量 AGU 指令，使用 `Immediate` 寻址，按小端序加载一个 `4` 字节值；当结果窄于 `PTO_XLEN` 时，对传输位进行零扩展。

<!-- PTO-READER-BLOCK: scalar-hl-lwui-u-mechanism role=mechanism -->
## 地址与传输机制

地址路径先对 `simm22` 做符号扩展并乘以 `1`，再把位移与快照中的 `SrcL` 相加，结果按 `2^PTO_XLEN` 取模。
...
所有结果或回写发布完成后，`HL.LWUI.U` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.lwui.u [SrcL, simm], ->{t, u, Rd}` 开始，找出已编码的地址字段。
