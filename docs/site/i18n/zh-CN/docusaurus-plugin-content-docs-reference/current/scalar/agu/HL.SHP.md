...
`HL.SHP` 是一条独立的 `48` 位 AGU 指令；它形成寄存器偏移地址，并存储两个相邻且对齐的小端 `2` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-shp-mechanism role=mechanism -->
## 地址与内存机制

`HL.SHP` 先按 `SrcRType` 转换 `SrcR`，再将结果乘以 `2`，并按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。
...
若基址为 `0x100`、偏移源为不经转换的 `2`，并采用固定左移 `1` 位，偏移量就是 `4`，基址加偏移量为 `0x104`。内存访问使用 `0x104`。两个地址全部通过预检后，第二个 `2` 字节存储使用 `0x106`。
<!-- SUPPLEMENTARY-END -->
...
| hl_shp_48_ccc507e71a27 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
