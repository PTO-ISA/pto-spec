...
`HL.SDP.U` 是一条独立的 `48` 位 AGU 指令；它形成寄存器偏移地址，并存储两个相邻且对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sdp-u-mechanism role=mechanism -->
## 地址与内存机制

`HL.SDP.U` 按 `SrcRType` 转换 `SrcR`，不再缩放，并将结果按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。
...
若基址为 `0x100`、偏移源为不经转换的 `8`，并采用固定左移 `0` 位，对齐位移就是 `8`，基址加位移为 `0x108`。内存访问使用 `0x108`。两个地址全部通过预检后，第二个 `8` 字节存储使用 `0x110`。
<!-- SUPPLEMENTARY-END -->
...
| hl_sdp_u_48_66de58724f2f | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
