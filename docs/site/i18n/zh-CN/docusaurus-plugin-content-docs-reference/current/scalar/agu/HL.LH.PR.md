...
`HL.LH.PR` 是一条独立的 `48` 位标量 AGU 指令，使用 `Register` 寻址，按小端序加载一个 `2` 字节值；当结果窄于 `PTO_XLEN` 时，对传输位进行符号扩展。
...
所有结果或回写发布完成后，`HL.LH.PR` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.lh.pr [SrcL, SrcR<{.sw,.uw,.neg}><<<shamt>], ->Dst0, Dst1` 开始，找出已编码的地址字段。
...
| --- | --- | ---: | --- | --- |
| hl_lh_pr_48_d59f64cde1dc | HL48 | 48 | 0x00001009002e / 0x0000707f07ff | [{"field":"SrcRType","operator":"one-of","values":[0,1,2]}] |
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| hl_lh_pr_48_d59f64cde1dc | RegDst0 | 5 | 0–31 | none | none | Reg5 first loaded-value destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_lh_pr_48_d59f64cde1dc | RegDst1 | 5 | 0–31 | none | none | Reg5 updated-base destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_lh_pr_48_d59f64cde1dc | SrcL | 5 | 0–31 | none | none | Reg5 address-base source | Encoded zero reads the architectural zero GPR. |
| hl_lh_pr_48_d59f64cde1dc | SrcR | 5 | 0–31 | none | none | Reg5 register-offset source | Encoded zero reads the architectural zero GPR. |
| hl_lh_pr_48_d59f64cde1dc | SrcRType | 2 | 0–2 | none | 3 | register-offset transformation selector | Encoded zero leaves the complete PTO_XLEN register-offset value unchanged. |
| hl_lh_pr_48_d59f64cde1dc | shamt | 5 | 0–31 | none | none | post-transformation logical-left-shift amount | Encoded zero performs no shift. |

- `hl_lh_pr_48_d59f64cde1dc.SrcRType` reserved values: Reserved encodings raise Fault_IllegalInstruction before architectural effects.
...
```asl
readonly func InstructionContractHandler_HL_LH_PR()
    => ScalarSemanticHandler
begin
    return ScalarHandler_ExecuteScalarLoad;
end;

pure func InstructionContractAGUAction_HL_LH_PR()
    => ScalarAGUAction
begin
    return ScalarAGU_Load;
end;

pure func InstructionContractAGUAddressKind_HL_LH_PR()
    => ScalarAGUAddressKind
begin
    return ScalarAGU_Register;
end;

pure func InstructionContractAGUSizeBytes_HL_LH_PR()
    => integer {1,2,4,8}
begin
    return 2;
end;

pure func InstructionContractAGUOffsetScale_HL_LH_PR()
    => integer {0..3}
begin
    return 0;
end;

pure func InstructionContractAGUUpdateMode_HL_LH_PR()
    => AddressUpdateMode
begin
    return AddressUpdate_PreIndex;
end;

pure func InstructionContractAGUSignedLoad_HL_LH_PR()
    => boolean
begin
    return TRUE;
end;

pure func InstructionContractAGUPrefetchReturnsAddress_HL_LH_PR()
    => boolean
begin
    return FALSE;
end;
