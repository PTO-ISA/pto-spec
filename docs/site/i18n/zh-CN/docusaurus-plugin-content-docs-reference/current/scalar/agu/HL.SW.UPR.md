...
`HL.SW.UPR` 是一条独立的 `48` 位 AGU 指令；它形成寄存器偏移地址，并存储一个对齐的小端 `4` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sw-upr-mechanism role=mechanism -->
## 地址与内存机制

`HL.SW.UPR` 按 `SrcRType` 转换 `SrcR`，不再缩放，并将结果按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。
...
若基址为 `0x100`、偏移源为不经转换的 `4`，并采用固定左移 `0` 位，对齐位移就是 `4`，基址加位移为 `0x104`。内存访问使用 `0x104`。若权限允许，指令在该对齐地址存储 `4` 字节。
<!-- SUPPLEMENTARY-END -->
