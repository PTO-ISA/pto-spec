...
`HL.LD.PO` 是一条独立的 `48` 位标量 AGU 指令，使用 `Register` 寻址，按小端序加载一个 `8` 字节值；当结果窄于 `PTO_XLEN` 时，对传输位进行零扩展。
...
所有结果或回写发布完成后，`HL.LD.PO` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.ld.po [SrcL, SrcR<{.sw,.uw,.neg}><<<shamt>], ->Dst0, Dst1` 开始，找出已编码的地址字段。
...
| --- | --- | ---: | --- | --- |
| hl_ld_po_48_870e30995d10 | HL48 | 48 | 0x00003009003e / 0x0000707f07ff | [] |
...
| hl_ld_po_48_870e30995d10 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
