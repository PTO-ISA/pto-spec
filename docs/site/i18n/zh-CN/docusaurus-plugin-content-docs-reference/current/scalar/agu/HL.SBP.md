...
`HL.SBP` 是一条独立的 `48` 位标量 AGU 指令，使用 `Register` 寻址，按地址顺序存储两个相邻的 `1` 字节值。
...
所有结果或回写发布完成后，`HL.SBP` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.sbp SrcD, SrcD1, [SrcL, SrcR<{.sw,.uw,.neg}>]` 开始，找出已编码的地址字段。
