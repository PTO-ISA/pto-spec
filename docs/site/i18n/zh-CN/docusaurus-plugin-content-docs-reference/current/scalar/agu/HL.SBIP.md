...
`HL.SBIP` 是一条独立的 `48` 位标量 AGU 指令，使用 `Immediate` 寻址，按地址顺序存储两个相邻的 `1` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sbip-mechanism role=mechanism -->
## 地址与传输机制

地址路径先对 `simm17` 做符号扩展并乘以 `1`，再把位移与快照中的 `SrcR` 相加，结果按 `2^PTO_XLEN` 取模。

两个相邻地址会在任一次对齐的小端序 `1` 字节存储发生前全部完成预检。两次存储按地址递增顺序提交。
...
执行成功时记录两个按地址顺序排列的 relaxed 存储事件；只有完整预检通过后，重叠的保留状态才会失效。

所有结果或回写发布完成后，`HL.SBIP` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.sbip SrcD, SrcD1, [SrcR, simm]` 开始，找出已编码的地址字段。
