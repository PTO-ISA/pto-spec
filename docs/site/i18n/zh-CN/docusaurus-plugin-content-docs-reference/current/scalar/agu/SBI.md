...
`SBI` 是一条独立的 `32` 位 AGU 指令；它形成有符号立即数地址，并存储一个对齐的小端 `1` 字节值。

<!-- PTO-READER-BLOCK: scalar-sbi-mechanism role=mechanism -->
## 地址与内存机制

`SBI` 将完整 `-2048..2047` 取值域中的 `simm12` 符号扩展，随后不再缩放，并将位移按 `2^PTO_XLEN` 取模后加到快照中的 `SrcR` 基址。
...
若基址为 `0x100`、有符号立即数为 `2`，位移就是 `2`，基址加位移为 `0x102`。内存访问使用 `0x102`。若地址对齐且权限允许，指令在该地址存储 `1` 字节。
<!-- SUPPLEMENTARY-END -->
