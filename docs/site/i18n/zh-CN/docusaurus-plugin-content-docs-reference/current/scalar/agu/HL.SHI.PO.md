...
`HL.SHI.PO` 是一条独立的 `48` 位 AGU 指令；它形成有符号立即数地址，并存储一个对齐的小端 `2` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-shi-po-mechanism role=mechanism -->
## 地址与内存机制

`HL.SHI.PO` 将完整 `-65536..65535` 取值域中的 `simm17` 符号扩展，随后乘以 `2`，并将位移按 `2^PTO_XLEN` 取模后加到快照中的 `SrcR` 基址。
...
若基址为 `0x100`、有符号立即数为 `2`，位移就是 `4`，基址加位移为 `0x104`。内存访问使用 `0x100`，计算出的和值只在成功后发布。若地址对齐且权限允许，指令在该地址存储 `2` 字节。
<!-- SUPPLEMENTARY-END -->
