...
`SD.PCR` 是一条独立的 `32` 位 AGU 指令；它形成PC 相对地址，并存储一个对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-sd-pcr-mechanism role=mechanism -->
## 地址与内存机制

`SD.PCR` 先清零 `TPC[1:0]`，再将 `-65536..65535` 取值域中的 `simm` 符号扩展并乘以 `4`，最后把位移按 `2^PTO_XLEN` 取模后加到对齐后的 `TPC` 基址。
...
若对齐后的 `TPC=0x100` 且编码位移为 `2`，字节位移就是 `8`，有效地址为 `0x108`。内存访问使用 `0x108`。若地址对齐且权限允许，指令在该地址存储 `8` 字节。
<!-- SUPPLEMENTARY-END -->
