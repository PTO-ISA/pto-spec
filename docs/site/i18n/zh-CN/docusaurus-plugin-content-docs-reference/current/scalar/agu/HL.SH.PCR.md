...
`HL.SH.PCR` 是一条独立的 `48` 位 AGU 指令；它形成PC 相对地址，并存储一个对齐的小端 `2` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sh-pcr-mechanism role=mechanism -->
## 地址与内存机制

`HL.SH.PCR` 先清零 `TPC[1:0]`，再将 `-268435456..268435455` 取值域中的 `simm` 符号扩展并乘以 `4`，最后把位移按 `2^PTO_XLEN` 取模后加到对齐后的 `TPC` 基址。
