...
`LBU` 是一条独立的 `32` 位 AGU 指令；它形成寄存器偏移地址，并加载一个对齐的小端 `1` 字节值。

<!-- PTO-READER-BLOCK: scalar-lbu-mechanism role=mechanism -->
## 地址与内存机制

`LBU` 先按 `SrcRType` 转换 `SrcR`，再按编码的 `shamt` 左移，并将结果按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。

完成全部预检后，该指令执行一次小端 `1` 字节加载，并将加载的 `1` 字节值零扩展到 `PTO_XLEN`，供目的端发布。
...
| lbu_32_a9a58ab4ea22 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
