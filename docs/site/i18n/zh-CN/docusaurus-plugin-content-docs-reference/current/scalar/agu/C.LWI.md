...
`C.LWI` 是一条独立的 `16` 位标量 AGU 指令，使用 `Compressed` 寻址，按小端序加载一个 `4` 字节值；当结果窄于 `PTO_XLEN` 时，对传输位进行符号扩展。压缩形式把加载结果发布到隐式 `T#1`。

<!-- PTO-READER-BLOCK: scalar-c-lwi-mechanism role=mechanism -->
## 地址与传输机制

地址路径先对 `simm5` 做符号扩展并乘以 `4`，再把位移与快照中的 `SrcL` 相加，结果按 `2^PTO_XLEN` 取模。
...
- `simm5` 是一个 `5` 位字段，用来选择乘以 `4` 缩放因子之前的有符号位移。
- `T#1` 是隐式加载值目的位置。
...
所有结果或回写发布完成后，`C.LWI` 把 `TPC` 前进 `2` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `c.lwi [srcL, simm], ->t` 开始，找出已编码的地址字段。
