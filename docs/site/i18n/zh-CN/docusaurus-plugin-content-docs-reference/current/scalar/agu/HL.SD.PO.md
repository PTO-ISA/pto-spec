...
`HL.SD.PO` 是一条独立的 `48` 位 AGU 指令；它形成寄存器偏移地址，并存储一个对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sd-po-mechanism role=mechanism -->
## 地址与内存机制

`HL.SD.PO` 先按 `SrcRType` 转换 `SrcR`，再将结果乘以 `8`，并按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。
...
若基址为 `0x100`、偏移源为不经转换的 `2`，并采用固定左移 `3` 位，偏移量就是 `16`，基址加偏移量为 `0x110`。内存访问使用 `0x100`，计算出的和值只在成功后发布。若地址对齐且权限允许，指令在该地址存储 `8` 字节。
<!-- SUPPLEMENTARY-END -->
