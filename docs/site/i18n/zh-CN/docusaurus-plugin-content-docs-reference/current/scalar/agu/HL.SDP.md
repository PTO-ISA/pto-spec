...
`HL.SDP` 是一条独立的 `48` 位 AGU 指令；它形成寄存器偏移地址，并存储两个相邻且对齐的小端 `8` 字节值。

<!-- PTO-READER-BLOCK: scalar-hl-sdp-mechanism role=mechanism -->
## 地址与内存机制

`HL.SDP` 先按 `SrcRType` 转换 `SrcR`，再将结果乘以 `8`，并按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。

该指令先预检两个相邻的 `8` 字节地址，再按地址递增顺序存储两个已快照的数据值。
...
若基址为 `0x100`、偏移源为不经转换的 `2`，并采用固定左移 `3` 位，偏移量就是 `16`，基址加偏移量为 `0x110`。内存访问使用 `0x110`。两个地址全部通过预检后，第二个 `8` 字节存储使用 `0x118`。
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- |
| hl_sdp_48_5884c49a7e55 | HL48 | 48 | 0x00003049001e / 0x00007ffff83f | [{"field":"SrcRType","operator":"one-of","values":[0,1,2]}] |
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| hl_sdp_48_5884c49a7e55 | SrcD | 5 | 0–31 | none | none | Reg5 first store-data source | Encoded zero reads the architectural zero GPR. |
| hl_sdp_48_5884c49a7e55 | SrcD1 | 5 | 0–31 | none | none | Reg5 second store-data source | Encoded zero reads the architectural zero GPR. |
| hl_sdp_48_5884c49a7e55 | SrcL | 5 | 0–31 | none | none | Reg5 address-base source | Encoded zero reads the architectural zero GPR. |
| hl_sdp_48_5884c49a7e55 | SrcR | 5 | 0–31 | none | none | Reg5 register-offset source | Encoded zero reads the architectural zero GPR. |
| hl_sdp_48_5884c49a7e55 | SrcRType | 2 | 0–2 | none | 3 | register-offset transformation selector | Encoded zero leaves the complete PTO_XLEN register-offset value unchanged. |

- `hl_sdp_48_5884c49a7e55.SrcRType` reserved values: Reserved encodings raise Fault_IllegalInstruction before architectural effects.
