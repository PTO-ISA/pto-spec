...
`HL.LWUI.PO` 是一条独立的 `48` 位标量 AGU 指令，使用 `Immediate` 寻址，按小端序加载一个 `4` 字节值；当结果窄于 `PTO_XLEN` 时，对传输位进行零扩展。
...
所有结果或回写发布完成后，`HL.LWUI.PO` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.lwui.po [SrcL, simm], ->Dst0, Dst1` 开始，找出已编码的地址字段。
