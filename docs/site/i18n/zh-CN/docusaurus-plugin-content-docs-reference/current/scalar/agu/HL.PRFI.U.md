...
`HL.PRFI.U` 是一条独立的 `48` 位标量 AGU 指令，使用 `Immediate` 寻址，形成非绑定预取提示，但不执行架构内存访问。
...
所有结果或回写发布完成后，`HL.PRFI.U` 把 `TPC` 前进 `6` 字节；被拒绝或发生故障的尝试不会退休。
...
- 从规范汇编形式 `hl.prfi.u{.l1,.l2,.l3} [SrcL, simm]` 开始，找出已编码的地址字段。
