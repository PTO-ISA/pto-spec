...
`LHU` 是一条独立的 `32` 位 AGU 指令；它形成寄存器偏移地址，并加载一个对齐的小端 `2` 字节值。

<!-- PTO-READER-BLOCK: scalar-lhu-mechanism role=mechanism -->
## 地址与内存机制

`LHU` 先按 `SrcRType` 转换 `SrcR`，再按编码的 `shamt` 左移，并将结果按 `2^PTO_XLEN` 取模后加到快照中的 `SrcL` 基址。

完成全部预检后，该指令执行一次小端 `2` 字节加载，并将加载的 `2` 字节值零扩展到 `PTO_XLEN`，供目的端发布。
...
若基址为 `0x100`、偏移源为不经转换的 `2`，并采用`shamt=1`，偏移量就是 `4`，基址加偏移量为 `0x104`。内存访问使用 `0x104`。若地址对齐且权限允许，指令从该地址加载 `2` 字节。
<!-- SUPPLEMENTARY-END -->
...
| lhu_32_730caf67ecd1 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero leaves the complete PTO_XLEN register-offset value unchanged. |
