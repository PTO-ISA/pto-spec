...
`TCOLEXPANDMIN` 是一条由 `SFU` 执行、通过选择器编码的 Tile 操作。它选择每个完整形状元素与同列广播值的有类型最小值；当前指令契约拥有精确的指令束形式和发布边界。

<!-- PTO-READER-BLOCK: tile-tcolexpandmin-mechanism role=mechanism -->
## 元素与 Tile 机制

所有描述符与操作数检查成功后，所属 ASL 处理函数选择每个完整形状元素与同列广播值的有类型最小值。当前契约允许别名时，源载荷会在目标写入前完成快照。
...
以一个小型 `TCOLEXPANDMIN` 示例说明：完整形状行 `[1, 5]` 与广播行 `[3, 4]` 产生 `[1, 4]`。
<!-- SUPPLEMENTARY-END -->
