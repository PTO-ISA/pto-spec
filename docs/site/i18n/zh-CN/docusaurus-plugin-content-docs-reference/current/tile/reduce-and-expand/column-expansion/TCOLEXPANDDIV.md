...
Divide a full-shape source by a first-row column-broadcast source with exact typed semantics.
...
`TCOLEXPANDDIV` 是一条由 `SFU` 执行、通过选择器编码的 Tile 操作。它把每个完整形状元素除以同列广播值；当前指令契约拥有精确的指令束形式和发布边界。

<!-- PTO-READER-BLOCK: tile-tcolexpanddiv-mechanism role=mechanism -->
## 元素与 Tile 机制

所有描述符与操作数检查成功后，所属 ASL 处理函数把每个完整形状元素除以同列广播值。当前契约允许别名时，源载荷会在目标写入前完成快照。
...
以一个小型 `TCOLEXPANDDIV` 示例说明：完整形状行 `[8, 9]` 除以广播行 `[2, 3]` 产生 `[4, 3]`。
<!-- SUPPLEMENTARY-END -->
