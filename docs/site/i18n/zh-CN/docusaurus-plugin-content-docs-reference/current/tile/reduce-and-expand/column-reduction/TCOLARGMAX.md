...
`TCOLARGMAX` 是一条由 `SFU` 执行、通过选择器编码的 Tile 操作。它按行号递增顺序扫描每一列，并返回最大值所在的最低行号；当前指令契约拥有精确的指令束形式和发布边界。

<!-- PTO-READER-BLOCK: tile-tcolargmax-mechanism role=mechanism -->
## 元素与 Tile 机制

所有描述符与操作数检查成功后，所属 ASL 处理函数按行号递增顺序扫描每一列，并返回最大值所在的最低行号。当前契约允许别名时，源载荷会在目标写入前完成快照。
...
以一个小型 `TCOLARGMAX` 示例说明：列数据 `[[1, 4], [3, 2]]` 产生最低获胜行索引 `[1, 0]`。
<!-- SUPPLEMENTARY-END -->
