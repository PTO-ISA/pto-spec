...
`TCOLPROD` 是一条由 `SFU` 执行、通过选择器编码的 Tile 操作。它按行号递增顺序对每一列执行有类型乘法折叠；当前指令契约拥有精确的指令束形式和发布边界。

<!-- PTO-READER-BLOCK: tile-tcolprod-mechanism role=mechanism -->
## 元素与 Tile 机制

所有描述符与操作数检查成功后，所属 ASL 处理函数按行号递增顺序对每一列执行有类型乘法折叠。当前契约允许别名时，源载荷会在目标写入前完成快照。
...
以一个小型 `TCOLPROD` 示例说明：列数据 `[[1, 4], [3, 2]]` 归约为 `[3, 8]`。
<!-- SUPPLEMENTARY-END -->
