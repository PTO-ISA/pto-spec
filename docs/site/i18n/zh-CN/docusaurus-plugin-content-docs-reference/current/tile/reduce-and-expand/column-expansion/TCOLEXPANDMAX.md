...
Take the typed maximum of a full-shape source and a first-row column-broadcast source.
...
`TCOLEXPANDMAX` 是一条由 `SFU` 执行、通过选择器编码的 Tile 操作。它选择每个完整形状元素与同列广播值的有类型最大值；当前指令契约拥有精确的指令束形式和发布边界。

<!-- PTO-READER-BLOCK: tile-tcolexpandmax-mechanism role=mechanism -->
## 元素与 Tile 机制

所有描述符与操作数检查成功后，所属 ASL 处理函数选择每个完整形状元素与同列广播值的有类型最大值。当前契约允许别名时，源载荷会在目标写入前完成快照。
...
以一个小型 `TCOLEXPANDMAX` 示例说明：完整形状行 `[1, 5]` 与广播行 `[3, 4]` 产生 `[3, 5]`。
<!-- SUPPLEMENTARY-END -->
...
- For every valid destination element, compute typed max(source0[r,c], BroadcastTile[0,c]) at the selected element width.
...
- TCOLEXPANDMAX is selected by the TEPL raw encoding carrier Mode 2 Function 25; canonical execution-engine assembly is BSTART.SFU and there is no standalone opcode.
...
- For every valid destination element, compute typed max(source0[r,c], operation-view BroadcastTile[0,c]) at the selected element width.
- Integer width, floating rounding, exceptional values, signed-zero behavior, and numeric status are exactly the corresponding TMAX typed operation.
...
- BSTART.SFU TCOLEXPANDMAX, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, BroadcastTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
