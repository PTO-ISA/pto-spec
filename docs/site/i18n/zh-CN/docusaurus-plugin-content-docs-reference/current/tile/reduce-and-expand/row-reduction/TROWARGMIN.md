<!-- GENERATED FROM: asl/tile/reduce-and-expand/row-reduction/TROWARGMIN.asl -->
# TROWARGMIN
...
`TROWARGMIN` 为每行最小值选择最低列索引。

<!-- PTO-READER-BLOCK: tile-c-trowargmin-mechanism role=mechanism -->
## 操作机制

每行都按严格递增的列顺序扫描，不允许重新关联。

多个相等的胜出值会保留最低列索引，目标存储 U32 索引。
...
`TROWARGMIN <bundle operands>` 对源取快照，按递增列顺序扫描每个有效行，再原子发布行结果。
<!-- SUPPLEMENTARY-END -->
...
<!-- GENERATED-ASL-BEGIN: decode source=asl/tile/reduce-and-expand/row-reduction/TROWARGMIN.asl -->
```asl
...
- BSTART.SFU TROWARGMIN, DataType; B.DATR Layout, PadValue (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, mask=PE_MASK, <last>, ->DstTile<TSize>; BSTOP
