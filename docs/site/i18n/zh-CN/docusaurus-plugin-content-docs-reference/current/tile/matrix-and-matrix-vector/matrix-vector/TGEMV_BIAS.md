...
`TGEMV_BIAS` 将矩阵与向量相乘，并加上 偏置 Tile。

<!-- PTO-READER-BLOCK: tile-tgemv-bias-mechanism role=mechanism -->
## 执行机制

ASL DOC 契约通过该指令的选择器编码块载体选择 `TileHandler_TGEMV_BIAS`。
...
`destination0` 是目的地；`source0` 是左向量；`source1` 是右矩阵；`source2` 是偏置。
