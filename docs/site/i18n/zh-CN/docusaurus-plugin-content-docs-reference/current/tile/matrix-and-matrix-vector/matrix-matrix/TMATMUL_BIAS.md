...
`TMATMUL_BIAS` 将矩阵相乘，并加上 偏置 Tile。

<!-- PTO-READER-BLOCK: tile-tmatmul-bias-mechanism role=mechanism -->
## 执行机制

ASL DOC 契约通过该指令的选择器编码块载体选择 `TileHandler_TMATMUL_BIAS`。
...
`destination0` 是目的地；`source0` 是左矩阵；`source1` 是右矩阵；`source2` 是偏置。
...
目的地使用当前操作拥有的 CUBE 布局和最终输出类型；当前助记符的偏置输入保持不变。
