...
`TMATMUL_MX_ACC` 将缩放后的矩阵相乘，并累加到给定累加器 Tile。

<!-- PTO-READER-BLOCK: tile-tmatmul-mx-acc-mechanism role=mechanism -->
## 执行机制

ASL DOC 契约通过该指令的选择器编码块载体选择 `TileHandler_TMATMUL_MX_ACC`。
...
`destination0` 是目的地；`source0` 是累加器；`source1` 是左矩阵；`source2` 是行缩放 Tile；`source3` 是右矩阵；`source4` 是列缩放 Tile。
