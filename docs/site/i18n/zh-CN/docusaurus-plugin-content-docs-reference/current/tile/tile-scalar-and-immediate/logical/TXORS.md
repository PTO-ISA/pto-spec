<!-- GENERATED FROM: asl/tile/tile-scalar-and-immediate/logical/TXORS.asl -->
# TXORS

**Normative ASL source:** `asl/tile/tile-scalar-and-immediate/logical/TXORS.asl`

Bitwise-XOR every valid integer Tile element with one scalar.
...
`TXORS` 对每个有效 integer 元素与一个标量执行按位 XOR，并发布一个新的 Local 目标。
...
`TXORS <bundle operands>` 先完成完整预检与源快照，再原子发布助记符定义的结果与填充状态。
<!-- SUPPLEMENTARY-END -->
...
<!-- GENERATED-ASL-BEGIN: decode source=asl/tile/tile-scalar-and-immediate/logical/TXORS.asl -->
```asl
...
<!-- GENERATED-ASL-BEGIN: operation source=asl/tile/tile-scalar-and-immediate/logical/TXORS.asl -->
```asl
...
- TXORS is selected only by the TEPL raw carrier Mode 1 Function 8; canonical execution-engine assembly is BSTART.VEC TXORS, DataType.
...
- For each valid element compute source XOR scalar in the selected element interpretation.
...
- BSTART.VEC TXORS, DataType; B.DATR PadValue, Layout (optional); B.DIM LB0=ValidCol; B.DIM LB1=ValidRow (optional); B.DIM LB2=Col (optional); B.IOT SrcTile, mask=PE_MASK, <last>, ->DstTile<TSize>; B.IOR ScalarGPR, zero, zero, ->zero (optional); BSTOP
