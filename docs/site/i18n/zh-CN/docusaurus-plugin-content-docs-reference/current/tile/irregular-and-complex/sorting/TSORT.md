...
`TSORT` 对独立行组执行稳定排序，并同时发布值与原始组内 U32 索引。

<!-- PTO-READER-BLOCK: tile-c-tsort-mechanism role=mechanism -->
## 操作机制

每个有效行从列零开始划分为连续的选定-宽度分组；最后一个短分组不读取填充。

排序是稳定的；两个方向都让数值值位于 NaN 前，NaN 保持源顺序，有符号零比较相等。
...
- 封闭的适用 DataType 集合为 `FP32`、`FP16`。

- 除非该助记符显式选择其他允许布局，数据 Tile 使用行主序布局。

- `LB0` 选择 `sort_width`，省略或为 `0` 时默认 `32`。`LB1`与`LB2` 必须不存在；源与两个目标保持相同的非零有效形状。
...
有效矩形外的物理坐标是未定义 Null 填充；B.DATR 的每个字段都必须保持零。
...
`TSORT <bundle operands>` 先完成完整预检与源快照，再原子发布助记符定义的结果与填充状态。
<!-- SUPPLEMENTARY-END -->
