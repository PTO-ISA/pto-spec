...
`TPARTMAX` 形成两个原点锚定 Local Tile 的并集，并在重叠区域选择最大值。
...
把 `BSTART.SFU TPARTMAX, S16; B.DIM LB0=16; B.IOT Left, Right, mask=1111, <last>, ->Dst<1>; BSTOP` 作为非规范绑定演练，再以下方生成契约确认精确维度、属性和故障行为。
<!-- SUPPLEMENTARY-END -->
