...
`TSize=SIZE` 声明 Shared destination size。其他 one-source 与 no-source form
省略无效 source operand。v5 不存在 `.reuse` modifier。
...
Matrix logical operand 顺序由 Function 与 PostProcessConfig 固定。Local stream 移除所有已由 C.B.IOS 绑定的 Shared role；destination 顺序固定为 D、RowMaxOut、GroupMaxOut。PE_MASK 对 cooperative TMATMUL 固定 1111，对 TGEMV 仍选择独立 PE。
