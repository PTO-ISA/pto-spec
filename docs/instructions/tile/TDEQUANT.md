...
{
  "schema_version": 1,
  "id": "intrinsic.tdequant",
  "kind": "intrinsic",
  "title": "TDEQUANT Intrinsic",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TDEQUANT.md",
    "pto": "tile/ops/irregular-and-complex/tdequant.md"
  },
  "opcode": "TDEQUANT",
  "family": "complex-special",
  "bundle": "BSTART.TEPL TDEQUANT, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT",
  "operands": {
    "output": "dst dequantized tile",
    "input0": "src quantized tile",
    "input1": "scale tile",
    "input2": "offset tile"
  },
  "dtypes": [
    "src: S8 or S16; dst/scale/offset: F32"
  ],
  "encoding": {
    "block": "TEPL",
    "mode": 3,
...
> 状态：DavinciOO v5 superscalar 指令页。本文只描述 `TDEQUANT` 的 PTO 语义、DavinciOO block intrinsic 展开与合法性；公共 header 的 opcode/bitfield 编码见 [`header/`](../bundle/overview/block-header-model.md)。

## PTO 语义来源

- PTO 来源页：[`../pto/TDEQUANT.md`](../pto/TDEQUANT.md)
- PTO 指令名：`TDEQUANT`
- C++ intrinsic / RecordEvent 签名摘录：

```cpp
template <typename TileDataDst, typename TileDataSrc, typename TileDataPara, typename... WaitEvents>
PTO_INST RecordEvent TDEQUANT(TileDataDst &dst, TileDataSrc &src, TileDataPara &scale, TileDataPara &offset, WaitEvents &... events);
```

PTO/DavinciOO 语义摘要：使用 scale/offset tile 把 quantized source tile 反量化为 destination tile。
...
```asm
TDEQUANT <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType>, SrcTile, ScaleTile, OffsetTile, ->DstTile<Size>
```
...
| `SrcTile` | Quantized source Tile operand。 |
| `ScaleTile` | Scale parameter Tile operand。 |
| `OffsetTile` | Offset parameter Tile operand；若后续 profile 允许缺省 offset，则该 operand 可由 profile 省略。 |
...
```asm
BSTART.TEPL TDEQUANT, FP16
B.DATR      PadValue.Null       # 可省略；省略时使用默认 data attribute
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TDEQUANT 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TDEQUANT 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TDEQUANT 需要
B.IOT       T#10, T#11, mask=1111        # SrcTile, ScaleTile
B.IOT       T#12, mask=1111, last, ->T<1KB> # OffsetTile, DstTile
```
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `TDEQUANT` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
- scale 与 offset 是 ISA-visible tile operands。

## Header 展开说明

| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TDEQUANT` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
- 二维 `TDEQUANT` 的 `ValidCol/ValidRow/Col` 均为 16-bit dimension 值，并应满足 `ValidCol <= Col`。
- src、scale、offset 与 dst 的 dtype contract 为 target/profile-defined。
- scale/offset shape 与 valid-region 兼容性由 selected backend profile 定义。
- 本指令不隐含 memory ordering、cross-PE visibility 或 group-level barrier。

## Lowering 摘要

1. Frontend 从 PTO `TDEQUANT` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride、profile/scalar 参数和 destination allocation size。
...
5. Execute 阶段按 PTO `TDEQUANT` 语义执行；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
