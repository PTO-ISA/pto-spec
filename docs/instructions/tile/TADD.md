---
{
  ...
  "sources": {
    "davincioo": "TADD.md",
    "pto": "tile/ops/elementwise-tile-tile/tadd.md"
  },
  "opcode": "TADD",
  "family": "element-wise",
  "bundle": "BSTART.TEPL TADD, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT",
  "operands": {
    "output": "dst tile",
    "input0": "src0 tile",
    "input1": "src1 tile",
...
> 状态：DavinciOO v5 superscalar 代表指令页。本文只描述 `TADD` 的 PTO 语义、DavinciOO block intrinsic 展开与合法性；公共 header 的 opcode/bitfield 编码见 [`header/`](../bundle/overview/block-header-model.md)。

## PTO 语义来源

- PTO 来源页：[`../pto/TADD.md`](../pto/TADD.md)
- PTO 指令名：`TADD`
...
PTO `TADD` 对两个 source tile 的有效区域逐元素相加，并写入 destination tile：

```text
dst[i, j] = src0[i, j] + src1[i, j]
```

PTO 语义中，执行域由 destination tile 的 valid region 决定；`src0/src1` 需要与 destination 的 valid region 和 dtype 兼容。本文定义该 PTO intrinsic 在 DavinciOO PE-local block ISA 上的展开方式；PTO 高级语义仍以 PTO 来源页为准。
...
```asm
TADD <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, SrcTile0, SrcTile1, ->DstTile<Size>
```
...
| 参数 | 说明 |
| --- | --- |
| `ValidCol` | 有效列数，写入 `LB0`。一维 `TADD` 可只声明 `LB0`。 |
| `ValidRow` | 有效行数，写入 `LB1`。二维 `TADD` 需要声明。 |
| `Col` | Tile 的总列数，写入 `LB2`。二维 row-major 访问中它也是第二行起始位置的 stride，单位为 element。 |
...
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TADD 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TADD 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TADD 需要，用于计算第二行起始位置
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `TADD` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TADD` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
- 二维 `TADD` 的 `ValidCol/ValidRow/Col` 均为 16-bit dimension 值，并应满足 `ValidCol <= Col`。
- `src0/src1/dst` 的 dtype、valid region 和 stride contract 必须兼容；不兼容时硬件不保证结果正确性或应触发 profile-defined trap。
- Source OOB 行为本轮不定义；PTO source OOB 中文/英文页差异后续在统一遗留项中收口。
- 本指令不隐含 memory ordering、cross-PE visibility 或 group-level barrier。

## Lowering 摘要

1. Frontend 从 PTO `TADD` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride 和 destination allocation size。
2. Lowering 生成 `BSTART.TEPL`、所需 `B.DIM` 与一条或多条 `B.IOT`；只有存在非默认 data attribute 时才生成 `B.DATR`。
...
5. Execute 阶段按 PTO `TADD` 语义执行逐元素加法；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
