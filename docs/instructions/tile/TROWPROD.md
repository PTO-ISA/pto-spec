...
{
  "schema_version": 1,
  "id": "intrinsic.trowprod",
  "kind": "intrinsic",
  "title": "TROWPROD Intrinsic",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TROWPROD.md",
    "pto": "tile/ops/reduce-and-expand/trowprod.md"
  },
  "opcode": "TROWPROD",
  "family": "reduce-expand",
  "bundle": "BSTART.TEPL TROWPROD, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT",
  "operands": {
    "output": "dst column/vector tile, one value per row",
    "input0": "src 2D tile",
    "input1": null,
    "input2": null
  },
...
> 状态：DavinciOO v5 superscalar 指令页。本文只描述 `TROWPROD` 的 PTO 语义、DavinciOO block intrinsic 展开与合法性；公共 header 的 opcode/bitfield 编码见 [`header/`](../bundle/overview/block-header-model.md)。

## PTO 语义来源

- PTO 来源页：[`../pto/TROWPROD.md`](../pto/TROWPROD.md)
- PTO 指令名：`TROWPROD`
...
PTO/DavinciOO 语义摘要：通过跨列乘积来归约每一行。执行域由 destination tile 的 valid region 与 `LB0/LB1/LB2` 描述的 PE-local block dimension 共同约束；dtype、mask/index、round/sat 等高级语义以 PTO 来源页和当前 active profile 为准。
...
```asm
TROWPROD <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, SrcTile0, ->DstTile<Size>
```
...
```asm
BSTART.TEPL TROWPROD, FP16
B.DATR      PadValue.Null       # 可省略；省略时使用默认 data attribute，包括 PadValue=Zero
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TROWPROD 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TROWPROD 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TROWPROD 需要，用于计算第二行起始位置
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `TROWPROD` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TROWPROD` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
- 二维 `TROWPROD` 的 `ValidCol/ValidRow/Col` 均为 16-bit dimension 值，并应满足 `ValidCol <= Col`。
...
1. Frontend 从 PTO `TROWPROD` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride、scalar/mask/index operand 和 destination allocation size。
...
5. Execute 阶段按 PTO `TROWPROD` 语义执行；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
