...
{
  "schema_version": 1,
  "id": "intrinsic.tgemv_acc",
  "kind": "intrinsic",
  "title": "TGEMV_ACC 内建函数",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TGEMV_ACC.md",
    "pto": "tile/ops/matrix-and-matrix-vector/tgemv-acc.md"
  },
  "opcode": "TGEMV_ACC",
  "family": "matrix-cube",
  "bundle": "BSTART.CUBE TGEMV.ACC AType\nB.DATR BType RMode Sat\nB.FPATR\nB.DIM LB0 M\nB.DIM LB1 N\nB.DIM LB2 K\nB.IOT Local sources and Local outputs\nB.IOR scalar PostProcess parameter (optional)",
  "operands": {
...
- PTO 来源页：[`../pto/TGEMV_ACC.md`](../pto/TGEMV_ACC.md)
- DavinciOO target public name：`TGEMV_ACC`
...
```cpp
template <PostProcessConfig PP = PostProcessConfig::None, typename TileDst, typename TileAccIn, typename TileA, typename TileB, typename... WaitEvents>
RecordEvent TGEMV_ACC(TileDst &d, TileAccIn &c, TileA &a, TileB &b, PostProcessOperands<PP> &...ppOperands, WaitEvents &...events);
```

`TGEMV*` 只定义 PE-local M=1 GEMV。API 不增加 `<pe_scope>`；`PE_MASK=1111` 表示四个 PE 各自执行一个独立 GEMV，不形成 rendezvous、共享 B 或跨 PE reduction。A、B、C/Bias、D 和 auxiliary operand 全部必须为 Local Tile，任何 `SharedTile` 或 `C.B.IOS` 都非法。
...
```asm
TGEMV.ACC <LB0:M, LB1:N, LB2:K, AType, BType, PostProcess>, C, A, B, [RowMaxIn], [QuantParam], [ReLUParam], ->D, [->RowMaxOut], [->GroupMaxOut]
```
...
B.IOT       B, mask=PE_MASK, last, ->D<TSize>
```
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.CUBE` | 选择 `TGEMV.ACC` 与 AType | [`header/BSTART.CUBE.md`](../bundle/BSTART.CUBE.md) |
