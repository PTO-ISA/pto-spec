...
`TGEMV*` 只定义 PE-local M=1 GEMV。API 不增加 `<pe_scope>`；`PE_MASK=1111` 表示四个 PE 各自执行一个独立 GEMV，不形成 rendezvous、共享 B 或跨 PE reduction。A、B、C/Bias、D 和 auxiliary operand 全部必须为 Local Tile，任何 `SharedTile` 或 `B.IOS` 都非法。
