...
- DavinciOO target public name：`TGEMV_MX_ACC`
...
```asm
TGEMVMX.ACC <LB0:M, LB1:N, LB2:K, AType, BType, PostProcess>, C, A, ScaleA, B, ScaleB, [RowMaxIn], [QuantParam], [ReLUParam], ->D, [->RowMaxOut], [->GroupMaxOut]
```
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.CUBE` | 选择 `TGEMVMX.ACC` 与 AType | [`header/BSTART.CUBE.md`](../bundle/BSTART.CUBE.md) |
