...
- DavinciOO target public name：`TGEMV_MX`
...
RecordEvent TGEMV_MX(TileDst &d, TileA &a, TileScaleA &scaleA, TileB &b, TileScaleB &scaleB, PostProcessOperands<PP> &...ppOperands, WaitEvents &...events);
```
...
```asm
TGEMVMX <LB0:M, LB1:N, LB2:K, AType, BType, PostProcess>, A, ScaleA, B, ScaleB, [RowMaxIn], [QuantParam], [ReLUParam], ->D, [->RowMaxOut], [->GroupMaxOut]
```
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.CUBE` | 选择 `TGEMVMX` 与 AType | [`header/BSTART.CUBE.md`](../bundle/BSTART.CUBE.md) |
