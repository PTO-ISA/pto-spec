---
{
  ...
  "id": "intrinsic.tsel",
  "kind": "intrinsic",
  "title": "TSEL Intrinsic",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TSEL.md",
    "pto": "tile/ops/elementwise-tile-tile/tsel.md"
  },
  "opcode": "TSEL",
  "family": "element-wise",
  "bundle": "BSTART.TEPL TSEL, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT\nB.IOT",
  "operands": {
    "output": "dst tile ",
    "input0": "mask tile",
    "input1": "src0 tile",
...
> 状态：DavinciOO v5 superscalar 指令页。本文只描述 `TSEL` 的 PTO 语义、DavinciOO block intrinsic 展开与合法性；公共 header 的 opcode/bitfield 编码见 [`header/`](../bundle/overview/block-header-model.md)。

## PTO 语义来源

- PTO 来源页：[`../pto/TSEL.md`](../pto/TSEL.md)
- PTO 指令名：`TSEL`
...
PTO/DavinciOO 语义摘要：使用掩码 Tile 在两个 Tile source 之间逐元素选择。执行域由 destination tile 的 valid region 与 `LB0/LB1/LB2` 描述的 PE-local block dimension 共同约束；dtype、mask/index、round/sat 等高级语义以 PTO 来源页和当前 active profile 为准。

PTO ISA 0.58.0 canonical encoding is `Mode=0, Function=26, selector=0x01A`;
the former DavinciOO `Mode=1 / 0x03A` value was incorrect and belongs to
`TSELS`.
...
```asm
TSEL <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, MaskTile, SrcTile0, SrcTile1, ->DstTile<Size>
```
...
| `MaskTile` | 条件选择 mask Tile，作为第一个 source operand 绑定。 |
| `SrcTile0/1` | 被选择的两个 source Tile，operand 顺序为 mask、true-source、false-source。 |
...
```asm
BSTART.TEPL TSEL, FP16
B.DATR      PadValue.Null       # 可省略；省略时使用默认 data attribute，包括 PadValue=Zero
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TSEL 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TSEL 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TSEL 需要，用于计算第二行起始位置
B.IOT       T#10, T#11, mask=1111        # MaskTile, SrcTile0
B.IOT       T#12, mask=1111, last, ->T<1KB> # SrcTile1, DstTile
```
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `TSEL` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TSEL` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
- 二维 `TSEL` 的 `ValidCol/ValidRow/Col` 均为 16-bit dimension 值，并应满足 `ValidCol <= Col`。
...
1. Frontend 从 PTO `TSEL` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride、scalar/mask/index operand 和 destination allocation size。
...
5. Execute 阶段按 PTO `TSEL` 语义执行；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
