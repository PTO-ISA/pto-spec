...
```cpp
template <PostProcessConfig PP = PostProcessConfig::None, typename TileDst, typename TileA, typename TileB, typename TileScaleA, typename TileScaleB, typename... WaitEvents>
RecordEvent TMATMUL_MX(TileDst &d, TileA &a, TileScaleA &scaleA, TileB &b, TileScaleB &scaleB, PostProcessOperands<PP> &...ppOperands, WaitEvents &...events);
```

Local form 使用 Local A/B 及 Local ScaleA/ScaleB。Cooperative form 的 `PE_MASK` 选择 Shared fixed-offset quarter；B/ScaleB 必须为 Shared，A/ScaleA 可以全 Local 或全 Shared。selected uninitialized data 采用 undefined-register semantics。
...
```asm
TMATMULMX <LB0:M, LB1:N, LB2:K, AType, BType, PostProcess>, A, ScaleA, B, ScaleB, [RowMaxIn], [QuantParam], [ReLUParam], ->D, [->RowMaxOut], [->GroupMaxOut]
```
...
B.IOT       B, ScaleB, mask=PE_MASK, last, ->D<TSize>
```
...
B.IOS     S17
B.IOS     S19
B.IOT       A, ScaleA, mask=1111, last, ->D<TSize>
```
...
- Cooperative Local-A form：`B.IOS Right`、`B.IOS ScaleRight`。
- Cooperative Shared-A form：`B.IOS Left`、`B.IOS ScaleLeft`、`B.IOS Right`、`B.IOS ScaleRight`。
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.CUBE` | 选择 `TMATMULMX` 与 AType | [`header/BSTART.CUBE.md`](../bundle/BSTART.CUBE.md) |
