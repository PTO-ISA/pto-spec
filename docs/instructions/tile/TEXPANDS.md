...
{
  "schema_version": 1,
  "id": "intrinsic.texpands",
  "kind": "intrinsic",
  "title": "TEXPANDS Intrinsic",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TEXPANDS.md",
    "pto": "tile/ops/tile-scalar-and-immediate/texpands.md"
  },
  "opcode": "TEXPANDS",
  "family": "tile-scalar",
  "bundle": "BSTART.TEPL TEXPANDS, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT\nB.IOR",
  "operands": {
    "output": "dst 2D tile",
    "input0": "src scalar",
    "input1": null,
...
> 状态：DavinciOO v5 superscalar 指令页。本文只描述 `TEXPANDS` 的 PTO 语义、DavinciOO block intrinsic 展开与合法性；公共 header 的 opcode/bitfield 编码见 [`header/`](../bundle/overview/block-header-model.md)。

## PTO 语义来源

- PTO 来源页：[`../pto/TEXPANDS.md`](../pto/TEXPANDS.md)
- PTO 指令名：`TEXPANDS`
...
PTO/DavinciOO 语义摘要：将 scalar 广播到 destination Tile。执行域由 destination tile 的 valid region 与 `LB0/LB1/LB2` 描述的 PE-local block dimension 共同约束；dtype、mask/index、round/sat 等高级语义以 PTO 来源页和当前 active profile 为准。
...
```asm
TEXPANDS <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, Scalar, ->DstTile<Size>
```
...
```asm
BSTART.TEPL TEXPANDS, FP16
B.DATR      PadValue.Null       # 可省略；省略时使用默认 data attribute，包括 PadValue=Zero
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TEXPANDS 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TEXPANDS 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TEXPANDS 需要，用于计算第二行起始位置
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TEXPANDS` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
| `B.IOT` | 绑定 destination Tile operand 并声明 output size class | [`header/B.IOT.md`](../bundle/B.IOT.md) |
...
1. Frontend 从 PTO `TEXPANDS` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride、scalar/mask/index operand 和 destination allocation size。
2. Lowering 生成 `BSTART.TEPL`、所需 `B.DIM` 与一条或多条 `B.IOT`；只有存在非默认 data attribute 时才生成 `B.DATR`，只有存在 scalar/GPR operand 时才生成 `B.IOR`。
3. Decoder 收集 block header，建立内部 tile uop/profile，并绑定 destination Tile operand。
4. Issue 阶段读取相关 Tile rename/allocation metadata，检查 dtype、shape、stride、size 与 operand role 兼容性。
5. Execute 阶段按 PTO `TEXPANDS` 语义执行；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
