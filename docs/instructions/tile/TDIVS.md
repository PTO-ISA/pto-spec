---
{
  ...
  "id": "intrinsic.tdivs",
  "kind": "intrinsic",
  "title": "TDIVS Intrinsic",
  "status": "mapped",
  "visibility": "public",
  "profile": "pto-isa-0.58.0",
  "sources": {
    "davincioo": "TDIVS.md",
    "pto": "tile/ops/tile-scalar-and-immediate/tdivs.md"
  },
  "opcode": "TDIVS",
  "family": "tile-scalar",
  "bundle": "BSTART.TEPL TDIVS, DataType\nB.DATR (optional)\nB.DIM LB0\nB.DIM (LB1/LB2 for 2D)\nB.IOT\nB.IOR",
  "operands": {
    "output": "dst tile",
    "input0": "src tile",
    "input1": "scalar value",
...
> 状态：DavinciOO v5 superscalar 指令页。本文只描述 `TDIVS` 的 PTO 语义、DavinciOO block intrinsic 展开与合法性；公共 header 的 opcode/bitfield 编码见 [`header/`](../bundle/overview/block-header-model.md)。

## PTO 语义来源

- PTO 来源页：[`../pto/TDIVS.md`](../pto/TDIVS.md)
- PTO 指令名：`TDIVS`
...
PTO/DavinciOO 语义摘要：Tile 与 scalar 的逐元素除法。执行域由 destination tile 的 valid region 与 `LB0/LB1/LB2` 描述的 PE-local block dimension 共同约束；dtype、mask/index、round/sat 等高级语义以 PTO 来源页和当前 active profile 为准。
...
```asm
TDIVS <LB0:ValidCol, LB1:ValidRow, LB2:Col, DataType, PadValue>, SrcTile0, Scalar, ->DstTile<Size>
```
...
```asm
BSTART.TEPL TDIVS, FP16
B.DATR      PadValue.Null       # 可省略；省略时使用默认 data attribute，包括 PadValue=Zero
B.DIM       rValidCol, 0, ->LB0 # ValidCol；一维 TDIVS 可只保留这一维
B.DIM       rValidRow, 0, ->LB1 # ValidRow；二维 TDIVS 需要
B.DIM       rCol, 0, ->LB2      # Col / row stride；二维 TDIVS 需要，用于计算第二行起始位置
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `TDIVS` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.TEPL` | 选择 `TDIVS` opcode profile 与主 `DataType` | [`header/BSTART.TEPL.md`](../bundle/BSTART.TEPL.md) |
...
- 二维 `TDIVS` 的 `ValidCol/ValidRow/Col` 均为 16-bit dimension 值，并应满足 `ValidCol <= Col`。
...
1. Frontend 从 PTO `TDIVS` intrinsic 取得 operand role、dtype、shape/valid 信息、row stride、scalar/mask/index operand 和 destination allocation size。
...
5. Execute 阶段按 PTO `TDIVS` 语义执行；retire 阶段提交 destination Tile 映射，source 生命周期由 reader/版本记账自动管理。
