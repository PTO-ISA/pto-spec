...
- DavinciOO target public name：`TMATMUL_MX_BIAS`
...
RecordEvent TMATMUL_MX_BIAS(TileDst &d, TileA &a, TileScaleA &scaleA, TileB &b, TileScaleB &scaleB, TileBias &bias, PostProcessOperands<PP> &...ppOperands, WaitEvents &...events);
```
...
```asm
TMATMULMX.BIAS <LB0:M, LB1:N, LB2:K, AType, BType, PostProcess>, A, ScaleA, B, ScaleB, Bias, [RowMaxIn], [QuantParam], [ReLUParam], ->D, [->RowMaxOut], [->GroupMaxOut]
```
...
| Header | 本指令用途 | 公共定义 |
| --- | --- | --- |
| `BSTART.CUBE` | 选择 `TMATMULMX.BIAS` 与 AType | [`header/BSTART.CUBE.md`](../bundle/BSTART.CUBE.md) |
