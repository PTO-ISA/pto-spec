...
PTO/DavinciOO 语义摘要：将离散内存空间中的数据聚集到 Tile 寄存器。TLSU family 负责 global memory 与 PE-local Tile state 之间的数据搬运；地址、stride、mask/index 和 dtype 约束以 PTO/Linx 来源页及当前 TLSU target profile 为准。
...
- `B.DIM LB0/LB1/LB2` 分别表达 `ValidCol/ValidRow/Col`。二维 `MGATHER` 需要 `LB2`，因为 `Col` 决定 row-major Tile 中下一行的起始 stride。
...
1. Frontend 从 `MGATHER` intrinsic 取得 GM operand、Tile operand、dtype、shape/valid 信息、GM stride、Tile stride 和 destination allocation size（若有）。
