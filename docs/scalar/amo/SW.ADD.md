...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-sw-add-purpose role=purpose -->
## What SW.ADD does

`SW.ADD` atomically applies modular addition to one word and stores the result without publishing the old value.

<!-- PTO-READER-BLOCK: scalar-sw-add-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-sw-add-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-sw-add-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-sw-add-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-sw-add-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `sw.add [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
