...
`SD.XOR` atomically applies bitwise XOR to one doubleword and stores the result without publishing the old value.
...
For a first reading, use `sd.xor [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
