...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-lw-and-purpose role=purpose -->
## What LW.AND does

`LW.AND` atomically applies bitwise AND to one word, stores the result, and publishes the prior memory value.

<!-- PTO-READER-BLOCK: scalar-lw-and-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-lw-and-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-lw-and-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-lw-and-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-lw-and-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `lw.and [SrcL], SrcR, ->Rd` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
...
```asm
lw.and [SrcL], SrcR, ->Rd
lw.and.aq [SrcL], SrcR, ->Rd
lw.and.rl [SrcL], SrcR, ->Rd
lw.and.f [SrcL], SrcR, ->Rd
lw.and.aqrl [SrcL], SrcR, ->Rd
lw.and.aqf [SrcL], SrcR, ->Rd
lw.and.rlf [SrcL], SrcR, ->Rd
lw.and.aqrlf [SrcL], SrcR, ->Rd
```
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| lw_and_32_f8dd9866b069 | RegDst | 5 | 0–31 | none | none | Reg5 old-value destination | Encoded zero discards the published old value. |
| lw_and_32_f8dd9866b069 | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| lw_and_32_f8dd9866b069 | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| lw_and_32_f8dd9866b069 | aq | 1 | 0–1 | none | none | acquire ordering bit | Encoded zero disables acquire ordering. |
| lw_and_32_f8dd9866b069 | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| lw_and_32_f8dd9866b069 | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero disables release ordering. |
...
- LW.AND computes the bitwise AND at 32-bit width and publishes the prior memory value only after a successful atomic commit.
...
- Atomically read one aligned 4-byte little-endian value, compute the width-sized bitwise AND, and write one 4-byte result to the same location.
