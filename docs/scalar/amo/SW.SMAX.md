...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-sw-smax-purpose role=purpose -->
## What SW.SMAX does

`SW.SMAX` atomically applies signed maximum to one word and stores the result without publishing the old value.

<!-- PTO-READER-BLOCK: scalar-sw-smax-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-sw-smax-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-sw-smax-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-sw-smax-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-sw-smax-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `sw.smax [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
...
```asm
sw.smax [SrcL], SrcR
sw.smax.rl [SrcL], SrcR
sw.smax.f [SrcL], SrcR
sw.smax.rlf [SrcL], SrcR
```
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| sw_smax_32_8bab38a878d7 | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| sw_smax_32_8bab38a878d7 | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| sw_smax_32_8bab38a878d7 | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| sw_smax_32_8bab38a878d7 | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero selects relaxed ordering. |
...
- SW.SMAX compares both values as two's-complement signed integers and selects the larger value at 32-bit width and stores that value; it does not publish the old value.
...
- Atomically read one aligned 4-byte little-endian value, compute the signed maximum, and write one 4-byte result to the same location.
