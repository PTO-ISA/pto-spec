<!-- GENERATED FROM: asl/scalar/amo/LW.SMAX.asl -->
# LW.SMAX

**Normative ASL source:** `asl/scalar/amo/LW.SMAX.asl`

LW.SMAX atomically stores the width-sized signed maximum and publishes the prior memory value.

## Normative identity {#PTO-INST-SCALAR-LW-SMAX}
...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-lw-smax-purpose role=purpose -->
## What LW.SMAX does

`LW.SMAX` atomically applies signed maximum to one word, stores the result, and publishes the prior memory value.

<!-- PTO-READER-BLOCK: scalar-lw-smax-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-lw-smax-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-lw-smax-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-lw-smax-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-lw-smax-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `lw.smax [SrcL], SrcR, ->Rd` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->

## Assembly

```asm
lw.smax [SrcL], SrcR, ->Rd
lw.smax.aq [SrcL], SrcR, ->Rd
lw.smax.rl [SrcL], SrcR, ->Rd
lw.smax.f [SrcL], SrcR, ->Rd
lw.smax.aqrl [SrcL], SrcR, ->Rd
lw.smax.aqf [SrcL], SrcR, ->Rd
lw.smax.rlf [SrcL], SrcR, ->Rd
lw.smax.aqrlf [SrcL], SrcR, ->Rd
```
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| lw_smax_32_838b8c673c91 | RegDst | 5 | 0–31 | none | none | Reg5 old-value destination | Encoded zero discards the published old value. |
| lw_smax_32_838b8c673c91 | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| lw_smax_32_838b8c673c91 | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| lw_smax_32_838b8c673c91 | aq | 1 | 0–1 | none | none | acquire ordering bit | Encoded zero disables acquire ordering. |
| lw_smax_32_838b8c673c91 | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| lw_smax_32_838b8c673c91 | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero disables release ordering. |
...
- LW.SMAX computes the signed maximum at 32-bit width and publishes the prior memory value only after a successful atomic commit.
...
- Atomically read one aligned 4-byte little-endian value, compute the width-sized signed maximum, and write one 4-byte result to the same location.
