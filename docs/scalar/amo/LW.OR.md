...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-lw-or-purpose role=purpose -->
## What LW.OR does

`LW.OR` atomically applies bitwise OR to one word, stores the result, and publishes the prior memory value.

<!-- PTO-READER-BLOCK: scalar-lw-or-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-lw-or-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-lw-or-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-lw-or-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-lw-or-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `lw.or [SrcL], SrcR, ->Rd` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
...
```asm
lw.or [SrcL], SrcR, ->Rd
lw.or.aq [SrcL], SrcR, ->Rd
lw.or.rl [SrcL], SrcR, ->Rd
lw.or.f [SrcL], SrcR, ->Rd
lw.or.aqrl [SrcL], SrcR, ->Rd
lw.or.aqf [SrcL], SrcR, ->Rd
lw.or.rlf [SrcL], SrcR, ->Rd
lw.or.aqrlf [SrcL], SrcR, ->Rd
```
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| lw_or_32_d6d2f87706fd | RegDst | 5 | 0–31 | none | none | Reg5 old-value destination | Encoded zero discards the published old value. |
| lw_or_32_d6d2f87706fd | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| lw_or_32_d6d2f87706fd | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| lw_or_32_d6d2f87706fd | aq | 1 | 0–1 | none | none | acquire ordering bit | Encoded zero disables acquire ordering. |
| lw_or_32_d6d2f87706fd | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| lw_or_32_d6d2f87706fd | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero disables release ordering. |
...
- LW.OR computes the bitwise OR at 32-bit width and publishes the prior memory value only after a successful atomic commit.
...
- Atomically read one aligned 4-byte little-endian value, compute the width-sized bitwise OR, and write one 4-byte result to the same location.
