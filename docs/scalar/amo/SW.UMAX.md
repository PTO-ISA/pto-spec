...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-sw-umax-purpose role=purpose -->
## What SW.UMAX does

`SW.UMAX` atomically applies unsigned maximum to one word and stores the result without publishing the old value.

<!-- PTO-READER-BLOCK: scalar-sw-umax-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-sw-umax-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-sw-umax-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-sw-umax-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-sw-umax-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `sw.umax [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
