...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-sw-and-purpose role=purpose -->
## What SW.AND does

`SW.AND` atomically applies bitwise AND to one word and stores the result without publishing the old value.

<!-- PTO-READER-BLOCK: scalar-sw-and-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-sw-and-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-sw-and-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-sw-and-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-sw-and-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `sw.and [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| sw_and_32_7ef0872d5502 | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| sw_and_32_7ef0872d5502 | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| sw_and_32_7ef0872d5502 | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| sw_and_32_7ef0872d5502 | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero selects relaxed ordering. |
...
- SW.AND computes the bitwise AND of the old value and operand at 32-bit width and stores that value; it does not publish the old value.
...
- Atomically read one aligned 4-byte little-endian value, compute the bitwise AND, and write one 4-byte result to the same location.
