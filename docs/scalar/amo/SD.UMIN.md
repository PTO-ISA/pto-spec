...
`SD.UMIN` atomically applies unsigned minimum to one doubleword and stores the result without publishing the old value.
...
For a first reading, use `sd.umin [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
