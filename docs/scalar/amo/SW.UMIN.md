...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-sw-umin-purpose role=purpose -->
## What SW.UMIN does

`SW.UMIN` atomically applies unsigned minimum to one word and stores the result without publishing the old value.

<!-- PTO-READER-BLOCK: scalar-sw-umin-mechanism role=mechanism -->
## Atomic mechanism
...
<!-- PTO-READER-BLOCK: scalar-sw-umin-inputs-outputs role=inputs-outputs -->
## Inputs and result
...
<!-- PTO-READER-BLOCK: scalar-sw-umin-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-sw-umin-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-sw-umin-example role=example -->
## Non-normative example

This example only shows one accepted spelling; the generated contract below remains authoritative.

For a first reading, use `sw.umin [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
...
```asm
sw.umin [SrcL], SrcR
sw.umin.rl [SrcL], SrcR
sw.umin.f [SrcL], SrcR
sw.umin.rlf [SrcL], SrcR
```
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| sw_umin_32_3ce114819cfc | SrcL | 5 | 0–31 | none | none | Reg5 atomic address source | Encoded zero reads the architectural zero register as the atomic address. |
| sw_umin_32_3ce114819cfc | SrcR | 5 | 0–31 | none | none | Reg5 atomic operand source | Encoded zero supplies numeric zero as the atomic operand. |
| sw_umin_32_3ce114819cfc | far | 1 | 0–1 | none | none | flat-address routing hint | Encoded zero selects the default flat-address route. |
| sw_umin_32_3ce114819cfc | rl | 1 | 0–1 | none | none | release ordering bit | Encoded zero selects relaxed ordering. |
...
- SW.UMIN compares both values as unsigned integers and selects the smaller value at 32-bit width and stores that value; it does not publish the old value.
...
- Atomically read one aligned 4-byte little-endian value, compute the unsigned minimum, and write one 4-byte result to the same location.
