...
`SW.OR` atomically applies bitwise OR to one word and stores the result without publishing the old value.
...
For a first reading, use `sw.or [SrcL], SrcR` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
