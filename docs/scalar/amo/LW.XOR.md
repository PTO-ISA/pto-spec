...
`LW.XOR` atomically applies bitwise XOR to one word, stores the result, and publishes the prior memory value.
...
For a first reading, use `lw.xor [SrcL], SrcR, ->Rd` and then vary only the ordering or route modifiers described above.
<!-- SUPPLEMENTARY-END -->
