...
`CMP.GE` evaluates signed greater-than-or-equal over decoded scalar operands and publishes canonical XLEN one or zero.

<!-- PTO-READER-BLOCK: scalar-cmp-ge-mechanism role=mechanism -->
## Mechanism

The instruction snapshots its operands, prepares the decoded right source, and then evaluates signed greater-than-or-equal.
...
`cmp.ge SrcL, SrcR<{.sw, .uw}>, ->{t, u, Rd}` publishes XLEN one when its condition is true and XLEN zero otherwise.
<!-- SUPPLEMENTARY-END -->
