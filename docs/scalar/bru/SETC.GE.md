...
`SETC.GE` evaluates signed greater-than-or-equal and publishes the result as the current Conditional bundle commit decision.
...
On success, `SETC.GE` advances `TPC` by `4` bytes. It has no scalar destination and no memory or reservation effect.
...
`setc.ge SrcL, SrcR<{.sw, .uw}>` evaluates the described condition, writes the canonical decision to commit state, and advances `TPC` only after that update.
<!-- SUPPLEMENTARY-END -->
