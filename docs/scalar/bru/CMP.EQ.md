...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-cmp-eq-purpose role=purpose -->
## What CMP.EQ does

`CMP.EQ` evaluates equality over decoded scalar operands and publishes canonical XLEN one or zero.

<!-- PTO-READER-BLOCK: scalar-cmp-eq-mechanism role=mechanism -->
## Mechanism

The instruction snapshots its operands, prepares the decoded right source, and then evaluates equality.

A true relation becomes XLEN one; a false relation becomes XLEN zero.

<!-- PTO-READER-BLOCK: scalar-cmp-eq-inputs-outputs role=inputs-outputs -->
## Inputs and output
...
<!-- PTO-READER-BLOCK: scalar-cmp-eq-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-cmp-eq-constraints role=constraints -->
## Legality and fault order

Encoding, reserved field values, and source availability are checked before destination, control, or `TPC` effects.

<!-- PTO-READER-BLOCK: scalar-cmp-eq-example role=example -->
## Non-normative example

This example illustrates the current owner and does not create a second semantic definition.

`cmp.eq SrcL, SrcR<{.sw, .uw}>, ->{t, u, Rd}` publishes XLEN one when its condition is true and XLEN zero otherwise.
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| cmp_eq_32_6af7c8f41300 | RegDst | 5 | 0–31 | none | none | absolute GPR destination | Encoded zero names the architectural zero GPR. |
| cmp_eq_32_6af7c8f41300 | SrcL | 5 | 0–31 | none | none | left absolute GPR source | Encoded zero names the architectural zero GPR. |
| cmp_eq_32_6af7c8f41300 | SrcR | 5 | 0–31 | none | none | right absolute GPR source | Encoded zero names the architectural zero GPR. |
| cmp_eq_32_6af7c8f41300 | SrcRType | 2 | 0–3 | none | none | right-source modifier selector | Encoded zero selects value zero of the right-source modifier selector. |
