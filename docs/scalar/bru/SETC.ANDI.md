...
`SETC.ANDI` derives a bitwise-AND condition and publishes it as the current Conditional bundle commit decision.
...
On success, `SETC.ANDI` advances `TPC` by `4` bytes. It has no scalar destination and no memory or reservation effect.
...
`setc.andi SrcL, simm` evaluates the described condition, writes the canonical decision to commit state, and advances `TPC` only after that update.
<!-- SUPPLEMENTARY-END -->
