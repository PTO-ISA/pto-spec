...
`CMP.GEI` evaluates signed greater-than-or-equal over decoded scalar operands and publishes canonical XLEN one or zero.
...
`cmp.gei SrcL, simm, ->{t, u, Rd}` publishes XLEN one when its condition is true and XLEN zero otherwise.
<!-- SUPPLEMENTARY-END -->
