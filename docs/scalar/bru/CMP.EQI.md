...
`CMP.EQI` evaluates equality over decoded scalar operands and publishes canonical XLEN one or zero.
...
`cmp.eqi SrcL, simm, ->{t, u, Rd}` publishes XLEN one when its condition is true and XLEN zero otherwise.
<!-- SUPPLEMENTARY-END -->
