...
`CMP.OR` applies bitwise OR to two decoded scalar values and publishes whether the combined value is nonzero.
...
`cmp.or SrcL, SrcR<.sw, .uw, .not>, ->{t, u, Rd}` publishes XLEN one when its condition is true and XLEN zero otherwise.
<!-- SUPPLEMENTARY-END -->
