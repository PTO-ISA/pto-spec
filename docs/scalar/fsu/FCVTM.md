...
`FCVTM` converts FP64, FP32, FP16, or E4M3 input to raw DstType codes `0..7` (UD/UW/UH/UB or SD/SW/SH/SB) with fixed round-down through the active numeric profile.
...
`fcvtm.fd2sd a0, ->a1` selects its carriers, snapshots its sources, invokes the active profile, accumulates returned flags, publishes the result, and then advances `TPC`.
<!-- SUPPLEMENTARY-END -->
