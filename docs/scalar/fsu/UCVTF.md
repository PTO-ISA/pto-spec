...
`UCVTF` converts unsigned 64-bit or 32-bit integer input to floating carrier code `0..14` through the active numeric profile.

<!-- PTO-READER-BLOCK: scalar-ucvtf-mechanism role=mechanism -->
## Numeric mechanism

`SrcType=00` selects an unsigned 64-bit integer carrier; `SrcType=01` selects the zero-extended low 32-bit unsigned integer carrier.
