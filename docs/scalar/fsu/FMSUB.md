...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-fmsub-purpose role=purpose -->
## What FMSUB does

`FMSUB` computes fused left times right minus addend through the active numeric profile.

<!-- PTO-READER-BLOCK: scalar-fmsub-mechanism role=mechanism -->
## Numeric mechanism
...
In the `pto-v0` reference profile, carrier multiplication minus the addend is evaluated modulo the selected width. This deterministic reference rule is not an IEEE-754 or target-hardware claim.

<!-- PTO-READER-BLOCK: scalar-fmsub-inputs-outputs role=inputs-outputs -->
## Inputs and output
...
<!-- PTO-READER-BLOCK: scalar-fmsub-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-fmsub-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-fmsub-example role=example -->
...
`fmsub.fd a0, a1, a2, ->a3` selects its carriers, snapshots its sources, invokes the active profile, accumulates returned flags, publishes the result, and then advances `TPC`.
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- |
| fmsub_32_b83012b83148 | L32 | 32 | 0x0000504b / 0x0000707f | [{"field":"SrcType","operator":"one-of","values":[0,1]}] |
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| fmsub_32_b83012b83148 | RegDst | 5 | 0–31 | none | none | Reg5 destination or discard | Encoded zero discards the result. |
| fmsub_32_b83012b83148 | SrcA | 5 | 0–31 | none | none | fused addend Reg5 source | Encoded zero reads the architectural zero GPR. |
| fmsub_32_b83012b83148 | SrcL | 5 | 0–31 | none | none | left or sole Reg5 source | Encoded zero reads the architectural zero GPR. |
| fmsub_32_b83012b83148 | SrcR | 5 | 0–31 | none | none | right Reg5 source | Encoded zero reads the architectural zero GPR. |
| fmsub_32_b83012b83148 | SrcType | 2 | 0–1 | none | 2–3 | source carrier selector | Encoded zero selects the 64-bit source carrier; it is not omission. |

- `fmsub_32_b83012b83148.SrcType` reserved values: Reserved encodings raise Fault_IllegalInstruction before architectural effects.
...
```asl
readonly func InstructionContractOperation_FMSUB()
    => ScalarOperation
begin
...
```asl
readonly func InstructionContractHandler_FMSUB()
    => ScalarSemanticHandler
begin
    return ScalarHandler_FloatingFused;
end;

pure func InstructionContractSourceTypeLegal_FMSUB(encoded: bits(2))
    => boolean
begin
    return encoded == '00' || encoded == '01';
end;

pure func InstructionContractSourceCarrier_FMSUB(encoded: bits(2))
    => bits(5)
begin
    assert InstructionContractSourceTypeLegal_FMSUB(encoded);
    return ScalarFPSourceTypeCode(encoded);
end;

pure func InstructionContractSourceArity_FMSUB()
    => integer {1..3}
begin
    return 3;
end;

pure func InstructionContractUsesProfileFlags_FMSUB()
    => boolean
begin
    return TRUE;
end;

pure func InstructionContractUsesActiveRounding_FMSUB()
    => boolean
begin
    return TRUE;
end;

pure func InstructionContractFusedOperation_FMSUB()
    => FloatingFusedOperation
begin
    return FloatingFused_MSUB;
end;
```
...
- fmsub.fd a0, a1, a2, ->a3
- fmsub.fs t#1, u#1, a0, ->t
