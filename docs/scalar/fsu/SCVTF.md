...
`SCVTF` converts signed 64-bit or 32-bit integer input to floating carrier code `0..14` through the active numeric profile.

<!-- PTO-READER-BLOCK: scalar-scvtf-mechanism role=mechanism -->
## Numeric mechanism

`SrcType=00` selects a signed 64-bit integer carrier; `SrcType=01` selects the sign-extended low 32-bit signed integer carrier.
