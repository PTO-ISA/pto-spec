...
`HL.SDI.PR` is a standalone `48`-bit AGU instruction that forms a signed-immediate address and stores one aligned little-endian `8`-byte value.

<!-- PTO-READER-BLOCK: scalar-hl-sdi-pr-mechanism role=mechanism -->
## Address and memory mechanism

`HL.SDI.PR` sign-extends `simm17` from its complete `-65536..65535` domain, multiplies it by `8`, and adds the displacement modulo `2^PTO_XLEN` to the snapshotted `SrcR` base.
