<!-- GENERATED FROM: asl/scalar/agu/HL.SW.PR.asl -->
# HL.SW.PR

**Normative ASL source:** `asl/scalar/agu/HL.SW.PR.asl`

HL.SW.PR snapshots its scalar sources, forms its encoded address, and stores one aligned little-endian 4-byte value.

## Normative identity {#PTO-INST-SCALAR-HL-SW-PR}
...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-hl-sw-pr-purpose role=purpose -->
## What HL.SW.PR does

`HL.SW.PR` is a standalone `48`-bit AGU instruction that forms a register-offset address and stores one aligned little-endian `4`-byte value.

<!-- PTO-READER-BLOCK: scalar-hl-sw-pr-mechanism role=mechanism -->
## Address and memory mechanism

`HL.SW.PR` transforms `SrcR` according to `SrcRType`, multiplies the result by `4`, and adds it modulo `2^PTO_XLEN` to the snapshotted `SrcL` base.
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-pr-inputs role=inputs-outputs -->
## Inputs and outputs
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-pr-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-pr-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-pr-example role=example -->
...
| --- | --- | ---: | --- | --- |
| hl_sw_pr_48_d80424b0a9cb | HL48 | 48 | 0x00002049002e / 0x00007fff07ff | [{"field":"SrcRType","operator":"one-of","values":[0,1,2]}] |
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| hl_sw_pr_48_d80424b0a9cb | RegDst | 5 | 0–31 | none | none | Reg5 updated-base destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_sw_pr_48_d80424b0a9cb | SrcD | 5 | 0–31 | none | none | Reg5 first store-data source | Encoded zero reads the architectural zero GPR. |
| hl_sw_pr_48_d80424b0a9cb | SrcL | 5 | 0–31 | none | none | Reg5 address-base source | Encoded zero reads the architectural zero GPR. |
| hl_sw_pr_48_d80424b0a9cb | SrcR | 5 | 0–31 | none | none | Reg5 register-offset source | Encoded zero reads the architectural zero GPR. |
| hl_sw_pr_48_d80424b0a9cb | SrcRType | 2 | 0–2 | none | 3 | register-offset transformation selector | Encoded zero leaves the complete PTO_XLEN register-offset value unchanged. |

- `hl_sw_pr_48_d80424b0a9cb.SrcRType` reserved values: Reserved encodings raise Fault_IllegalInstruction before architectural effects.
...
```asl
readonly func InstructionContractHandler_HL_SW_PR()
    => ScalarSemanticHandler
begin
    return ScalarHandler_ExecuteScalarStore;
end;

pure func InstructionContractAGUAction_HL_SW_PR()
    => ScalarAGUAction
begin
    return ScalarAGU_Store;
end;

pure func InstructionContractAGUAddressKind_HL_SW_PR()
    => ScalarAGUAddressKind
begin
    return ScalarAGU_Register;
end;

pure func InstructionContractAGUSizeBytes_HL_SW_PR()
    => integer {1,2,4,8}
begin
    return 4;
end;

pure func InstructionContractAGUOffsetScale_HL_SW_PR()
    => integer {0..3}
begin
    return 2;
end;

pure func InstructionContractAGUUpdateMode_HL_SW_PR()
    => AddressUpdateMode
begin
    return AddressUpdate_PreIndex;
end;

pure func InstructionContractAGUSignedLoad_HL_SW_PR()
    => boolean
begin
    return FALSE;
end;

pure func InstructionContractAGUPrefetchReturnsAddress_HL_SW_PR()
    => boolean
begin
    return FALSE;
end;
