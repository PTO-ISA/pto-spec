...
`HL.SH.UPR` is a standalone `48`-bit AGU instruction that forms a register-offset address and stores one aligned little-endian `2`-byte value.

<!-- PTO-READER-BLOCK: scalar-hl-sh-upr-mechanism role=mechanism -->
## Address and memory mechanism

`HL.SH.UPR` transforms `SrcR` according to `SrcRType` and adds the unscaled result modulo `2^PTO_XLEN` to the snapshotted `SrcL` base.
...
| hl_sh_upr_48_719102d66e45 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
