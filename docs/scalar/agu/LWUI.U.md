...
`LWUI.U` is a standalone `32`-bit AGU instruction that forms a signed-immediate address and loads one aligned little-endian `4`-byte value.

<!-- PTO-READER-BLOCK: scalar-lwui-u-mechanism role=mechanism -->
## Address and memory mechanism

`LWUI.U` sign-extends `simm12` from its complete `-2048..2047` domain, uses it without scaling, and adds the displacement modulo `2^PTO_XLEN` to the snapshotted `SrcL` base.
