...
`HL.LDP` is a standalone `48`-bit scalar AGU instruction that loads two adjacent 8-byte little-endian values and zero-extends each transferred value when it is narrower than `PTO_XLEN` using `Register` addressing.
...
After all result or writeback publication, `HL.LDP` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.
...
- Start with the canonical assembly `hl.ldp [SrcL, SrcR<{.sw,.uw,.neg}><<<shamt>], ->Dst0, Dst1` and identify the encoded address fields.
...
| hl_ldp_48_a7a45a43dff9 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
