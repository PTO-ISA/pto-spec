...
`HL.LWIP` is a standalone `48`-bit scalar AGU instruction that loads two adjacent 4-byte little-endian values and sign-extends each transferred value when it is narrower than `PTO_XLEN` using `Immediate` addressing.
...
After all result or writeback publication, `HL.LWIP` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.
...
- Start with the canonical assembly `hl.lwip [SrcL, simm], ->Dst0, Dst1` and identify the encoded address fields.
