...
`HL.LWUIP.U` is a standalone `48`-bit scalar AGU instruction that loads two adjacent 4-byte little-endian values and zero-extends each transferred value when it is narrower than `PTO_XLEN` using `Immediate` addressing.
...
After all result or writeback publication, `HL.LWUIP.U` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.
...
- Start with the canonical assembly `hl.lwuip.u [SrcL, simm], ->Dst0, Dst1` and identify the encoded address fields.
