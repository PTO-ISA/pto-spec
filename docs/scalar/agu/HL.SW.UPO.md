...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-hl-sw-upo-purpose role=purpose -->
## What HL.SW.UPO does

`HL.SW.UPO` is a standalone `48`-bit AGU instruction that forms a register-offset address and stores one aligned little-endian `4`-byte value.

<!-- PTO-READER-BLOCK: scalar-hl-sw-upo-mechanism role=mechanism -->
## Address and memory mechanism

`HL.SW.UPO` transforms `SrcR` according to `SrcRType` and adds the unscaled result modulo `2^PTO_XLEN` to the snapshotted `SrcL` base.
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-upo-inputs role=inputs-outputs -->
## Inputs and outputs
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-upo-effects role=effects -->
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-upo-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-hl-sw-upo-example role=example -->
## Non-normative address example

This example demonstrates the address calculation only; exact behavior remains in the current ASL and instruction contract.

With base `0x100`, unchanged offset source `4`, and the fixed shift `0`, the aligned displacement is `4` and base plus displacement is `0x104`. The memory access uses `0x100`, and the aligned computed sum is published only after success. If permitted, the instruction stores `4` bytes at that aligned address.
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- |
| hl_sw_upo_48_59be7b468f8a | HL48 | 48 | 0x00006049003e / 0x00007fff07ff | [{"field":"SrcRType","operator":"one-of","values":[0,1,2]}] |
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| hl_sw_upo_48_59be7b468f8a | RegDst | 5 | 0–31 | none | none | Reg5 updated-base destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_sw_upo_48_59be7b468f8a | SrcD | 5 | 0–31 | none | none | Reg5 first store-data source | Encoded zero reads the architectural zero GPR. |
| hl_sw_upo_48_59be7b468f8a | SrcL | 5 | 0–31 | none | none | Reg5 address-base source | Encoded zero reads the architectural zero GPR. |
| hl_sw_upo_48_59be7b468f8a | SrcR | 5 | 0–31 | none | none | Reg5 register-offset source | Encoded zero reads the architectural zero GPR. |
| hl_sw_upo_48_59be7b468f8a | SrcRType | 2 | 0–2 | none | 3 | register-offset transformation selector | Encoded zero leaves the complete PTO_XLEN register-offset value unchanged. |

- `hl_sw_upo_48_59be7b468f8a.SrcRType` reserved values: Reserved encodings raise Fault_IllegalInstruction before architectural effects.
...
```asl
readonly func InstructionContractHandler_HL_SW_UPO()
    => ScalarSemanticHandler
begin
    return ScalarHandler_ExecuteScalarStore;
end;

pure func InstructionContractAGUAction_HL_SW_UPO()
    => ScalarAGUAction
begin
    return ScalarAGU_Store;
end;

pure func InstructionContractAGUAddressKind_HL_SW_UPO()
    => ScalarAGUAddressKind
begin
    return ScalarAGU_Register;
end;

pure func InstructionContractAGUSizeBytes_HL_SW_UPO()
    => integer {1,2,4,8}
begin
    return 4;
end;

pure func InstructionContractAGUOffsetScale_HL_SW_UPO()
    => integer {0..3}
begin
    return 0;
end;

pure func InstructionContractAGUUpdateMode_HL_SW_UPO()
    => AddressUpdateMode
begin
    return AddressUpdate_PostIndex;
end;

pure func InstructionContractAGUSignedLoad_HL_SW_UPO()
    => boolean
begin
    return FALSE;
end;

pure func InstructionContractAGUPrefetchReturnsAddress_HL_SW_UPO()
    => boolean
begin
    return FALSE;
end;
