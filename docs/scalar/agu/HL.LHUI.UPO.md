...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-hl-lhui-upo-purpose role=purpose -->
## What HL.LHUI.UPO does

`HL.LHUI.UPO` is a standalone `48`-bit scalar AGU instruction that loads one 2-byte little-endian value and zero-extends the transferred bits when the result is narrower than `PTO_XLEN` using `Immediate` addressing.

<!-- PTO-READER-BLOCK: scalar-hl-lhui-upo-mechanism role=mechanism -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhui-upo-inputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhui-upo-effects role=effects -->
...
After all result or writeback publication, `HL.LHUI.UPO` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.

<!-- PTO-READER-BLOCK: scalar-hl-lhui-upo-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhui-upo-example role=example -->
## Non-normative reading walkthrough

This walkthrough explains how to use the page and does not add instruction behavior.

- Start with the canonical assembly `hl.lhui.upo [SrcL, simm], ->Dst0, Dst1` and identify the encoded address fields.
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| hl_lhui_upo_48_7f2c3eae793e | RegDst0 | 5 | 0–31 | none | none | Reg5 first loaded-value destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_lhui_upo_48_7f2c3eae793e | RegDst1 | 5 | 0–31 | none | none | Reg5 updated-base destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_lhui_upo_48_7f2c3eae793e | SrcL | 5 | 0–31 | none | none | Reg5 address-base source | Encoded zero reads the architectural zero GPR. |
| hl_lhui_upo_48_7f2c3eae793e | simm17 | 17 | 0–131071 | none | none | signed address displacement | Encoded zero supplies a zero displacement; it does not denote omission. |
...
```asl
readonly func InstructionContractHandler_HL_LHUI_UPO()
    => ScalarSemanticHandler
begin
    return ScalarHandler_ExecuteScalarLoad;
end;

pure func InstructionContractAGUAction_HL_LHUI_UPO()
    => ScalarAGUAction
begin
    return ScalarAGU_Load;
end;

pure func InstructionContractAGUAddressKind_HL_LHUI_UPO()
    => ScalarAGUAddressKind
begin
    return ScalarAGU_Immediate;
end;

pure func InstructionContractAGUSizeBytes_HL_LHUI_UPO()
    => integer {1,2,4,8}
begin
    return 2;
end;

pure func InstructionContractAGUOffsetScale_HL_LHUI_UPO()
    => integer {0..3}
begin
    return 0;
end;

pure func InstructionContractAGUUpdateMode_HL_LHUI_UPO()
    => AddressUpdateMode
begin
    return AddressUpdate_PostIndex;
end;

pure func InstructionContractAGUSignedLoad_HL_LHUI_UPO()
    => boolean
begin
    return FALSE;
end;

pure func InstructionContractAGUPrefetchReturnsAddress_HL_LHUI_UPO()
    => boolean
begin
    return FALSE;
end;
