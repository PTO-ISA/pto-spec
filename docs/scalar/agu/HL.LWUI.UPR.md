...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-hl-lwui-upr-purpose role=purpose -->
## What HL.LWUI.UPR does

`HL.LWUI.UPR` is a standalone `48`-bit scalar AGU instruction that loads one 4-byte little-endian value and zero-extends the transferred bits when the result is narrower than `PTO_XLEN` using `Immediate` addressing.

<!-- PTO-READER-BLOCK: scalar-hl-lwui-upr-mechanism role=mechanism -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lwui-upr-inputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lwui-upr-effects role=effects -->
...
After all result or writeback publication, `HL.LWUI.UPR` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.

<!-- PTO-READER-BLOCK: scalar-hl-lwui-upr-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lwui-upr-example role=example -->
## Non-normative reading walkthrough

This walkthrough explains how to use the page and does not add instruction behavior.

- Start with the canonical assembly `hl.lwui.upr [SrcL, simm], ->Dst0, Dst1` and identify the encoded address fields.
