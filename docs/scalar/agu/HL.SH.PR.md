...
`HL.SH.PR` is a standalone `48`-bit AGU instruction that forms a register-offset address and stores one aligned little-endian `2`-byte value.

<!-- PTO-READER-BLOCK: scalar-hl-sh-pr-mechanism role=mechanism -->
## Address and memory mechanism

`HL.SH.PR` transforms `SrcR` according to `SrcRType`, multiplies the result by `2`, and adds it modulo `2^PTO_XLEN` to the snapshotted `SrcL` base.
...
With base `0x100`, unchanged offset source `2`, and the fixed shift `1`, the offset is `4` and base plus offset is `0x104`. The memory access uses `0x104`. If aligned and permitted, the instruction stores `2` bytes at that address.
<!-- SUPPLEMENTARY-END -->
...
| --- | --- | ---: | --- | --- |
| hl_sh_pr_48_40ab17f5a580 | HL48 | 48 | 0x00001049002e / 0x00007fff07ff | [{"field":"SrcRType","operator":"one-of","values":[0,1,2]}] |
...
| hl_sh_pr_48_40ab17f5a580 | SrcRType | 2 | 0–2 | none | 3 | register-offset transformation selector | Encoded zero leaves the complete PTO_XLEN register-offset value unchanged. |

- `hl_sh_pr_48_40ab17f5a580.SrcRType` reserved values: Reserved encodings raise Fault_IllegalInstruction before architectural effects.
