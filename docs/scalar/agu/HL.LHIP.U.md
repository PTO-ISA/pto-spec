<!-- GENERATED FROM: asl/scalar/agu/HL.LHIP.U.asl -->
# HL.LHIP.U

**Normative ASL source:** `asl/scalar/agu/HL.LHIP.U.asl`
...
## Normative identity {#PTO-INST-SCALAR-HL-LHIP-U}
...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-hl-lhip-u-purpose role=purpose -->
## What HL.LHIP.U does

`HL.LHIP.U` is a standalone `48`-bit scalar AGU instruction that loads two adjacent 2-byte little-endian values and sign-extends each transferred value when it is narrower than `PTO_XLEN` using `Immediate` addressing.

<!-- PTO-READER-BLOCK: scalar-hl-lhip-u-mechanism role=mechanism -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhip-u-inputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhip-u-effects role=effects -->
...
After all result or writeback publication, `HL.LHIP.U` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.

<!-- PTO-READER-BLOCK: scalar-hl-lhip-u-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhip-u-example role=example -->
## Non-normative reading walkthrough

This walkthrough explains how to use the page and does not add instruction behavior.

- Start with the canonical assembly `hl.lhip.u [SrcL, simm], ->Dst0, Dst1` and identify the encoded address fields.
...
| --- | --- | ---: | --- | --- | --- | --- | --- |
| hl_lhip_u_48_e37e98d63bfd | RegDst0 | 5 | 0–31 | none | none | Reg5 first loaded-value destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_lhip_u_48_e37e98d63bfd | RegDst1 | 5 | 0–31 | none | none | Reg5 second loaded-value destination or discard | Encoded zero discards this result without suppressing the instruction's other effects. |
| hl_lhip_u_48_e37e98d63bfd | SrcL | 5 | 0–31 | none | none | Reg5 address-base source | Encoded zero reads the architectural zero GPR. |
| hl_lhip_u_48_e37e98d63bfd | simm17 | 17 | 0–131071 | none | none | signed address displacement | Encoded zero supplies a zero displacement; it does not denote omission. |
...
```asl
readonly func InstructionContractHandler_HL_LHIP_U()
    => ScalarSemanticHandler
begin
    return ScalarHandler_ExecuteScalarLoadPair;
end;

pure func InstructionContractAGUAction_HL_LHIP_U()
    => ScalarAGUAction
begin
    return ScalarAGU_LoadPair;
end;

pure func InstructionContractAGUAddressKind_HL_LHIP_U()
    => ScalarAGUAddressKind
begin
    return ScalarAGU_Immediate;
end;

pure func InstructionContractAGUSizeBytes_HL_LHIP_U()
    => integer {1,2,4,8}
begin
    return 2;
end;

pure func InstructionContractAGUOffsetScale_HL_LHIP_U()
    => integer {0..3}
begin
    return 0;
end;

pure func InstructionContractAGUUpdateMode_HL_LHIP_U()
    => AddressUpdateMode
begin
    return AddressUpdate_None;
end;

pure func InstructionContractAGUSignedLoad_HL_LHIP_U()
    => boolean
begin
    return TRUE;
end;

pure func InstructionContractAGUPrefetchReturnsAddress_HL_LHIP_U()
    => boolean
begin
    return FALSE;
end;
