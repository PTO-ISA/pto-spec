...
<!-- SUPPLEMENTARY-BEGIN -->
<!-- PTO-READER-BLOCK: scalar-hl-lhi-pr-purpose role=purpose -->
## What HL.LHI.PR does

`HL.LHI.PR` is a standalone `48`-bit scalar AGU instruction that loads one 2-byte little-endian value and sign-extends the transferred bits when the result is narrower than `PTO_XLEN` using `Immediate` addressing.

<!-- PTO-READER-BLOCK: scalar-hl-lhi-pr-mechanism role=mechanism -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhi-pr-inputs role=inputs-outputs -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhi-pr-effects role=effects -->
...
After all result or writeback publication, `HL.LHI.PR` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.

<!-- PTO-READER-BLOCK: scalar-hl-lhi-pr-constraints role=constraints -->
...
<!-- PTO-READER-BLOCK: scalar-hl-lhi-pr-example role=example -->
## Non-normative reading walkthrough

This walkthrough explains how to use the page and does not add instruction behavior.

- Start with the canonical assembly `hl.lhi.pr [SrcL, simm], ->Dst0, Dst1` and identify the encoded address fields.
