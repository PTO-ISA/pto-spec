...
`HL.LB.PO` is a standalone `48`-bit scalar AGU instruction that loads one 1-byte little-endian value and sign-extends the transferred bits when the result is narrower than `PTO_XLEN` using `Register` addressing.
...
After all result or writeback publication, `HL.LB.PO` advances `TPC` by `6` bytes; a rejected or faulting attempt does not retire.
...
- Start with the canonical assembly `hl.lb.po [SrcL, SrcR<{.sw,.uw,.neg}><<<shamt>], ->Dst0, Dst1` and identify the encoded address fields.
...
| hl_lb_po_48_5c7f5c82b186 | SrcRType | 2 | 0–3 | none | none | register-offset transformation selector | Encoded zero sign-extends SrcR[31:0] to PTO_XLEN; it does not mean an omitted modifier. |
