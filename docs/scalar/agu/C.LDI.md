...
`C.LDI` is a standalone `16`-bit scalar AGU instruction that loads one 8-byte little-endian value and zero-extends the transferred bits when the result is narrower than `PTO_XLEN` using `Compressed` addressing. The compressed form publishes the loaded result to implicit `T#1`.
...
After all result or writeback publication, `C.LDI` advances `TPC` by `2` bytes; a rejected or faulting attempt does not retire.
...
- Start with the canonical assembly `c.ldi [srcL, simm], ->t` and identify the encoded address fields.
