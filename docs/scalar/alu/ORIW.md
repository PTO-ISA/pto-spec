...
`ORIW` is a 32-bit scalar ALU instruction. It performs bitwise inclusive OR under the low 32-bit word, followed by sign-extension to XLEN result rules; its current instruction contract defines the result publication path and any additional state effect.
...
For a small `ORIW` example, `SrcL=0xc` and `simm12=0xa` produce `0xe`.
<!-- SUPPLEMENTARY-END -->
