...
`OR` is a 32-bit scalar ALU instruction. It performs bitwise inclusive OR under the complete XLEN value result rules; its current instruction contract defines the result publication path and any additional state effect.
...
For a small `OR` example, `SrcL=0xc`, `SrcR=0xa`, `SrcRType=11`, and `shamt=0` produce `0xe`.
<!-- SUPPLEMENTARY-END -->
