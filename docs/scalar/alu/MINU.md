...
`MINU` is a 32-bit scalar ALU instruction. It compares the complete operands as unsigned values and selects the minimum bit pattern; its current instruction contract defines the result publication path and any additional state effect.

<!-- PTO-READER-BLOCK: scalar-minu-mechanism role=mechanism -->
## How the result is formed

Execution snapshots the encoded inputs, then compares the complete operands as unsigned values and selects the minimum bit pattern, and only afterward performs the destination effects.
...
For a small `MINU` example, operands `7` and `3` select result `3`.
<!-- SUPPLEMENTARY-END -->
