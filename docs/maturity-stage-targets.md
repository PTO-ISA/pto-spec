...
| `S6-T1-B` — links and boundaries | Closed | Attach PTO requirement, model, executable witness, and bounded status to every unit without turning instrumentation into ISA state. | All paths exist; 73 state roots expand to 238 leaf fields and classify architectural state, storage/ordering/effect abstractions, or verification-only state. |
