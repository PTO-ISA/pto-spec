...
| 5 — conformance (`S5-T1`–`S5-T3`) | M4 regression floor remains green. | Independently validate all 18 numeric domains, 28 hooks, and 104 operations, while retaining the closed 682-row executable-model comparison: 648 exact matches, 11 classified divergences, and 23 non-comparable rows. | All 12 numeric decisions and 18 domain rules are accepted; six oracles, complete vectors/results, zero unclassified mismatches, and two reviews are recorded. | In progress: `S5-T2` open |
| 6 — release (`S6-T1`–`S6-T2`) | Stage 5 closed. | Close cumulative traceability and reproduce every release gate at one signed immutable candidate. | Eleven dependent requirements and 28 hooks close; all 924 trace units, ten gates, manual release validation, and nine retained controls name the same commit. | Open |
...
| `S4-T9` | TLSU | Closed | All ten selectors have decoded effects plus packed four-bit, duplicate-lane, masked, compare-and-swap, event, restart, and first/middle/last preflight-fault evidence. |
...
| `S5-T3` | Closed | The exhaustive 682-row disposition matrix is complete: 648 exact matches, 11 explicit divergences, and 23 non-comparable rows. All eight clean-snapshot repository gates plus the pinned Sail parser/C-backend gate pass. The separate `spec/evidence/noncomparable-oracle-coverage.json` ledger owns the third grade. Independent executable parity for the 23 rows is orthogonal and remains 0/23. |
...
| `S6-T1-B` — links and boundaries | Closed | Attach PTO requirement, model, executable witness, and bounded status to every unit without turning instrumentation into ISA state. | All paths exist; 74 state roots expand to 233 leaf fields and classify architectural state, storage/ordering/effect abstractions, or verification-only state. |
...
| `S6-T2-B` — execution topology | Closed | Prove the manual release entry point and parallel suite are bounded and exact. | `make release-validate` invokes the pinned toolchain and exact 34-shard/110-call/105-subprogram topology. |
