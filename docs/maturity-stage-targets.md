...
| 2 | `S6-T1` | PTO release maintainers | Close the nine Stage-5-dependent requirements and 30 numeric hooks, then review the 937-unit traceability ledger at one immutable commit. | Depends on `S5-T2`; immutable-commit claim-hygiene review is absent. |
...
| 6 — release (`S6-T1`–`S6-T2`) | Stage 5 closed. | Close cumulative traceability and reproduce every release gate at one signed immutable candidate. | Nine dependent requirements and 30 hooks close; all 937 trace units, ten gates, ten controls, hosted validation, and both approvals name the same commit. | Open |
...
| Sub-stage | Current state | Clear target | Required exit evidence |
| --- | --- | --- | --- |
| `S6-T1-A` — exact inventory | Closed | Enumerate every release-traceability unit exactly once. | 44 requirements, 474 scalar forms, 99 command forms, 120 tile operations, 72 system registers, 13 traps, 37 hooks, and 70 state roots are present with globally unique IDs. |
...
| `S6-T1-C` — cumulative closure | Blocked by S5-T2 | Close every Stage 0–5 prerequisite and dependent requirement status. | S5-T2 closes; the nine currently open requirement rows and 30 numeric hooks receive accepted conformance evidence. |
