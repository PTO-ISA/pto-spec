...
| `S2-T4` | 2 | Closed | Define architectural use or reserved behavior for the eight 32-bit warp predicates and the separate 64-bit MPAR/MSEQ execution mask; reconcile the current public v0.6 typed-mask source surface without broadening 0.57.1; no visible predicate-domain state remains accidental. | ADRs 0046 and 0051 plus P0 hardwiring, P1/P7 width boundaries, all-nine bundle-kind domain selection, branch-source selection, trap preservation, reset, no-encoding-consumer tests, and exact-pin/source-class predicate-version evidence |
...
| `S4-T10` | CUBE | Matrix, bias, accumulate, MX, ACCCVT, and matrix/vector forms | Closed: ADR 0034 and `cube-totality.json` cover all 13 selectors, 25 raw carriers, layouts, aliases, and composite preflight; target numeric results remain `S5-T2`. |
