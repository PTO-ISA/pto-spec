...
| --- | --- | --- | --- | --- |
| `S0-T1` | 0 | Closed | Freeze and regenerate the 474 scalar, 99 bundle/command, and 120 direct-tile accepted-form inventories. | Deterministic generation diff, catalog checks, positive and reserved-code witnesses |
...
| `S4-T9` | 4 | Closed | Close TLSU precise accesses, masks, restart, atomicity, ordering, fault precedence, and sub-byte transfer disposition. | ADR 0033; all-nine decoded-selector matrix; packed nibble, duplicate-index, masked, CAS, event, and first/middle/last fault-preflight evidence |
...
- Cover every semantically distinct equivalence class across all 474 scalar
  forms, 96 command forms, and 106 tile selectors.
...
| `S4-T9` | TLSU | Load/store/move/prefetch/gather/scatter/masked/CAS | Closed: ADR 0033 and `tlsu-totality.json` cover all ten selectors, packed sub-byte transfer, duplicate lanes, masks, compare-and-swap, events, atomicity, restart, and first/middle/last preflight faults. |
...
promotion input. It covers 924 globally unique traceability units: 47
...
ASL state roots. Composite roots expand to 233 leaf fields so nested bundle,
...
manual release entry point, and proves the exact 36-shard/129-call/126-subprogram
...
| `S6-T2-B` | Closed | Manual and parallel execution contract | `make release-validate` and the exact 36/129/126 partition pass fail-closed checks. |
