...
| Encodings and execution status | 474 scalar forms + 107 bundle/command forms + 120 direct tile operations | M4 instruction reference semantics closed | generated decoders, operand/handler bindings, reserved-code rejection, one-tick success/rejection matrix, stale-fault isolation, preserved trap record, scalar/command/tile legality witnesses, independently calculated decoded effects, and a fail-closed maturity ownership ledger | Stage 5 numeric conformance and the final independent documentation gate remain open |
