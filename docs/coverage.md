...
| Hardware numeric profile | 25 accepted DataTypes; exact OCP/HiF encodings and packed lanes; B.DATR round/sat/canonicalize/compare; conversion, reduction, sort, and ordinary/MX matrix rules | normative contract and generated boundary vectors; implementation conformance unproven | checked profile schema, low-format/packed-lane/canonical-NaN/signed-zero/rounding/invalid-conversion/reduction/32-element sort/K-block matrix vectors, release-manifest hash | independent hardware, emulator, RTL, and model parity evidence |
