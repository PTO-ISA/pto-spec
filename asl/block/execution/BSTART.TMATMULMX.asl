...
// [K,group_M] for TransA=1.  Its physical index MUST be m*pitch+k (K-major)
// for TransA=0 or k*pitch+m (M-major) for TransA=1.  Shared B MUST have exact
// physical valid shape [N,K] for TransB=0 or [K,N] for TransB=1.  Its physical
// index MUST be n*pitch+k (K-major, ordinary dense column-major/DN-equivalent)
// for TransB=0 or k*pitch+n (N-major, ordinary dense row-major) for TransB=1.
// Physical columns MAY be padded only within legal descriptor capacity.
...
// NDF-END: PTO-BSTART-TMATMULMX-CONTRACT-001


// DOC-BEGIN: decode
readonly func InstructionContractMatches_BSTART_TMATMULMX(
    operation: CommandOperation) => boolean
begin
    return operation ==
        CommandOperation_bstart_tmatmulmx_32_e26cb3874648;
end;
// DOC-END: decode
// DOC-BEGIN: operation
readonly func InstructionContractTileOperation_BSTART_TMATMULMX()
    => TileOperation
begin
    return TileOperation_TMATMUL_MX;
end;

readonly func InstructionContractCubeFunction_BSTART_TMATMULMX()
    => integer {0..31}
begin
    return 4;
end;

readonly func InstructionContractSharedOperandsAllowed_BSTART_TMATMULMX()
    => boolean
begin
    return TRUE;
end;

readonly func InstructionContractHandler_BSTART_TMATMULMX()
