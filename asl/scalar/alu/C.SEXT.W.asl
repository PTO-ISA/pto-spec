// PTO-INSTRUCTION: {"assembly":["c.sext.w srcL, ->t"],"block":[],"catalog_indices":[41],"catalog_records":[{"asm":"c.sext.w srcL, ->t","constraints":[],"encoding":[{"index":0,"mask":"0xf83f","match":"0x501c","width_bits":16}],"encoding_kind":"C16","fields":[{"name":"SrcL","pieces":[{"instruction_lsb":6,"value_lsb":0,"width":5}],"signedness":"encoding-defined","width":5}],"form_id":"c_sext_w_16_f2bb13f0797b","length_bits":16,"mnemonic":"C.SEXT.W","semantic_family":"ALU","semantic_group":"ALU","semantic_handler":"ExtendScalarValue","semantic_summary":"C.SEXT.W sign-extends SrcL[31:0] to XLEN and pushes the result to T.","status":"accepted"}],"classification":["alu"],"contract":{"block_composition":["none"],"canonical_assembly":["c.sext.w srcL, ->t"],"defaults":["Every encoded source, immediate, and explicit destination field is required; no field can be omitted.","The mnemonic fixes immediate signedness, selected source width, and implicit-versus-explicit destination behavior."],"encoding_class":"standalone-encoded","examples":["c.sext.w srcl, ->t"],"exceptions":["Materialization, movement, and extension are total fixed-width operations and raise no arithmetic exception.","An unavailable selected T/U source raises Fault_IllegalInstruction before the destination effect and before TPC advances."],"field_contracts":{},"field_zero_meanings":{"SrcL":"Encoded zero reads the architectural zero GPR."},"legality":["SrcL codes 0..23 select absolute GPRs, 24..27 select T#1..T#4, and 28..31 select U#1..U#4 without consumption.","The compressed form has no destination field and always pushes exactly one result to T.","Every encoded operand value is assigned; fixed encoding bits must match the canonical form."],"memory_effects":["none"],"operands":[{"field":"SrcL","role":"Reg5 source"}],"ordering":["Snapshot any Reg5 source before the destination effect.","Publish the result, then advance TPC by the encoded instruction length."],"standalone_opcode":true,"state_effects":["Sign-extend source bit 31 through the XLEN result.","Push the complete XLEN result to T. The source queue is non-consuming, and no explicit destination encoding exists.","No memory, reservation, descriptor, numeric-status, block, privilege, branch-target, or other control state changes. Successful execution advances TPC by two bytes."]},"depends_on":["PTO-BLOCK-MODEL-SCHEMA-BUNDLE-ENCODING"],"id":"PTO-SCALAR-C-SEXT-W","mnemonic":"C.SEXT.W","summary":"C.SEXT.W sign-extends SrcL[31:0] to XLEN and pushes the result to T.","surface":"scalar"}
