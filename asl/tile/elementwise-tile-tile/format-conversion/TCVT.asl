...
// FP32 sources and MUST apply PTO-TCVT-E8M0-001 exactly.
...
// from the backing type only when both types are non-packed, equal-width, and
// carrier-compatible. This operation-view rule also applies to CUBE_M16/M32
// without mutating the source descriptor. The destination backing type MUST
// equal the resolved destination operation type.
// Every source/destination pair in the shared scalar twelve-type set MUST
// apply PTO-COMMON-CONVERSION-001 with the resolved RMode and Sat.
// E2M1X2 and E1M2X2 convert in adjacent logical column pairs, with an odd
// terminal column represented as [valid | padding]. HiF4X2 is not a legal
// TCVT source or destination. E6M2 is legal only with FP16/BF16, RCPE6M2 is
// source-only to FP16/BF16, and no conversion to RCPE6M2 is defined.
// E8M0 is source-only to FP16/BF16/FP32: 0x00..0xFE denote 2^(code-127),
// while 0xFF produces the target canonical quiet NaN without NV.
