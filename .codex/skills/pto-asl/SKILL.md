...
For instruction-set changes, also keep the three coverage layers synchronized:

1. the accepted form or operation in `spec/catalog/`;
2. its decoder witness generated by `scripts/generate-asl-decoders`;
3. an ASL semantic handler and executable feature test enforced by `scripts/check-catalogs`.

An accepted spelling without all three layers is a specification defect. Do not reduce semantic coverage to a mnemonic
count, and do not hand-edit generated decoder output under `build/`.
...
- Every accepted tile operation resolves to exactly one checked ASL semantic handler.
