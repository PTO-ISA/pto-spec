...
For instruction-set changes, also keep the four coverage layers synchronized:
...
4. an executable feature test enforced by `scripts/check-catalogs`.

An accepted spelling without all four layers is a specification defect. Do not reduce semantic coverage to a mnemonic
