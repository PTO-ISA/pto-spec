...
def fixture(row: dict, operation_index: int, role: str, starts: dict[str, int],
    ...
    return {
        "case_id": f"RANGE-MATRIX-CASE-{case_index + 1:04d}",
        ...
        "test_id": test_id,
        "row_assertion": (f"case_{case_index + 1:04d}_exact_fault"
                          if expected_fault else
                          f"case_{case_index + 1:04d}_normal_commit"),
        "shard": shard,
...
def render_shard(rows: list[dict], shard: int) -> str:
    test_id = f"PTO-AVS-BLOCK-RANGE-OPERATION-MATRIX-{shard + 1:03d}"
    ids = ", ".join(row["case_id"] for row in rows)
    lines = [
        ...
        f"// Generated by scripts/generate-bundle-operation-matrix.py; cases: {ids}",
    ]
    for row in rows:
        lines += [
            f"// {row['case_id']} {row['operation']} {row['role']} -> {row['row_assertion']}",
            f"func RunCase_{row['case_id'].split('-')[-1]}() => boolean",
            "begin",
        ]
        lines += render_case(row)
        lines += ["end;", ""]
    lines += ["func main() => integer", "begin"]
    for row in rows:
...
def build() -> tuple[dict, dict[int, str]]:
    ...
    return evidence, {shard: render_shard(items, shard) for shard, items in grouped.items()}
