// PTO-TEST: {"id":"PTO-AVS-BLOCK-BSTART-STD-STATE-001","source":"asl/block/execution/BSTART.STD.asl","requirements":["PTO-INST-BLOCK-BSTART-STD"],"kind":"state-transition","summary":"Accepted BSTART.STD forms install the unique STD BARG and defer continuation to block commit.","pass_condition":"DIRECT, COND, FALL, IND, and RET publish their exact transfer state and selected continuation.","related_sources":["asl/block/model/dispatch/start.asl","asl/block/model/state/barg.asl"]}
func main() => integer
begin
    ResetProfileState();
    WriteTPC(Zeros{PTO_XLEN} + 0x100);
    let direct = ExecuteCommandInstruction(Zeros{64} + 0x0000a001, 32);
    ...
    let conditional = ExecuteCommandInstruction(Zeros{64} + 0x0000b001, 32);
    ...
    let returned = ExecuteCommandInstruction(Zeros{64} + 0x00007001, 32);
