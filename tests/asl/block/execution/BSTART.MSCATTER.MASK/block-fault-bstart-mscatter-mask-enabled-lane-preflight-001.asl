...
begin
    ResetProfileState();
    ConfigureTile(1, 128, 3, 1, 3, 1, TileDataType_U16,
        TileLayout_RowMajor, TileLocation_Any);
    ConfigureTile(2, 128, 1, 4, 1, 3, TileDataType_U32,
        TileLayout_RowMajor, TileLocation_Any);
    ConfigurePredicateTile(3, 128, 3, 1, 3, 1);
    WriteTileElement(1, 0, 0, Zeros{PTO_XLEN} + 0x1111);
    WriteTileElement(1, 1, 0, Zeros{PTO_XLEN} + 0x2222);
    WriteTileElement(1, 2, 0, Zeros{PTO_XLEN} + 0x3333);
    WriteTileElement(2, 0, 0, Zeros{PTO_XLEN});
    WriteTileElement(2, 0, 1, Ones{PTO_XLEN});
    WriteTileElement(2, 0, 2, Zeros{PTO_XLEN} + 1);
    WriteTilePredicateBit(3, 0, 0, TRUE);
    WriteTilePredicateBit(3, 1, 0, TRUE);
    WriteTilePredicateBit(3, 2, 0, TRUE);
    Store(Zeros{PTO_XLEN} + 4092, 2, Zeros{PTO_XLEN} + 0xaaaa);
    ...
    let observed = LoadUnsigned(Zeros{PTO_XLEN} + 4092, 2);
