// PTO-TEST: {"id":"PTO-AVS-BLOCK-B-FPATR-CSCALE-001","source":"asl/block/attributes/B.FPATR.asl","requirements":["PTO-CUBE-CSCALE-001","PTO-INST-BLOCK-B-FPATR"],"kind":"execution","summary":"Decoded CScale scales explicit FP32 C before Matrix post-processing.","pass_condition":"TMATMUL.ACC with C=4.0, zero dot product, and U8 CScale exponent one publishes FP32 2.0 through a distinct destination.","related_sources":["asl/block/model/dispatch/cube-tmatmul.asl","asl/tile/model/execution/cube.asl"]}
...
begin
    ...
    let c_ready = ConfigureCubeTileForMask(3, 128, 1, 1,
        ...
        TileLocation_Matrix, '1111');
    let scale_ready = ConfigureCubeTileForMask(4, 128, 1, 1,
