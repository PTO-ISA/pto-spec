// PTO-TEST: {"id":"PTO-AVS-BLOCK-TMATMUL-MX-SHARED-RIGHT-001","source":"asl/block/model/dispatch/cube-tmatmul.asl","requirements":["PTO-TMATMUL-MX-CONTRACT-001","PTO-CUBE-MATRIX-SCALE-001","PTO-CUBE-SHARED-TRANSPOSE-001","PTO-B-SUBVIEW-RANGE-001"],"kind":"execution","summary":"TMATMULMX preserves Local ScaleA while TransB changes only a Shared B primary.","pass_condition":"Local E4M3 A and ScaleA plus TransB=1 Shared E5M2 B and ScaleB produce one FP32 destination containing 64; the corresponding Shared scale remains in its fixed K-group-major mapping.","related_sources":["asl/block/model/dispatch/shared-cube-matrix.asl","asl/tile/model/execution/matrix-scale.asl"]}
func main() => integer
begin
    ResetProfileState();
    ...
    ConfigureTileForMask(10, 128, 32, 1, 32, 1, TileDataType_E5M2,
        TileLayout_RowMajor, '1111');
    ConfigureTileForMask(11, 128, 1, 1, 1, 1, TileDataType_E8M0,
        TileLayout_RowMajor, '1111');
    for inner = 0 to 31 looplimit 32 do
        WriteTileElement(0, 0, inner, Zeros{PTO_XLEN} + 2);
        WriteTileElement(10, inner, 0, Zeros{PTO_XLEN} + 1);
    end;
