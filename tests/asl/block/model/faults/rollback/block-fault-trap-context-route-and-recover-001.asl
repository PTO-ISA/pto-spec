...
// PTO-TEST: {"id":"PTO-AVS-BLOCK-TESTTRAPCONTEXTROUTEANDRECOVER-FAULT-001","source":"asl/block/model/faults/rollback.asl","requirements":[],"kind":"fault","summary":"Covers Trap Context Route And Recover.","pass_condition":"TestTrapContextRouteAndRecover completes without assertion failure","related_sources":[]}
...
begin
    ...
    assert _TrapContexts[[1]].tpc == Zeros{PTO_XLEN} + 0x304;
    assert _TrapContexts[[1]].bpc == Zeros{PTO_XLEN} + 0x300;
    ...
    // Corrupt live state without issuing a second architectural B.DIM: a
    // duplicate dimension command correctly traps and would replace the saved
    // context that this recovery point is intended to test.
    _BundleDimensions[[2]] = Ones{PTO_XLEN};
