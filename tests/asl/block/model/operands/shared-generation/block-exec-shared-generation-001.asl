...
pure func SharedGenerationBIOS(size_code: integer) => bits(64)
begin
...
begin
    ...
    let binder = ExecuteCommandInstruction(
        SharedGenerationBIOS(if init then 2 else 0), 32);
    ...
    if init then
        ...
        assert BundleSharedBindingIsReusedDestination(0);
        assert BundleSharedBindingId(0) == ((Zeros{6} + 8) as SharedTileID);
        assert BundleSharedGenerationCapacity(0) == 2;
    end;
...
begin
    ResetProfileState();
    let first = ExecuteSharedWriter(TRUE, FALSE, 0, 1, 0x11);
    ...
    let second = ExecuteSharedWriter(FALSE, TRUE, 1, 1, 0x22);
