// PTO-TEST: {"id":"PTO-AVS-BLOCK-B-IOT-SIZECODES-002","source":"asl/block/operands/B.IOT.asl","requirements":["PTO-INST-BLOCK-B-IOT"],"kind":"execution","summary":"Decoded B.IOT destination paths cover every legal and reserved Local SizeCode.","pass_condition":"Codes 1 through 10 reach Local destination binding with the all-PE semantic mask; codes 11 through 15 reject before binding effects.","related_sources":["asl/block/model/schema/profile-encoding.asl","asl/block/model/operands/tile-bindings.asl"]}
...
begin
    ...
    for reserved_size = 11 to 15 looplimit 5 do
        ResetProfileState();
        let started = ExecuteCommandInstruction(
            BIOTSizeCodeStart(Zeros{5} + 27), 32);
        let status = ExecuteCommandInstruction(
            BIOTSizeCodeDestination(
                (Zeros{4} + reserved_size) as bits(4), '111'), 32);
        assert started == CommandExecution_Executed;
