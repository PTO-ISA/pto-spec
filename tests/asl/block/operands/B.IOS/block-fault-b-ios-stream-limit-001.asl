...
pure func SharedBindingInstruction(id: bits(6)) => bits(64)
begin
    var instruction = Zeros{64} + 0x00001013;
    instruction[25:20] = id;
...
begin
    ...
    let first = ExecuteCommandInstruction(SharedBindingInstruction(Zeros{6}), 32);
    assert first == CommandExecution_Executed;
    let duplicate = ExecuteCommandInstruction(
        SharedBindingInstruction(Zeros{6}), 32);
    ...
    for id = 0 to 3 do
        let status = ExecuteCommandInstruction(
            SharedBindingInstruction(Zeros{6} + id), 32);
        assert status == CommandExecution_Executed;
    end;
    let fifth = ExecuteCommandInstruction(
        SharedBindingInstruction(Zeros{6} + 4), 32);
