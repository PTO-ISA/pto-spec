// PTO-TEST: {"id":"PTO-AVS-ARCH-E1M2X2-DECOMP-001","source":"asl/arch/data-types/formats/e1m2x2.asl","requirements":["PTO-NUMERIC-FINITE-DECOMPOSITION-001"],"kind":"boundary","summary":"E1M2X2 exposes its exact descriptor and finite-value decomposition.","pass_condition":"Field metadata, finite boundaries, unavailable special values, and existing value-class rules agree.","related_sources":["asl/arch/data-types/numeric-formats.asl","asl/arch/features/mx-formats.asl"]}
...
func AssertE1M2X2Lane(raw: integer {0..15}, negative: boolean,
                       significand: Word,
                       exponent: integer {-1074..1023})
begin
    AssertNumericFiniteDecomposition(TileDataType_E1M2X2,
        Zeros{PTO_XLEN} + raw, negative, significand, exponent);
end;

func TestE1M2X2NumericFormat()
begin
    let descriptor = TileNumericFormatDescriptor(TileDataType_E1M2X2);
    ...
    AssertE1M2X2Lane(1, FALSE, Zeros{PTO_XLEN} + 1, -2);
    AssertE1M2X2Lane(2, FALSE, Zeros{PTO_XLEN} + 2, -2);
    AssertE1M2X2Lane(3, FALSE, Zeros{PTO_XLEN} + 3, -2);
    AssertE1M2X2Lane(4, FALSE, Zeros{PTO_XLEN} + 4, -2);
    AssertE1M2X2Lane(5, FALSE, Zeros{PTO_XLEN} + 5, -2);
    AssertE1M2X2Lane(6, FALSE, Zeros{PTO_XLEN} + 6, -2);
    AssertE1M2X2Lane(7, FALSE, Zeros{PTO_XLEN} + 7, -2);
    AssertE1M2X2Lane(8, TRUE, Zeros{PTO_XLEN}, 0);
    AssertE1M2X2Lane(9, TRUE, Zeros{PTO_XLEN} + 1, -2);
    AssertE1M2X2Lane(10, TRUE, Zeros{PTO_XLEN} + 2, -2);
    AssertE1M2X2Lane(11, TRUE, Zeros{PTO_XLEN} + 3, -2);
    AssertE1M2X2Lane(12, TRUE, Zeros{PTO_XLEN} + 4, -2);
    AssertE1M2X2Lane(13, TRUE, Zeros{PTO_XLEN} + 5, -2);
    AssertE1M2X2Lane(14, TRUE, Zeros{PTO_XLEN} + 6, -2);
    AssertE1M2X2Lane(15, TRUE, Zeros{PTO_XLEN} + 7, -2);
