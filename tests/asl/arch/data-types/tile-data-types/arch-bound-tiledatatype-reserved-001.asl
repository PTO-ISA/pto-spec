// PTO-TEST: {"id":"PTO-AVS-ARCH-TILEDATATYPE-RESERVED-001","source":"asl/arch/data-types/tile-data-types.asl","requirements":[],"kind":"boundary","summary":"The five-bit Tile DataType namespace assigns the frozen codes and rejects exactly five future-extension codes.","pass_condition":"All 32 five-bit encodings have the required assigned or reserved disposition and assigned encodings round-trip.","related_sources":["asl/block/attributes/B.DATR.asl"]}
func TestTileDataTypeReservedEncodings()
begin
    for code = 0 to 31 looplimit 32 do
        let encoded = (Zeros{5} + code) as TileDataTypeEncoding;
        let expected_reserved = (22 <= code && code <= 23) ||
                                (29 <= code && code <= 31);
    ...
    assert TileDataTypeToEncoding(TileDataType_E6M2) == Zeros{5} + 15;
    assert TileDataTypeToEncoding(TileDataType_S4X2) == Zeros{5} + 20;
    assert TileDataTypeToEncoding(TileDataType_RCPE6M2) == Zeros{5} + 21;
