// PTO-TEST: {"id":"PTO-AVS-TILE-TMATMUL-MX-BIAS-EXEC-001","source":"asl/tile/matrix-and-matrix-vector/matrix-matrix/TMATMUL_MX_BIAS.asl","requirements":["PTO-TMATMUL-MX-BIAS-CONTRACT-001","PTO-INST-TILE-TMATMUL-MX-BIAS"],"kind":"execution","summary":"TMATMUL_MX_BIAS executes its direct 1x1 CUBE_M32 semantic handler","pass_condition":"the legal FP16 operands commit the expected FP32 result with matching M32 Bias and destination layouts","related_sources":["asl/tile/model/legality/matrix-shape.asl","asl/tile/model/execution/cube.asl"]}
...
begin
    ...
    let cube_configuration_1 = ConfigureCubeTile(1, 256, 1, 1, TileDataType_FP16,
        TileLayout_CUBE_M32);
    ...
    let cube_configuration_3 = ConfigureCubeTile(4, 256, 1, 1, TileDataType_FP32,
        TileLayout_CUBE_M32);
    ...
    assert !TileOperandsLegal_TMATMUL_MX_BIAS(4, 1, 0, 2, 0, 5);
    assert !TileOperandsLegal_TMATMUL_MX_BIAS(4, 1, 0, 2, 0, 6);
    assert !TileOperandsLegal_TMATMUL_MX_BIAS(4, 1, 0, 2, 0, 7);
    return 0;
end;
