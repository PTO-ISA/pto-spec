...
// PTO-TEST: {"id":"PTO-AVS-TILE-TESTTILESELECTORCLOSUREEXTENSIONS-BOUNDARY-001","source":"asl/tile/model/dispatch/top-level.asl","requirements":[],"kind":"boundary","summary":"migrated independent behavior point for TestTileSelectorClosureExtensions","pass_condition":"TestTileSelectorClosureExtensions completes without assertion failure","related_sources":[]}
...
begin
    ...
    ExecuteTileFillScalar(5, Zeros{PTO_XLEN} + 1);
    ConfigureTile(11, 256, 2, 1, 2, 1, TileDataType_FP32,
        TileLayout_RowMajor, TileLocation_Any);
    TGEMV_MX_BIAS(11, 2, 5, 8, 9, 5);
    assert ReadTileElement(11, 0, 0) == Zeros{PTO_XLEN} + 4;
    TGEMV_MX_ACC(11, 11, 2, 5, 8, 9);
    assert ReadTileElement(11, 0, 0) == Zeros{PTO_XLEN} + 7;
end;
func main() => integer
begin
    ResetProfileState();
