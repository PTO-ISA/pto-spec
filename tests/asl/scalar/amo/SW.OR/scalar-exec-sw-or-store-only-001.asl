// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-OR-STORE-001","source":"asl/scalar/amo/SW.OR.asl","requirements":["PTO-INST-SCALAR-SW-OR"],"kind":"execution","summary":"SW.OR performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSWORStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SW_OR() == Atomic_OR;
    assert InstructionContractAtomicSizeBytes_SW_OR() == 4;
    assert !InstructionContractPublishesOldValue_SW_OR();
end;
