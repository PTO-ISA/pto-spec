// PTO-TEST: {"id":"PTO-AVS-SCALAR-LD-AND-PRECISE-001","source":"asl/scalar/amo/LD.AND.asl","requirements":["PTO-INST-SCALAR-LD-AND"],"kind":"fault","summary":"LD.AND alignment fault is precise and restores the original TPC","pass_condition":"trap routing, destination, memory, event, reservation, queues, and recovery match the LD.AND contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/fault-precision.asl"]}
func CheckLDANDPreciseAlignmentFault()
begin
...
begin
    ResetProfileState();
    CheckLDANDPreciseAlignmentFault();
    return 0;
end;
