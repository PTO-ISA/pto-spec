// PTO-TEST: {"id":"PTO-AVS-SCALAR-LW-ADD-FORMS-001","source":"asl/scalar/amo/LW.ADD.asl","requirements":["PTO-INST-SCALAR-LW-ADD"],"kind":"boundary","summary":"LW.ADD reads T and U sources and pushes its result to U with default modifiers","pass_condition":"relative sources, U push, relaxed ordering, nonoverlap reservation, and TPC match the LW.ADD contract","related_sources":["asl/scalar/model/types/operands.asl","asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl"]}
func CheckLWADDRelativeDefaultForm()
begin
    ...
    assert ReadTemporaryQueue(FALSE, 0) == NormalizeAtomicReturn(old_value, 4);
