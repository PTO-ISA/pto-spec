// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-SMIN-STORE-001","source":"asl/scalar/amo/SW.SMIN.asl","requirements":["PTO-INST-SCALAR-SW-SMIN"],"kind":"execution","summary":"SW.SMIN performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSWSMINStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SW_SMIN() == Atomic_SMIN;
    assert InstructionContractAtomicSizeBytes_SW_SMIN() == 4;
    assert !InstructionContractPublishesOldValue_SW_SMIN();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSWSMINStoreOnly();
    return 0;
end;
