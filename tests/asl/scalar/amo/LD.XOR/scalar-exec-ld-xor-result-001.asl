// PTO-TEST: {"id":"PTO-AVS-SCALAR-LD-XOR-RESULT-001","source":"asl/scalar/amo/LD.XOR.asl","requirements":["PTO-INST-SCALAR-LD-XOR"],"kind":"execution","summary":"LD.XOR performs its atomic update and publishes the prior value","pass_condition":"memory, destination, event, ordering, reservation, and TPC match the LD.XOR contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckLDXORResult()
begin
    let address = Zeros{PTO_XLEN} + 256;
    let old_value = Zeros{PTO_XLEN} + 5;
    let operand = Zeros{PTO_XLEN} + 7;
    let expected = Zeros{PTO_XLEN} + 2;
    assert AtomicValueSized(Atomic_XOR, old_value, operand, 8) == expected;
    ...
    assert InstructionContractAtomicOperation_LD_XOR() == Atomic_XOR;
    assert InstructionContractAtomicSizeBytes_LD_XOR() == 8;
    assert InstructionContractPublishesOldValue_LD_XOR();
    assert !InstructionContractSignExtendsOldValue_LD_XOR();
end;
