// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-UMAX-STORE-001","source":"asl/scalar/amo/SW.UMAX.asl","requirements":["PTO-INST-SCALAR-SW-UMAX"],"kind":"execution","summary":"SW.UMAX performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSWUMAXStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SW_UMAX() == Atomic_UMAX;
    assert InstructionContractAtomicSizeBytes_SW_UMAX() == 4;
    assert !InstructionContractPublishesOldValue_SW_UMAX();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSWUMAXStoreOnly();
    return 0;
end;
