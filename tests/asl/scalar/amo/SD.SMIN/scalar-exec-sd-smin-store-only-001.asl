// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-SMIN-STORE-001","source":"asl/scalar/amo/SD.SMIN.asl","requirements":["PTO-INST-SCALAR-SD-SMIN"],"kind":"execution","summary":"SD.SMIN performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDSMINStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_SMIN() == Atomic_SMIN;
    assert InstructionContractAtomicSizeBytes_SD_SMIN() == 8;
    assert !InstructionContractPublishesOldValue_SD_SMIN();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSDSMINStoreOnly();
    return 0;
end;
