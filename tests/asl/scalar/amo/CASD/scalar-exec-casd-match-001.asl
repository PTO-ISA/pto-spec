// PTO-TEST: {"id":"PTO-AVS-SCALAR-CASD-MATCH-001","source":"asl/scalar/amo/CASD.asl","requirements":["PTO-INST-SCALAR-CASD"],"kind":"execution","summary":"CASD matching compare conditionally stores and publishes the prior value","pass_condition":"memory, destination, atomic event, reservation, ordering, and TPC match the CASD contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckCASDMatch()
begin
    ...
    assert InstructionContractCompareSizeBytes_CASD() == 8;
    assert InstructionContractHasFarField_CASD() == FALSE;
end;
