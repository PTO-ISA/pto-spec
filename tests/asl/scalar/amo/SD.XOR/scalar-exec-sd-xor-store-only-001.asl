// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-XOR-STORE-001","source":"asl/scalar/amo/SD.XOR.asl","requirements":["PTO-INST-SCALAR-SD-XOR"],"kind":"execution","summary":"SD.XOR performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDXORStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_XOR() == Atomic_XOR;
    assert InstructionContractAtomicSizeBytes_SD_XOR() == 8;
    assert !InstructionContractPublishesOldValue_SD_XOR();
end;
