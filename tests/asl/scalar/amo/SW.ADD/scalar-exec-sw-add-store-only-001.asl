// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-ADD-STORE-001","source":"asl/scalar/amo/SW.ADD.asl","requirements":["PTO-INST-SCALAR-SW-ADD"],"kind":"execution","summary":"SW.ADD performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSWADDStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SW_ADD() == Atomic_ADD;
    assert InstructionContractAtomicSizeBytes_SW_ADD() == 4;
    assert !InstructionContractPublishesOldValue_SW_ADD();
end;
