// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-AND-STORE-001","source":"asl/scalar/amo/SD.AND.asl","requirements":["PTO-INST-SCALAR-SD-AND"],"kind":"execution","summary":"SD.AND performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDANDStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_AND() == Atomic_AND;
    assert InstructionContractAtomicSizeBytes_SD_AND() == 8;
    assert !InstructionContractPublishesOldValue_SD_AND();
end;
