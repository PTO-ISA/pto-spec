// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-AND-PRECISE-001","source":"asl/scalar/amo/SD.AND.asl","requirements":["PTO-INST-SCALAR-SD-AND"],"kind":"fault","summary":"SD.AND alignment fault is precise and restores the original TPC","pass_condition":"trap routing, saved context, memory, event, reservation, register, queue, and recovery state match the SD.AND contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/fault-precision.asl"]}
func CheckSDANDPreciseAlignmentFault()
begin
    ...
    var instruction: bits(48) = Zeros{48} + 0x1000500b;
...
begin
    ResetProfileState();
    CheckSDANDPreciseAlignmentFault();
    return 0;
end;
