// PTO-TEST: {"id":"PTO-AVS-SCALAR-LD-SMIN-RESULT-001","source":"asl/scalar/amo/LD.SMIN.asl","requirements":["PTO-INST-SCALAR-LD-SMIN"],"kind":"execution","summary":"LD.SMIN performs its atomic update and publishes the prior value","pass_condition":"memory, destination, event, ordering, reservation, and TPC match the LD.SMIN contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckLDSMINResult()
begin
    let address = Zeros{PTO_XLEN} + 256;
    let old_value = Ones{PTO_XLEN};
    let operand = Zeros{PTO_XLEN} + 5;
    let expected = Ones{PTO_XLEN};
    assert AtomicValueSized(Atomic_SMIN, old_value, operand, 8) == expected;
    ...
    assert InstructionContractAtomicOperation_LD_SMIN() == Atomic_SMIN;
    assert InstructionContractAtomicSizeBytes_LD_SMIN() == 8;
    assert InstructionContractPublishesOldValue_LD_SMIN();
    assert !InstructionContractSignExtendsOldValue_LD_SMIN();
end;
