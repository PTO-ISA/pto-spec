// PTO-TEST: {"id":"PTO-AVS-SCALAR-LW-AND-RESULT-001","source":"asl/scalar/amo/LW.AND.asl","requirements":["PTO-INST-SCALAR-LW-AND"],"kind":"execution","summary":"LW.AND performs its atomic update and publishes the prior value","pass_condition":"memory, destination, event, ordering, reservation, and TPC match the LW.AND contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckLWANDResult()
begin
    ...
    assert AtomicValueSized(Atomic_AND, old_value, operand, 4) == expected;
    ...
    assert InstructionContractAtomicOperation_LW_AND() == Atomic_AND;
    assert InstructionContractAtomicSizeBytes_LW_AND() == 4;
    assert InstructionContractPublishesOldValue_LW_AND();
    assert InstructionContractSignExtendsOldValue_LW_AND();
end;
