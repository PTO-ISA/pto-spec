// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-AND-STORE-001","source":"asl/scalar/amo/SW.AND.asl","requirements":["PTO-INST-SCALAR-SW-AND"],"kind":"execution","summary":"SW.AND performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSWANDStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SW_AND() == Atomic_AND;
    assert InstructionContractAtomicSizeBytes_SW_AND() == 4;
    assert !InstructionContractPublishesOldValue_SW_AND();
end;
