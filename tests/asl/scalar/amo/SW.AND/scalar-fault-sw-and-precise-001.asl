// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-AND-PRECISE-001","source":"asl/scalar/amo/SW.AND.asl","requirements":["PTO-INST-SCALAR-SW-AND"],"kind":"fault","summary":"SW.AND alignment fault is precise and restores the original TPC","pass_condition":"trap routing, saved context, memory, event, reservation, register, queue, and recovery state match the SW.AND contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/fault-precision.asl"]}
func CheckSWANDPreciseAlignmentFault()
begin
    let aligned_address = Zeros{PTO_XLEN} + 256;
    let fault_address = aligned_address + 1;
    let old_value = Zeros{PTO_XLEN} + 0xf0f0f0f0;
    ...
    var instruction: bits(48) = Zeros{48} + 0x1000300b;
...
begin
    ResetProfileState();
    CheckSWANDPreciseAlignmentFault();
    return 0;
end;
