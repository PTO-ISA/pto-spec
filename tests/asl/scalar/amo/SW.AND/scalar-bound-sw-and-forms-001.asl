// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-AND-FORMS-001","source":"asl/scalar/amo/SW.AND.asl","requirements":["PTO-INST-SCALAR-SW-AND"],"kind":"boundary","summary":"SW.AND reads T and U relative sources with relaxed local defaults","pass_condition":"relative sources, default modifiers, memory, ordering, reservation, queues, and TPC match the SW.AND contract","related_sources":["asl/scalar/model/types/operands.asl","asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl"]}
func CheckSWANDRelativeDefaultForm()
begin
    let address = Zeros{PTO_XLEN} + 256;
    let old_value = Zeros{PTO_XLEN} + 0xf0f0f0f0;
    let operand = Zeros{PTO_XLEN} + 0x0ff00ff0;
    let expected = Zeros{PTO_XLEN} + 0x00f000f0;
