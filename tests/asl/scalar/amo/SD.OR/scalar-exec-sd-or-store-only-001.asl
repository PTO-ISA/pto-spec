// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-OR-STORE-001","source":"asl/scalar/amo/SD.OR.asl","requirements":["PTO-INST-SCALAR-SD-OR"],"kind":"execution","summary":"SD.OR performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDORStoreOnly()
begin
    let address = Zeros{PTO_XLEN} + 320;
    let old_value = Zeros{PTO_XLEN} + 0xf000f000;
    let operand = Zeros{PTO_XLEN} + 0x0ff00ff0;
    let expected = Zeros{PTO_XLEN} + 0xfff0fff0;

    // Seed values that expose an accidental RegDst or queue result.
    WriteGPR(5, Zeros{PTO_XLEN} + 0x55);
    PushTemporaryQueue(TRUE, Zeros{PTO_XLEN} + 0x66);
    PushTemporaryQueue(FALSE, Zeros{PTO_XLEN} + 0x77);
    let old_t = ReadTemporaryQueue(TRUE, 0);
    let old_u = ReadTemporaryQueue(FALSE, 0);

    Store(address, 8, old_value);
    _ReservationValid = TRUE;
    _ReservationAddress = address;
    _ReservationSize = 8;
    WriteGPR(2, address);
    WriteGPR(3, operand);
    WriteTPC(Zeros{PTO_XLEN} + 0x40);
    ClearFault();
    StartMemoryEventCapture(0);

    var instruction: bits(48) = Zeros{48} + 0x2000500b;
    instruction[19:15] = Zeros{5} + 2;
    instruction[24:20] = Zeros{5} + 3;
    instruction[25] = '1';
    instruction[27] = '1';

    let status = ExecuteScalarInstruction(instruction, 32);
    assert status == ScalarExecution_Executed;
    assert _LastFault == Fault_None;
    assert LoadTranslatedUnsigned(address, 8) == expected;
    assert ReadGPR(5) == Zeros{PTO_XLEN} + 0x55;
    assert ReadTemporaryQueue(TRUE, 0) == old_t;
    assert ReadTemporaryQueue(FALSE, 0) == old_u;
    assert ReadTPC() == Zeros{PTO_XLEN} + 0x44;
    assert !_ReservationValid;

    assert _MemoryEventCount == 1;
    assert _MemoryEvents[[0]].kind == MemoryEvent_Atomic;
    assert _MemoryEvents[[0]].address == address;
    assert _MemoryEvents[[0]].size_bytes == 8;
    assert _MemoryEvents[[0]].read_value ==
        NormalizeMemoryAccessValue(old_value, 8);
    assert _MemoryEvents[[0]].write_value == expected;
    assert _MemoryEvents[[0]].write_performed;
    assert _MemoryEvents[[0]].order == MemoryOrder_Release;

    assert InstructionContractAtomicOperation_SD_OR() == Atomic_OR;
    assert InstructionContractAtomicSizeBytes_SD_OR() == 8;
    assert !InstructionContractPublishesOldValue_SD_OR();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSDORStoreOnly();
    return 0;
end;
