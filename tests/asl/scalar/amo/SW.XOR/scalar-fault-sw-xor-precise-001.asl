// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-XOR-PRECISE-001","source":"asl/scalar/amo/SW.XOR.asl","requirements":["PTO-INST-SCALAR-SW-XOR"],"kind":"fault","summary":"SW.XOR alignment fault is precise and restores the original TPC","pass_condition":"trap routing, saved context, memory, event, reservation, register, queue, and recovery state match the SW.XOR contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/fault-precision.asl"]}
func CheckSWXORPreciseAlignmentFault()
begin
    let aligned_address = Zeros{PTO_XLEN} + 256;
    let fault_address = aligned_address + 1;
    let old_value = Zeros{PTO_XLEN} + 0xffff0000;
    let operand = Zeros{PTO_XLEN} + 0xff00ff00;
    ...
    var instruction: bits(48) = Zeros{48} + 0x3000300b;
    ...
    assert LoadTranslatedUnsigned(aligned_address, 4) ==
...
begin
    ResetProfileState();
    CheckSWXORPreciseAlignmentFault();
    return 0;
end;
