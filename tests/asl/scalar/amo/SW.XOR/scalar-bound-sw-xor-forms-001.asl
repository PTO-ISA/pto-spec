// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-XOR-FORMS-001","source":"asl/scalar/amo/SW.XOR.asl","requirements":["PTO-INST-SCALAR-SW-XOR"],"kind":"boundary","summary":"SW.XOR reads T and U relative sources with relaxed local defaults","pass_condition":"relative sources, default modifiers, memory, ordering, reservation, queues, and TPC match the SW.XOR contract","related_sources":["asl/scalar/model/types/operands.asl","asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl"]}
func CheckSWXORRelativeDefaultForm()
begin
    let address = Zeros{PTO_XLEN} + 256;
    let old_value = Zeros{PTO_XLEN} + 0xffff0000;
    let operand = Zeros{PTO_XLEN} + 0xff00ff00;
    let expected = Zeros{PTO_XLEN} + 0x00ffff00;
