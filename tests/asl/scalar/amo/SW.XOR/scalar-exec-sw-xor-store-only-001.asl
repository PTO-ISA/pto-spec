// PTO-TEST: {"id":"PTO-AVS-SCALAR-SW-XOR-STORE-001","source":"asl/scalar/amo/SW.XOR.asl","requirements":["PTO-INST-SCALAR-SW-XOR"],"kind":"execution","summary":"SW.XOR performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSWXORStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SW_XOR() == Atomic_XOR;
    assert InstructionContractAtomicSizeBytes_SW_XOR() == 4;
    assert !InstructionContractPublishesOldValue_SW_XOR();
end;
