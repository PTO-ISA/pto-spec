// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-ADD-STORE-001","source":"asl/scalar/amo/SD.ADD.asl","requirements":["PTO-INST-SCALAR-SD-ADD"],"kind":"execution","summary":"SD.ADD performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDADDStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_ADD() == Atomic_ADD;
    assert InstructionContractAtomicSizeBytes_SD_ADD() == 8;
    assert !InstructionContractPublishesOldValue_SD_ADD();
end;
