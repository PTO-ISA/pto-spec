// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-SMAX-STORE-001","source":"asl/scalar/amo/SD.SMAX.asl","requirements":["PTO-INST-SCALAR-SD-SMAX"],"kind":"execution","summary":"SD.SMAX performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDSMAXStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_SMAX() == Atomic_SMAX;
    assert InstructionContractAtomicSizeBytes_SD_SMAX() == 8;
    assert !InstructionContractPublishesOldValue_SD_SMAX();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSDSMAXStoreOnly();
    return 0;
end;
