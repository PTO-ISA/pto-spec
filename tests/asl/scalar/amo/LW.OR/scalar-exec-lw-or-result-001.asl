// PTO-TEST: {"id":"PTO-AVS-SCALAR-LW-OR-RESULT-001","source":"asl/scalar/amo/LW.OR.asl","requirements":["PTO-INST-SCALAR-LW-OR"],"kind":"execution","summary":"LW.OR performs its atomic update and publishes the prior value","pass_condition":"memory, destination, event, ordering, reservation, and TPC match the LW.OR contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckLWORResult()
begin
    ...
    var instruction: bits(48) = Zeros{48} + 0x2000200b;
    ...
    assert InstructionContractAtomicOperation_LW_OR() == Atomic_OR;
    assert InstructionContractAtomicSizeBytes_LW_OR() == 4;
    assert InstructionContractPublishesOldValue_LW_OR();
    assert InstructionContractSignExtendsOldValue_LW_OR();
end;
