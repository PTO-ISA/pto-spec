// PTO-TEST: {"id":"PTO-AVS-SCALAR-LW-SMIN-RESULT-001","source":"asl/scalar/amo/LW.SMIN.asl","requirements":["PTO-INST-SCALAR-LW-SMIN"],"kind":"execution","summary":"LW.SMIN performs its atomic update and publishes the prior value","pass_condition":"memory, destination, event, ordering, reservation, and TPC match the LW.SMIN contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckLWSMINResult()
begin
    ...
    var instruction: bits(48) = Zeros{48} + 0x5000200b;
    ...
    assert InstructionContractAtomicOperation_LW_SMIN() == Atomic_SMIN;
    assert InstructionContractAtomicSizeBytes_LW_SMIN() == 4;
    assert InstructionContractPublishesOldValue_LW_SMIN();
    assert InstructionContractSignExtendsOldValue_LW_SMIN();
end;
