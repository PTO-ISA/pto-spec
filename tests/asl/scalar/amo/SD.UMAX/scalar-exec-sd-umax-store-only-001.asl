// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-UMAX-STORE-001","source":"asl/scalar/amo/SD.UMAX.asl","requirements":["PTO-INST-SCALAR-SD-UMAX"],"kind":"execution","summary":"SD.UMAX performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDUMAXStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_UMAX() == Atomic_UMAX;
    assert InstructionContractAtomicSizeBytes_SD_UMAX() == 8;
    assert !InstructionContractPublishesOldValue_SD_UMAX();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSDUMAXStoreOnly();
    return 0;
end;
