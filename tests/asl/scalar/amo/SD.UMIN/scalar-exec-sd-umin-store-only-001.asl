// PTO-TEST: {"id":"PTO-AVS-SCALAR-SD-UMIN-STORE-001","source":"asl/scalar/amo/SD.UMIN.asl","requirements":["PTO-INST-SCALAR-SD-UMIN"],"kind":"execution","summary":"SD.UMIN performs one store-only atomic update without publishing the old value","pass_condition":"memory, event, ordering, reservation, queues, registers, and TPC match the mnemonic contract","related_sources":["asl/scalar/model/dispatch/amo.asl","asl/scalar/model/amo/semantics.asl","asl/arch/memory-model/atomicity.asl"]}
func CheckSDUMINStoreOnly()
begin
    ...
    assert InstructionContractAtomicOperation_SD_UMIN() == Atomic_UMIN;
    assert InstructionContractAtomicSizeBytes_SD_UMIN() == 8;
    assert !InstructionContractPublishesOldValue_SD_UMIN();
end;

func main() => integer
begin
    ResetProfileState();
    CheckSDUMINStoreOnly();
    return 0;
end;
