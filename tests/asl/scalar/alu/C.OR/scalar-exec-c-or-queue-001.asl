// PTO-TEST: {"id":"PTO-AVS-SCALAR-C-OR-QUEUE-001","source":"asl/scalar/alu/C.OR.asl","requirements":["PTO-INST-SCALAR-C-OR"],"kind":"execution","summary":"C.OR computes the bitwise inclusive OR of snapshotted T and U sources","pass_condition":"decoded execution snapshots both relative sources, pushes one exact result to T, and advances TPC by two bytes","related_sources":["asl/scalar/model/dispatch/alu.asl","asl/scalar/model/types/operands.asl"]}
func main() => integer
begin
    ResetProfileState();
    ...
    assert InstructionContractResult_C_OR(
        Zeros{PTO_XLEN} + 5,
        Zeros{PTO_XLEN} + 3) == Zeros{PTO_XLEN} + 0x0000000000000007;
    return 0;
end;
