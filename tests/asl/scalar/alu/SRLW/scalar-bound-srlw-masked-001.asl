// PTO-TEST: {"id":"PTO-AVS-SCALAR-SRLW-MASKED-001","source":"asl/scalar/alu/SRLW.asl","requirements":["PTO-INST-SCALAR-SRLW"],"kind":"boundary","summary":"SRLW masks the register count and snapshots T and U sources before U publication","pass_condition":"masked count, non-consuming sources, queue alias order, and mnemonic contract match SRLW","related_sources":["asl/scalar/model/types/operands.asl","asl/scalar/model/dispatch/alu.asl"]}
func main() => integer
begin
    ResetProfileState();
    ...
    assert InstructionContractShiftAmount_SRLW(Zeros{PTO_XLEN} + 32) == 0;
    assert InstructionContractResult_SRLW(
