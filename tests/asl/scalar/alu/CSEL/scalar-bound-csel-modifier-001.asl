...
// PTO-TEST: {"id":"PTO-AVS-SCALAR-CSEL-MODIFIER-001","source":"asl/scalar/alu/CSEL.asl","requirements":["PTO-INST-SCALAR-CSEL"],"kind":"boundary","summary":"CSEL keeps raw modifier codes 00, 01, and 10 as unmodified aliases and negates only raw code 11","pass_condition":"the CSEL-specific modifier decoder and selected-right transformation match ADR-0059","related_sources":["asl/scalar/model/dispatch/decode.asl","asl/scalar/model/alu/semantics.asl"]}
...
begin
    ...
    assert InstructionContractRightModifier_CSEL('10') == ScalarRight_None;
    assert InstructionContractRightModifier_CSEL('11') == ScalarRight_NegateOrNot;

    assert ApplySelectModifier(
        value,
        InstructionContractRightModifier_CSEL('10')) == value;
    assert ApplySelectModifier(
        value,
        InstructionContractRightModifier_CSEL('11')) == Zeros{PTO_XLEN} - value;
    return 0;
end;
