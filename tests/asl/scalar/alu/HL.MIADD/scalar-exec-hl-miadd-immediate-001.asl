// PTO-TEST: {"id":"PTO-AVS-SCALAR-HL-MIADD-IMMEDIATE-001","source":"asl/scalar/alu/HL.MIADD.asl","requirements":["PTO-INST-SCALAR-HL-MIADD"],"kind":"execution","summary":"HL.MIADD applies its unsigned 19-bit immediate to the right source","pass_condition":"Decoded execution and the mnemonic result contract agree on the representative immediate result","related_sources":["asl/scalar/model/alu/semantics.asl"]}
func main() => integer
begin
    ResetProfileState();
    ...
    WriteGPR(1, left);
    WriteGPR(2, right);
    var instruction: bits(48) = Zeros{48} + 0x0000004d000e;
    ...
    assert ReadGPR(3) == Zeros{PTO_XLEN} + 17;
    assert InstructionContractResult_HL_MIADD(left, right, immediate) == Zeros{PTO_XLEN} + 17;
    return 0;
end;
