// PTO-TEST: {"id":"PTO-AVS-SCALAR-C-ADD-QUEUE-001","source":"asl/scalar/alu/C.ADD.asl","requirements":["PTO-INST-SCALAR-C-ADD"],"kind":"execution","summary":"C.ADD adds the snapshotted T and U sources modulo XLEN","pass_condition":"decoded execution snapshots both relative sources, pushes one exact result to T, and advances TPC by two bytes","related_sources":["asl/scalar/model/dispatch/alu.asl","asl/scalar/model/types/operands.asl"]}
func main() => integer
begin
    ResetProfileState();
    ...
    assert InstructionContractResult_C_ADD(
        Zeros{PTO_XLEN} + 5,
        Zeros{PTO_XLEN} + 3) == Zeros{PTO_XLEN} + 0x0000000000000008;
    return 0;
end;
