// PTO-TEST: {"id":"PTO-AVS-SCALAR-C-SUB-QUEUE-001","source":"asl/scalar/alu/C.SUB.asl","requirements":["PTO-INST-SCALAR-C-SUB"],"kind":"execution","summary":"C.SUB subtracts the snapshotted U source from the snapshotted T source","pass_condition":"decoded execution snapshots both relative sources, pushes one exact result to T, and advances TPC by two bytes","related_sources":["asl/scalar/model/dispatch/alu.asl","asl/scalar/model/types/operands.asl"]}
func main() => integer
begin
    ResetProfileState();
    ...
    assert InstructionContractResult_C_SUB(
        Zeros{PTO_XLEN} + 5,
        Zeros{PTO_XLEN} + 3) == Zeros{PTO_XLEN} + 0x0000000000000002;
    return 0;
end;
