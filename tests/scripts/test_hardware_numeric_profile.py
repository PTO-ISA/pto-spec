...
class HardwareNumericProfileTest(unittest.TestCase):
    ...
    def test_current_release_owns_the_current_numeric_profile(self) -> None:
        self.assertEqual(self.profile["isa_release"], "0.58.4")
        self.assertEqual(
            self.profile["profile_id"],
            "pto-hardware-numeric-0.58.4-ieee-v1",
        )
        generator = (ROOT / "scripts/generate-release-manifest").read_text(
            encoding="utf-8"
        )
        self.assertIn(
            'NUMERIC_PROFILE_ID = "pto-hardware-numeric-0.58.4-ieee-v1"',
            generator,
        )
        self.assertIn('NUMERIC_PROFILE_RELEASE = "0.58.4"', generator)
